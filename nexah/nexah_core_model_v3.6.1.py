...
def nexah_detection(t, voltage):
    ...
    # thresholds (adjusted)
    composite_th = 2.5
    collapse_th = 2.4   # 🔥 relaxed

    # stability controls
    MIN_TIME = 10
    PERSISTENCE = 4
    REQUIRED_RATIO = 0.75  # 🔥 soft persistence
    ...
    for i in range(len(t)):
        ...
        if condition_now:
            ...
            # 🔥 SOFT persistence instead of ALL
...
def run():
    ...
    plt.title("NEXAH v3.6.1 – IEEE Detection (Soft Stable)")
    plt.legend()
    plt.xlabel("time")
    plt.ylabel("voltage")
