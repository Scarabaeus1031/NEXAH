...
def robustness_test(n_runs=20):
    ...
    for i in range(n_runs):
        params = MultiNodeParams(
            seed=i,
            coupling_k=np.random.uniform(0.28, 0.42),
            split_push_gain=np.random.uniform(0.18, 0.28),
        )

        sim = simulate(params)
        ind = compute_indicators(sim, params)
        split, classic, lead, _ = detect(sim, ind, params)
        ...
        print(f"run {i:02d} | split={split:.2f} | lead={lead}")

    leads_clean = [l for l in leads if l is not None]

    print("\n--- Summary ---")

    if leads_clean:
        print(f"mean lead: {np.mean(leads_clean):.2f}s")
        print(f"std lead:  {np.std(leads_clean):.2f}s")
        print(f"min lead:  {np.min(leads_clean):.2f}s")
        print(f"max lead:  {np.max(leads_clean):.2f}s")

    print(f"success rate: {success}/{n_runs}")
