...
print("Components:", metrics["components"])        gx = grads[1][yi % SIZE, xi % SIZE]
        gy = grads[0][yi % SIZE, xi % SIZE]
...
labeled, num_features = label(symbol_mask)
symbol_centroids = center_of_mass(symbol_mask, labeled, range(1, num_features + 1))

symbol_centroids = np.array(symbol_centroids) if len(symbol_centroids) > 0 else np.zeros((0, 2))

# --------------------------------------------------
# LEVEL 32 — SEMANTIC GROUPING
# --------------------------------------------------
...
if len(symbol_centroids) > 0:
    ...
    for i in range(len(symbol_centroids)):
        ...
        while stack:
            n = stack.pop()
            for j in range(len(symbol_centroids)):
                if j not in visited and dist_matrix[n][j] < GROUP_DISTANCE:
                    visited.add(j)
                    group.append(j)
                    stack.append(j)
...
# --------------------------------------------------
# LEVEL 33 — SEMANTIC COMPOSITION GRAPH
# --------------------------------------------------
...
# --------------------------------------------------
# COMPONENT STRUCTURE (SYNTAX CLUSTERS)
# --------------------------------------------------
...
# COMPOSITION GRAPH (KEY VISUAL)
axs[1, 1].imshow(symbol_mask, cmap='gray')
axs[1, 1].set_title("Semantic Composition Graph")
...
# --------------------------------------------------
# CLUSTER COMPOSITION (CONNECTED COMPONENTS)
# --------------------------------------------------
