# ENGINE/analysis/navigation_level49b_persistent_torus_tuned.py
...
def build_field():
    ...
    if HAS_EXTERNAL_FIELD:
        ...
        # fallback: synthetic gaussian field (stabil!)
        x = np.linspace(0, SIZE-1, SIZE)
        y = np.linspace(0, SIZE-1, SIZE)
        X, Y = np.meshgrid(x, y)

        cx1, cy1 = SIZE*0.4, SIZE*0.4
        cx2, cy2 = SIZE*0.7, SIZE*0.6

        field = (
            np.exp(-((X-cx1)**2 + (Y-cy1)**2) / 200)
            + 0.7 * np.exp(-((X-cx2)**2 + (Y-cy2)**2) / 180)
        )
...
def run_particles(field):
    ...
    for p in particles:
        ...
        for t in range(STEPS):
            ...
            # Drift (entscheidend für "nicht wiederholen")
...
def run():
    ...
    out_dir = "ENGINE/analysis/output_level49b"
    ...
    axs[1].set_title("Persistent Torus (Level 49b Tuned)")
