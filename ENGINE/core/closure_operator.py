...
from ENGINE.core.fixpoint_lattice import FixpointStructure, build_fixpoint_structure
...
class ClosureOperator:
    ...
    def __post_init__(self) -> None:
        # Validate that gamma maps into the carrier set
        for x in self.poset.elements:
            gx = self.gamma(x)
            if gx not in self.poset.elements:
                raise ValueError(f"gamma(x) must be in poset.elements. Got gamma({x})={gx}")

        # Validate closure axioms on the finite carrier
    ...
    def _validate_extensive(self) -> None:
        for x in self.poset.elements:
            ...
            if not self.poset.is_leq(x, gx):
                raise ValueError(f"Not extensive: {x} ≤ gamma({x}) fails (gamma({x})={gx}).")
            ...
            for y in elems:
                if self.poset.is_leq(x, y):
                    ...
                    if not self.poset.is_leq(gx, gy):
                        raise ValueError(
                            f"Not monotone: {x} ≤ {y} but gamma({x}) ≤ gamma({y}) fails "
