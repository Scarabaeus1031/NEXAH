...
# CONTROL SIGNAL
# -----------------------------
def compute_control(v, t, k1=2.0, k2=1.0):
    dv = np.gradient(v, t)
    d2v = np.gradient(dv, t)

    control = -k1 * dv - k2 * d2v
...
# APPLY CONTROL
# -----------------------------
def apply_control(v, control, alpha=0.1):
    return v + alpha * control
...
def simulate_with_control(case):
    ...
    if split_time is not None:
        ...
        for i in range(idx, len(v)):
            control = compute_control(v_controlled, t)
            v_controlled[i] += 0.1 * control[i]
