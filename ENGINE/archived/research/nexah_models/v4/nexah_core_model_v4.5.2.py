...
def is_irreversible(t, v, split_time,
    ...
    # 2. deviation strength
    z = compute_deviation(v)
    local_z = z[idx:idx+window]

    # 3. persistence
    persistence = np.mean(local_z > deviation_threshold)

    # 🔥 FINAL DECISION
    if slope < -slope_tol and persistence > persistence_ratio:
        return True
