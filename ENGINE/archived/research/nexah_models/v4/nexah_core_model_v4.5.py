# nexah_core_model_v4.5.py

import numpy as np
import matplotlib.pyplot as plt

# -----------------------------
# Synthetic scenarios (wie v4.4)
# -----------------------------
...
# -----------------------------
# Core Detection (v4.4 style)
# -----------------------------
def detect_split(t, v, threshold=3.0):
    dv = np.gradient(v)
    dv2 = np.gradient(dv)
...
# -----------------------------
# NEW: Irreversibility Check
# -----------------------------
def is_irreversible(t, v, split_time, window=20, recovery_tol=0.05):
    ...
    v_before = np.mean(v[max(0, idx-10):idx])
    v_after  = np.mean(v[idx:idx+window])

    # if system "recovers" → fake
    if abs(v_after - v_before) < recovery_tol:
        return False

    # if still drifting away → real
    return True
...
def run_case(case, runs=20):
    ...
    for _ in range(runs):
        ...
        v += np.random.normal(0, 0.002, size=len(v))  # noise
