...
# ============================================================
# 8. SIMULATION USING POLICY
# ============================================================

def simulate_policy(policy, start=0, steps=80):
    path = [start]
    current = start

    for _ in range(steps):
        nxt = policy.get(current, None)
        if nxt is None:
            break
        path.append(nxt)
        current = nxt
        if current == policy[current] == current:
            # remain at target once reached
            continue

    return path


def simulate_natural(prob_adj, start=0, steps=80):
    path = [start]
    current = start
...
def plot_policy_graph(cost_to_target, policy, energy_table, target):
    clusters = sorted(cost_to_target.keys())
    n = max(clusters) + 1

    theta = np.linspace(0, 2*np.pi, n, endpoint=False)
    pos = {c: np.array([np.cos(t), np.sin(t)]) for c, t in zip(clusters, theta)}

    fig, ax = plt.subplots(figsize=(7, 7))

    # draw all available edges faintly
    for (src, dst), info in energy_table.items():
        p1 = pos[src]
        p2 = pos[dst]
        ax.plot([p1[0], p2[0]], [p1[1], p2[1]],
                color="lightgray", linewidth=1.2, alpha=0.5, zorder=1)

    # draw optimal policy arrows strongly
    for src, dst in policy.items():
        p1 = pos[src]
        p2 = pos[dst]
        ax.annotate(
            "",
    ...
    for c in clusters:
        x, y = pos[c]
        face = "yellow" if c != target else "magenta"
        ax.scatter(x, y, s=1400, c=face, edgecolor="black", zorder=4)
        ax.text(x, y + 0.08, f"C{c}", ha="center", va="center", fontsize=16, zorder=5)
        ax.text(x, y - 0.14, f"{cost_to_target[c]:.2f}", ha="center", va="center", fontsize=11, zorder=5)

    ax.set_title(f"V15 Optimal Policy Graph (target = C{target})")
    ax.axis("off")

    out = os.path.join(OUTPUT_DIR, "v15_optimal_policy_graph.png")
    plt.savefig(out, dpi=200, bbox_inches="tight")
    plt.close()
    print(f"Saved: {out}")


def plot_policy_backprojection(H, xedges, yedges, nodes, groups, centroids,
                               natural_path, natural_pts,
                               policy_path, policy_pts,
                               target, cost_to_target):
    cluster_colors = {
        0: "#1f77b4",  # blue
        1: "#d62728",  # red
        2: "#e377c2",  # magenta
    ...
    panels = [
        ("Natural Cluster Trajectory", natural_path, natural_pts),
        (f"Optimal Policy Trajectory (target = C{target})", policy_path, policy_pts),
    ]

    for ax, (title, path, pts) in zip(axes, panels):
        ax.imshow(
            H.T,
            origin="lower",
        ...
        ax.plot(pts[:, 0], pts[:, 1], color="white", linewidth=1.4, alpha=0.65, zorder=4)
        ax.scatter(pts[:, 0], pts[:, 1],
                   c=[cluster_colors[c] for c in path], s=18, alpha=0.95, zorder=6)
    ...
    out = os.path.join(OUTPUT_DIR, "v15_optimal_policy_backprojection.png")
    plt.savefig(out, dpi=200, bbox_inches="tight")
    plt.close()
    print(f"Saved: {out}")
...
def main():
    print("Running V15 Optimal Policy...")
    ...
    cost_to_target, parent = shortest_costs_to_target(energy_table, target_cluster, clusters)
    policy = derive_policy(parent, target_cluster, clusters)

    print("\nOptimal cost-to-target:")
    for c in clusters:
        print(f"  C{c} -> C{target_cluster}: {cost_to_target[c]:.3f}")
    ...
    # simulate
    natural_path = simulate_natural(prob_adj, start=start_cluster, steps=90)
    policy_path = simulate_policy(policy, start=start_cluster, steps=40)

    natural_pts = make_spatial_path(natural_path, centroids, jitter=0.14)
    policy_pts = make_spatial_path(policy_path, centroids, jitter=0.14)

    print("\nNatural visit counts:")
    nc = Counter(natural_path)
    for c in clusters:
        print(f"  C{c}: {nc[c]}")

    print("\nPolicy visit counts:")
    pc = Counter(policy_path)
    for c in clusters:
        print(f"  C{c}: {pc[c]}")

    plot_policy_graph(cost_to_target, policy, energy_table, target_cluster)
    plot_policy_backprojection(
        H, xedges, yedges,
        nodes, groups, centroids,
        natural_path, natural_pts,
        policy_path, policy_pts,
        target_cluster, cost_to_target
    )
