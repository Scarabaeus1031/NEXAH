...
SAVE_DIR = "ENGINE/visuals/navigation_level12_2"
LOG_DIR = "ENGINE/logs"
...
def compute_path_length(paths):
    lengths = []
    for path in paths:
        d = np.sum(np.linalg.norm(np.diff(path, axis=0), axis=1))
        lengths.append(d)
...
def compute_flow_field(field, memory, resolution=20):
    SIZE = field.shape[0]

    xs = np.linspace(0, SIZE-1, resolution)
    ys = np.linspace(0, SIZE-1, resolution)
    ...
    for i, x in enumerate(xs):
        for j, y in enumerate(ys):
            ...
            grad_f = compute_gradient(field, x, y)
            grad_m = compute_gradient(memory, x, y)

            vec = ALPHA_FIELD * grad_f + GAMMA_MEMORY * grad_m
...
def plot_results(base, history, paths_all):
    ...
    xs, ys, U, V = compute_flow_field(field, memory)
    ...
    plt.quiver(xs, ys, U, V, color='white', scale=30)

    plt.axis("off")

    path = os.path.join(SAVE_DIR, "level12_2_flow.png")
...
if __name__ == "__main__":
    ...
    metrics = {
        ...
        "peak": compute_peak(field),
        "spread": compute_spread(field),
        "center_of_mass": compute_center_of_mass(field),
        "path_length": compute_path_length(final_paths)
    }
