...
# -----------------------------
# Simulation with Adaptive Control (v5.1)
# -----------------------------
def simulate_with_control(case):
    ...
    for i in range(idx, len(t) - 1):
        ...
        # -----------------------------
        # Adaptive Control (KEY PART)
        # -----------------------------

        # distance to collapse threshold
        distance = v_controlled[i] - 0.7

        # adaptive strength (0 → 1)
        strength = np.clip((0.2 - distance) / 0.2, 0, 1)

        # soft control (less aggressive)
        control = strength * (-1.2 * dv[i] - 0.3 * d2v)
