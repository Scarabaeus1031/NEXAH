...
def compute_shell_ids(
    ...
    edges = np.quantile(
        janus,
        np.linspace(0.0, 1.0, n_shells + 1),
    )

    shell_ids = np.digitize(
        janus,
        edges[1:-1],
    )
...
def compute_shell_crossings(
    ...
    crossings = np.zeros_like(
        shell_ids,
        dtype=float,
    )

    crossings[1:] = (
        shell_ids[1:] != shell_ids[:-1]
    ).astype(float)
...
def classify_phase_quadrants(
    ...
    low_janus = (
        jn < np.quantile(jn, 0.30)
    )

    high_janus = (
        jn > np.quantile(jn, 0.70)
    )

    high_breathing = (
        vn > np.quantile(vn, 0.70)
    )

    low_axis = (
        an < np.quantile(an, 0.35)
    )

    high_memory = (
        mn > np.quantile(mn, 0.70)
    )
...
def plot_phase_quadrant_timeseries(
    ...
    axes[0].set_title(
        "JANUS Recursive Phase Quadrant Timeseries"
    )
...
def plot_phase_quadrant_density(
    ...
    ax.set_title(
        "JANUS Phase Quadrant / Shell Occupation Density"
    )
...
def write_summary(
    ...
    lines.append(
        "Decompose JANUS transport into four recursive phase regimes."
    )
    ...
    lines.append(
        f"shell crossings: {int(np.sum(shell_crossings))}"
    )

    lines.append(
        f"mean memory signal: {float(np.mean(memory)):.6f}"
    )

    lines.append(
        "mean abs breathing velocity: "
        f"{float(np.mean(np.abs(breathing_velocity))):.6f}"
    )
    ...
    lines.append(
        "- expansion and compression form breathing transport phases"
    )

    lines.append(
        "- memory / echo captures delayed recurrence structure"
    )

    lines.append(
        "- transition / gate identifies low-coherence axis-linked crossings"
    )

    lines.append(
        "- the four regimes form a recursive phase decomposition of JANUS flow"
    )
...
def run_experiment() -> None:
    ...
    print(
        "Running JANUS recursive phase quadrants..."
    )

    t, states = simulate_lorenz(cfg)

    core_states, janus = compute_janus(
        t,
        states,
    )

    radius, breathing_velocity = compute_breathing(
        core_states
    )
    ...
    memory = compute_memory_signal(
        janus,
        cfg.memory_lag,
    )

    shell_ids = compute_shell_ids(
        janus,
        cfg.n_shells,
    )

    shell_crossings = compute_shell_crossings(
        shell_ids
    )
    ...
    plot_recursive_phase_quadrants(
        ...
        / "janus_recursive_phase_quadrants.png",
        output_cfg.dpi,
    )
    ...
    plot_phase_quadrant_timeseries(
        ...
        quadrants,
        output_cfg.output_dir
        / "janus_phase_quadrant_timeseries.png",
        output_cfg.dpi,
    )
    ...
    plot_phase_quadrant_density(
        ...
        shell_ids,
        output_cfg.output_dir
        / "janus_phase_quadrant_density.png",
        output_cfg.dpi,
    )
    ...
    write_summary(
        ...
        breathing_velocity,
        output_cfg.output_dir
        / "janus_phase_quadrant_summary.txt",
    )
