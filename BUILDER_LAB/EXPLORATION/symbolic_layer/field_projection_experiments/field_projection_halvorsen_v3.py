...
Halvorsen system field projection experiment.

Same pipeline as Rössler V3:
1. Simulate Halvorsen trajectory
2. PCA projection: alpha, beta, gamma
3. Phase theta = arctan2(gamma, beta)
4. Phase drift delta_theta
5. Regime classification: Theta / Tao / Dao / Iota
6. Iota peak detection on |delta_theta|
7. Iota event window extraction
8. Export CSV, plots, and JSON summary
...
class HalvorsenConfig:
    ...
    iota_window: int = 100
    output_dir: str = "outputs/halvorsen_v3"
    random_seed: int = 42


def halvorsen_rhs(_t: float, state: np.ndarray, a: float) -> np.ndarray:
    """Halvorsen system right-hand side."""
    x, y, z = state
...
def simulate_halvorsen(config: HalvorsenConfig) -> Tuple[np.ndarray, np.ndarray]:
    """Simulate the Halvorsen system and return time array and state matrix."""
    ...
    if not sol.success:
        raise RuntimeError(f"Halvorsen integration failed: {sol.message}")
...
def save_plots(df: pd.DataFrame, output_dir: Path) -> None:
    ...
    ax.set_title("Halvorsen V3 — PCA Field Projection")
    ax.set_xlabel("alpha / PC1")
    ax.set_ylabel("beta / PC2")
    ax.set_zlabel("gamma / PC3")
    fig.tight_layout()
    fig.savefig(output_dir / "halvorsen_v3_pca_projection.png", dpi=180)
    plt.close(fig)
    ...
    ax.set_title("Halvorsen V3 — Phase Drift and Iota Events")
    ax.set_xlabel("t")
    ax.set_ylabel("delta_theta")
    ax.legend(loc="best")
    fig.tight_layout()
    fig.savefig(output_dir / "halvorsen_v3_phase_drift_iota.png", dpi=180)
    plt.close(fig)
    ...
    ax.set_title("Halvorsen V3 — Regime Distribution")
    ax.set_xlabel("Regime")
    ax.set_ylabel("Samples")
    fig.tight_layout()
    fig.savefig(output_dir / "halvorsen_v3_regime_distribution.png", dpi=180)
    plt.close(fig)

    fig, ax = plt.subplots(figsize=(8, 5))
    ax.hist(df["abs_delta_theta"], bins=120)
    ax.set_title("Halvorsen V3 — Absolute Phase Drift Distribution")
    ax.set_xlabel("abs(delta_theta)")
    ax.set_ylabel("Count")
    fig.tight_layout()
    fig.savefig(output_dir / "halvorsen_v3_abs_phase_drift_histogram.png", dpi=180)
    plt.close(fig)


def run_experiment(config: HalvorsenConfig) -> Dict[str, object]:
    """Run full Halvorsen FIELD_LAYER V3 experiment."""
    np.random.seed(config.random_seed)
    output_dir = Path(config.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    t_raw, states_raw = simulate_halvorsen(config)
    ...
    summary = summarize(df, iota_windows_df, pca, thresholds, config)

    df.to_csv(output_dir / "halvorsen_v3_field_projection.csv", index=False)
    iota_windows_df.to_csv(output_dir / "halvorsen_v3_iota_event_windows.csv", index=False)
    with open(output_dir / "halvorsen_v3_summary.json", "w", encoding="utf-8") as f:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Halvorsen FIELD_LAYER V3 experiment")
    parser.add_argument("--a", type=float, default=1.4)
    ...
    parser.add_argument("--output-dir", type=str, default="outputs/halvorsen_v3")
