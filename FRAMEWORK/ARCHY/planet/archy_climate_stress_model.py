...
def compute_climate_stress(lat, year):

    warming = (year - 2025) * 0.002

    heat_equator = max(0, 1 - abs(lat) / 60)

    drought_noise = np.random.normal(0, 0.1)

    stress = heat_equator * warming + drought_noise * 0.02
...
def run():
    ...
    for step in range(30):
        ...
        for c in cities:
            ...
            s = compute_climate_stress(c["lat"],year)
