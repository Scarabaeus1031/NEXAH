...
def simulate_city_stability():
    ...
    for name, lat, lon in CITIES:
        ...
        city, field, modified = simulate_city("coastal", 20, 10)

        stability = modified.mean()

        results.append((name, lat, lon, stability))
...
def plot_world(cities):
    ...
    for name, lat, lon, _ in cities:
        ax.text(lon + 1, lat + 1, name, fontsize=8)

    plt.colorbar(sc, label="City Stability")

    plt.title("ARCHY Global Stability Map — Real Cities")
