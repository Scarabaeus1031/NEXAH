...
|---|---|---|---|---|
| Orientation | Evidence-aware process for locating and relating possible action | Typed state, temporal and graph backends, report generators, memory, and domain validation | Verified bounded vertical paths | Add observed non-power graph evidence |
...
| Orientation Report | Evidence-linked orientation output | Contract, temporal and graph-native generators, `nexah orient`, and `nexah orient-network` | Verified for v0.7, scoped IEEE, and illustrative network paths | Add outcome-linked graph episodes |
