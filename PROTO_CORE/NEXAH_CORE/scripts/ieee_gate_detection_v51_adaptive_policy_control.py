# ============================================================
# NEXAH — IEEE GATE DETECTION v51
# Adaptive Policy Control / Edge Selection
# ============================================================
#
# PURPOSE:
# --------
# Fix the v50 problem:
#
#   naive multi-edge policy control can damage the basin network.
#
# v51 tests each policy edge individually, measures its network effect,
# then keeps only edges that improve the requested policy score.
#
# BUILDS ON:
# ----------
# - v49: single-transition control
# - v50: multi-transition policy control
#
# CORE IDEA:
# ----------
# Policy control is not additive.
#
# Therefore:
#
#   1. Test each edge alone.
#   2. Measure its effect.
#   3. Keep only beneficial edges.
#   4. Apply selected edges as adaptive policy.
#
# DEFAULT POLICY:
# ---------------
# Reinforce dominant loop:
#
#   2 → 0
#   0 → 1
#   1 → 2
...
# OUTPUTS:
# --------
# v51_adaptive_policy_control.png
# v51_adaptive_policy_summary.txt
# v51_adaptive_policy_states.npy
# v51_adaptive_policy_transition_probs.npy
#
# ============================================================
...
def parse_policy(policy_string):
    ...
    for part in policy_string.split(","):
        ...
        if ":" not in part:
            raise ValueError(
                f"Invalid policy rule '{part}'. Use source:target."
            )
...
def apply_policy_control(
    ...
    """
    Apply policy control for selected edges.
    """
...
# ------------------------------------------------------------
# Basin assignment and probability tools
# ------------------------------------------------------------
...
def policy_score(probs, basin_list, policy_rules):
    """
    Mean probability of requested policy edges.
    """
...
def evaluate_controlled_policy(data, policy_rules, policy_gain, base_gain):
    """
    Apply a policy rule set and compute controlled transition probabilities.
    """

    controlled, active_mask, target_by_state = apply_policy_control(
    ...
    return {
        ...
        "basin_ids": controlled_basin_ids,
        "transition_counts": counts,
        "transition_probs": probs,
        "basin_list": basin_list,
    }
...
def select_positive_edges(
    ...
    """
    Test each policy edge alone.
    Keep only edges that improve their own transition probability.
    """

    selected = []
    diagnostics = []

    for edge in requested_policy:

        source, target = edge

        before = transition_probability(
        ...
        test = evaluate_controlled_policy(
            data,
            [edge],
            policy_gain,
            base_gain
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="NEXAH v51 Adaptive Policy Control"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Minimum edge delta required to keep an edge. Default: 0.0"
    )
    ...
    requested_policy = parse_policy(args.policy)

    data = build_pipeline()

    selected_policy, diagnostics = select_positive_edges(
        data=data,
    ...
    adaptive_result = evaluate_controlled_policy(
        data=data,
        policy_rules=selected_policy,
        policy_gain=args.policy_gain,
        base_gain=args.base_gain
    )
    ...
    score_after = policy_score(
        adaptive_result["transition_probs"],
        adaptive_result["basin_list"],
        requested_policy
    )
    ...
    plt.scatter(
        adaptive_result["controlled"][:, 1],
        adaptive_result["controlled"][:, 0],
        s=3,
        alpha=0.55,
        label="v51 adaptive policy"
    )

    active_mask = adaptive_result["active_mask"]

    plt.scatter(
        adaptive_result["controlled"][active_mask, 1],
        adaptive_result["controlled"][active_mask, 0],
        s=8,
        alpha=0.85,
        label="active selected edges"
    )
    ...
    requested_label = ", ".join([f"{s}->{t}" for s, t in requested_policy])
    selected_label = ", ".join([f"{s}->{t}" for s, t in selected_policy])
    if selected_label == "":
        selected_label = "none"
    ...
    plt.ylabel("r")
    plt.title(
        "NEXAH v51 — Adaptive Policy Control\n"
        f"requested: {requested_label} | selected: {selected_label}\n"
        f"score before={score_before:.3f}, after={score_after:.3f}, "
        f"Δ={delta_score:.3f}"
    )
    ...
    out_path = os.path.join(
        OUT_DIR,
        f"v51_adaptive_policy_control_{safe_policy_name}.png"
    )
    ...
    np.save(
        os.path.join(
            OUT_DIR,
            f"v51_adaptive_policy_states_{safe_policy_name}.npy"
        ),
        adaptive_result["controlled"]
    )

    np.save(
        os.path.join(
            OUT_DIR,
            f"v51_adaptive_policy_transition_probs_{safe_policy_name}.npy"
        ),
        adaptive_result["transition_probs"]
    )
    ...
    summary_path = os.path.join(
        OUT_DIR,
        f"v51_adaptive_policy_summary_{safe_policy_name}.txt"
    )

    with open(summary_path, "w", encoding="utf-8") as f:
        f.write("NEXAH v51 — Adaptive Policy Control Summary\n")
        f.write("==========================================\n\n")

        f.write(f"Requested policy: {requested_policy}\n")
        f.write(f"Selected policy:  {selected_policy}\n")
        f.write(f"Policy gain: {args.policy_gain}\n")
        f.write(f"Base gain: {args.base_gain}\n")
        f.write(f"Min delta: {args.min_delta}\n\n")

        f.write("Edge diagnostics:\n")
        for d in diagnostics:
            f.write(
                f"  {d['edge'][0]}->{d['edge'][1]}: "
                f"before={d['before']:.4f}, "
                f"after={d['after']:.4f}, "
                f"delta={d['delta']:.4f}, "
                f"keep={d['keep']}\n"
            )

        f.write("\nPolicy score over requested edges:\n")
        f.write(f"  before={score_before:.4f}\n")
        f.write(f"  after={score_after:.4f}\n")
        f.write(f"  delta={delta_score:.4f}\n\n")

        f.write("Baseline transition probs:\n")
        f.write(str(data["transition_probs"]))
        f.write("\n\nAdaptive controlled transition probs:\n")
        f.write(str(adaptive_result["transition_probs"]))
        f.write("\n")

    print("NEXAH v51 complete")
    print(f"Requested policy: {requested_policy}")
