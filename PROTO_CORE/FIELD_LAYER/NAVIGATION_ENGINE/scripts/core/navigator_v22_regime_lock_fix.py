...
def instability_score(point, current_cluster, centroids, regime_center_cluster=2):
...
def choose_intended_transition(current, score, br,
                               lock_threshold=0.68,
                               risk_threshold=0.19):
    """
    Hard-coded regime holding logic.
    ...
    C3 = overshoot / bad
    """
    # strong risk handling first
    if current == 3:
        return 0, "recover"

    if current == 2:
        # NEW: hold target unless risk is truly high
        if (score > 0.82) or (br > 0.24):
            return 1, "stabilize"
        return 2, "lock"

    if current == 1:
        if (score > 0.80) or (br > 0.23):
            return 0, "stabilize"
        return 2, "lock"
...
def run_regime_lock_fix(prob_adj, centroids, start=0, steps=260):
    cluster_path = [start]
    ...
    for _ in range(steps):
        p = points[-1]
        score, br, od, rd = instability_score(p, current, centroids, regime_center_cluster=2)
        ...
        intended, mode = choose_intended_transition(current, score, br)
...
def plot_v22(H, xedges, yedges, nodes, groups, centroids, result):
    cluster_colors = {
        0: "#1f77b4",
        1: "#d62728",
        2: "#e377c2",
    ...
    ax1.scatter(*centroids[2], s=760, facecolors="none",
                edgecolors="yellow", linewidths=2.5, zorder=9, label="lock target C2")
    ...
    ax1.set_title("Q1 — Regime Lock Fix")
    ax1.set_xlabel("α")
    ax1.set_ylabel("β")
    ax1.set_xlim(*xlim)
    ax1.set_ylim(*ylim)
    ...
    ax2.set_title("Q2 — Fixed Regime Lock Signals")
    ax2.set_xlabel("step")
    ...
    ax3.set_title("Q3 — State Trace with Lock Fix")
    ax3.set_xlabel("step")
    ...
    ax4.set_title("Q4 — Transition Usage with Lock Fix")
    ...
    out = os.path.join(OUTPUT_DIR, "v22_regime_lock_fix.png")
    plt.tight_layout()
...
def main():
    print("Running V22 Regime Lock Fix...")
    ...
    result = run_regime_lock_fix(prob_adj, centroids, start=0, steps=260)
    ...
    plot_v22(H, xedges, yedges, nodes, groups, centroids, result)
