import os
import numpy as np
import matplotlib.pyplot as plt
from sklearn.decomposition import PCA
from scipy.ndimage import gaussian_filter
import random
from collections import defaultdict, Counter

OUTPUT_DIR = "FIELD_LAYER/outputs/plots"
os.makedirs(OUTPUT_DIR, exist_ok=True)

# ============================================================
# 1. LORENZ
# ============================================================

def generate_lorenz(n_steps=5000, dt=0.01):
    sigma = 10.0
    rho = 28.0
    beta = 8.0 / 3.0

    X = np.zeros((n_steps, 3), dtype=float)
    x, y, z = 1.0, 1.0, 1.0

    for i in range(n_steps):
        dx = sigma * (y - x)
        dy = x * (rho - z) - y
        dz = x * y - beta * z

        x += dx * dt
        y += dy * dt
        z += dz * dt

        X[i] = [x, y, z]

    return X


# ============================================================
# 2. PCA PROJECTION
# ============================================================

def compute_basis(X):
    pca = PCA(n_components=3)
    pca.fit(X)
    return pca.components_


def project(X, e1, e2):
    alpha = X @ e1
    beta = X @ e2
    return alpha, beta


# ============================================================
# 3. DENSITY FIELD
# ============================================================

def density_field(alpha, beta, bins=180, sigma=2.0):
    H, xedges, yedges = np.histogram2d(alpha, beta, bins=bins)
    H = gaussian_filter(H, sigma=sigma)
    return H, xedges, yedges


# ============================================================
# 4. NODE + CLUSTER DEFINITIONS (consistent with V12.6 / V12.7)
# ============================================================

def get_nodes():
    return np.array([
        [11.0, 26.0],  # N0
        [10.5, 25.5],  # N1
        [12.0, 24.8],  # N2
        [13.0, 26.0],  # N3
        [12.0, 23.5],  # N4
        [11.5, 24.0],  # N5
        [10.8, 27.8],  # N6
        [9.8, 25.2],   # N7
        [13.5, 25.5],  # N8
        [11.5, 28.5],  # N9
        [10.2, 24.2],  # N10
    ], dtype=float)


node_to_cluster = {
    0: 0, 1: 0, 7: 0, 10: 0,   # Cluster 0
    2: 1, 4: 1, 5: 1,          # Cluster 1
    3: 2, 8: 2,                # Cluster 2
    6: 3, 9: 3                 # Cluster 3
}


def build_cluster_centroids(nodes, node_to_cluster):
    clusters = defaultdict(list)
    for n_idx, c_idx in node_to_cluster.items():
        clusters[c_idx].append(nodes[n_idx])

    centroids = {}
    for c_idx, pts in clusters.items():
        centroids[c_idx] = np.mean(np.array(pts), axis=0)

    return centroids, clusters


# ============================================================
# 5. CLUSTER TRANSITION GRAPH (from V12.7)
# ============================================================

cluster_edges = {
    (2, 1): 23,
    (0, 2): 16,
    (1, 0): 15,
    (3, 0): 15,
    (0, 1): 14,
    (1, 2): 12,
    (0, 0): 12,
    (1, 3): 9,
    (3, 3): 7
}


def build_probabilities(cluster_edges):
    adj = defaultdict(list)
    for (src, dst), w in cluster_edges.items():
        adj[src].append((dst, w))

    prob_adj = {}
    for src, edges in adj.items():
        total = sum(w for _, w in edges)
        prob_adj[src] = [(dst, w / total) for dst, w in edges]

    return prob_adj


# ============================================================
# 6. SIMULATION
# ============================================================

def simulate_cluster_path(prob_adj, start=0, steps=120):
    path = [start]
    current = start

    for _ in range(steps):
        if current not in prob_adj:
            break

        choices = prob_adj[current]
        dsts = [d for d, _ in choices]
        probs = [p for _, p in choices]
        current = random.choices(dsts, weights=probs)[0]
        path.append(current)

    return path


# ============================================================
# 7. BACKPROJECTION TO FIELD
# ============================================================

def make_spatial_path(cluster_path, centroids, jitter=0.18):
    """
    Convert cluster states into points in alpha-beta space.
    Small jitter avoids exact overlap and makes the path visible.
    """
    pts = []
    for c in cluster_path:
        center = centroids[c].copy()
        noise = np.random.normal(scale=jitter, size=2)
        pts.append(center + noise)
    return np.array(pts)


# ============================================================
# 8. PLOT
# ============================================================

def main():
    print("Running V12.9 Backprojection of Cluster Dynamics...")

    # Background field
    X = generate_lorenz()
    e1, e2, _ = compute_basis(X)
    alpha, beta = project(X, e1, e2)
    H, xedges, yedges = density_field(alpha, beta, bins=180, sigma=2.0)

    # Nodes + clusters
    nodes = get_nodes()
    centroids, clusters = build_cluster_centroids(nodes, node_to_cluster)

    # Probabilities + simulation
    prob_adj = build_probabilities(cluster_edges)
    cluster_path = simulate_cluster_path(prob_adj, start=0, steps=140)
    spatial_path = make_spatial_path(cluster_path, centroids, jitter=0.16)

    # Count visits
    counts = Counter(cluster_path)
    print("\nCluster Visit Counts:")
    for k in sorted(counts):
        print(f"  C{k}: {counts[k]}")

    # Plot
    fig, ax = plt.subplots(figsize=(11, 9))

    ax.imshow(
        H.T,
        origin="lower",
        extent=[xedges[0], xedges[-1], yedges[0], yedges[-1]],
        aspect="auto",
        cmap="viridis"
    )

    # plot nodes
    cluster_colors = {
        0: "#1f77b4",  # blue
        1: "#d62728",  # red
        2: "#e377c2",  # magenta
        3: "#17becf",  # cyan
    }

    for c_idx, node_list in clusters.items():
        pts = np.array(node_list)
        ax.scatter(
            pts[:, 0], pts[:, 1],
            s=260,
            c=cluster_colors[c_idx],
            edgecolor="black",
            alpha=0.95,
            zorder=5,
            label=f"C{c_idx} nodes"
        )

    # node labels
    for i, (x, y) in enumerate(nodes):
        ax.text(x, y, f"N{i}", fontsize=9, ha="center", va="center", zorder=6)

    # centroids
    for c_idx, center in centroids.items():
        ax.scatter(
            center[0], center[1],
            s=420,
            c=cluster_colors[c_idx],
            marker="X",
            edgecolor="white",
            linewidth=1.8,
            zorder=7
        )
        ax.text(center[0], center[1] + 0.45, f"C{c_idx}", color="white",
                fontsize=11, ha="center", va="bottom", zorder=8)

    # simulated path
    ax.plot(
        spatial_path[:, 0], spatial_path[:, 1],
        color="white",
        linewidth=1.4,
        alpha=0.65,
        zorder=4
    )

    ax.scatter(
        spatial_path[:, 0], spatial_path[:, 1],
        c=[cluster_colors[c] for c in cluster_path],
        s=20,
        alpha=0.95,
        zorder=6
    )

    # start / end
    ax.scatter(spatial_path[0, 0], spatial_path[0, 1], s=220, c="lime",
               edgecolor="black", zorder=9, label="start")
    ax.scatter(spatial_path[-1, 0], spatial_path[-1, 1], s=220, c="yellow",
               edgecolor="black", zorder=9, label="end")

    # cycle text
    visit_txt = "\n".join([f"C{k}: {counts[k]}" for k in sorted(counts)])
    ax.text(
        0.02, 0.98, visit_txt,
        transform=ax.transAxes,
        ha="left", va="top",
        fontsize=10,
        color="white",
        bbox=dict(facecolor="black", alpha=0.45, edgecolor="white")
    )

    ax.set_title("V12.9 Backprojected Cluster Dynamics")
    ax.set_xlabel("α")
    ax.set_ylabel("β")
    ax.set_xlim(6.0, 16.5)
    ax.set_ylim(21.0, 30.8)
    ax.legend(loc="lower left", fontsize=9)

    out = os.path.join(OUTPUT_DIR, "v12_9_backprojected_cluster_dynamics.png")
    plt.savefig(out, dpi=200, bbox_inches="tight")
    plt.close()

    print(f"\nSaved: {out}")


if __name__ == "__main__":
    main()
