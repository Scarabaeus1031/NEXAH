...
class Config:
    ...
    # output
    out_dir = "APPLICATIONS/power_systems/nexah_ieee9/results/controller_v7_1"
...
def compute_barrier(lmbd: float) -> float:
    """
    Local barrier near the critical transition zone.
    This prevents trivial freezing in a single minimum and creates
    richer field structure.
    """
...
def compute_drift(risk: float) -> float:
    """
    Persistent forward pressure, weakened near high risk.
    Prevents freezing exactly at local minima.
    """
...
def run(steps: int = 120):
    ...
    for step in range(steps):
        ...
        # update
        lmbd = max(0.0, lmbd + ctrl + steer + drift)
...
def save_csv(data):
    path = os.path.join(cfg.out_dir, "output_v7_1_data.csv")
...
def plot_timeseries(data):
    ...
    plt.xlabel("Step")
    plt.ylabel("Value")
    plt.title("NEXAH Closed Loop v7.1 (Field + Drift + Barrier)")
    plt.legend()
    plt.tight_layout()

    path = os.path.join(cfg.out_dir, "output_v7_1_plot.png")
...
def plot_phase(data):
    ...
    plt.xlabel("Risk")
    plt.ylabel("Distance")
    plt.title("NEXAH v7.1 Phase Plot (Risk vs Distance)")
    plt.legend()
    plt.colorbar(sc, label="Step")
    plt.tight_layout()

    path = os.path.join(cfg.out_dir, "output_v7_1_phase.png")
    plt.savefig(path, dpi=200)
    plt.close()
    return path
