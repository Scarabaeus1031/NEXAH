...
root_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../..'))  # Wurzelverzeichnis hinzufügen
...
# --------------------------------------------------
# 6. Intervention (complexer intervention, early warning)
# --------------------------------------------------

def apply_intervention(c, dc, critical_idx):
    dc_new = dc.copy()
    dc_new[critical_idx] = 0.75 * c[critical_idx]  # Stärkere Intervention
    return dc_new
