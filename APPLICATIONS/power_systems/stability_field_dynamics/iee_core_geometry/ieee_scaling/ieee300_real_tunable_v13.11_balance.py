...
JANU_STRENGTH = 1.35
KAPPA_L = 0.95
IOTA_YUGO_STRENGTH = 1.45      # stärkerer Pivot
...
def nexah_lorenz_ode(t, x):
    ...
    d_phi = 0.0
    if t > 24.0 and abs(dc) > 2.2:          # später und weicher
        d_phi = 24.0 + 14.0 * (phi == 2)    # moderater Boost in Forward2
    
    return [dc * contraction, d_dc, d_phi]

print("🚀 Lade IEEE 300-Bus Netz... (v13.11 Balance – Iota-Yugo stärker)")
net = pn.case300()
...
ax1.set_title("IEEE 300-Bus – Voltage Collapse (Iota-Yugo Balance v13.11)")
...
plt.tight_layout()
plt.savefig("ieee300_real_tunable_v13.11_iota_yugo_balance.png", dpi=420, bbox_inches='tight')
print("\n📸 Plot gespeichert als: ieee300_real_tunable_v13.11_iota_yugo_balance.png")
plt.show()
