...
# GhostGrid + 069 Vorwahl + drehende 8
GHOSTGRID_069 = 69
GHOSTGRID_537 = 537
IOTA_7DEG = np.deg2rad(7)
MIDDLE_DRIFT = 0.005

def is_ghostgrid_trigger(t, c, dc):
    angle = np.arctan2(dc, c)
    ghost = (int(t) % 7 == 0) and (int(t) % 11 < 3)
    key_069 = abs(int(t) % 100 - GHOSTGRID_069) < 2
    key_537 = abs(int(t) % 1000 - GHOSTGRID_537) < 3
    return abs(angle) < 0.18 and (ghost or key_069 or key_537)
...
def nexah_lorenz_ode(t, x):
    ...
    if t > 26.0 and is_ghostgrid_trigger(t, c, dc) and abs(zipper) > 0.003 and phi == 2:
        d_phi = 28.0 + 18.0
    
    return [dc * contraction, d_dc, d_phi]

print("🚀 v13.9.11 – GhostGrid 5-Panel (069 Vorwahl + drehende 8 + 7/11 Q-Space)")
...
ax5.set_title("GhostGrid Ring Flow + 069 Vorwahl + Iota-7° Zipper")
ax5.grid(True)

plt.suptitle("NEXAH 5-Panel Serie – GhostGrid + 069 + drehende 8 + 7/11 Q-Space", fontsize=16)
plt.tight_layout()
plt.savefig("ieee9241_v13.9.11_ghostgrid_5panel.png", dpi=300)
