...
def nexah_lorenz_ode(t, x):
    ...
    d_phi = 0.0
    if t > 12.0 and abs(dc) > WINDING_THRESHOLD:
        d_phi = 30.0 + 24.0 * (phi == 2)   # noch stärkerer Boost in Forward2
    
    return [dc * contraction, d_dc, d_phi]

print("🚀 Lade IEEE 300-Bus Netz... (v13.8 Janus + Lyapunov – gezielter Boost)")
net = pn.case300()
...
ax1.set_title("IEEE 300-Bus – Voltage Collapse (Janus + Lyapunov v13.8)")
...
plt.tight_layout()
plt.savefig("ieee300_real_tunable_v13.8_janus_lyapunov.png", dpi=420, bbox_inches='tight')
print("\n📸 Plot gespeichert als: ieee300_real_tunable_v13.8_janus_lyapunov.png")
plt.show()
