...
# === Noch stärkerer + früherer Trigger ===
ALPHA_FLOW = 0.95
BETA_SWIRL = 0.65
GAMMA_MEMORY = 0.40
...
JANU_STRENGTH = 1.45
KAPPA_L = 1.05
WINDING_THRESHOLD = 1.8       # deutlich niedriger
...
def nexah_lorenz_ode(t, x):
    ...
    d_phi = 0.0
    if t > 12.0 and abs(dc) > WINDING_THRESHOLD:      # noch früher
        d_phi = 28.0 + 20.0 * (phi == 2)               # sehr starker Boost in Forward2
    
    return [dc * contraction, d_dc, d_phi]

# ====================== Simulation ======================
print("🚀 Lade IEEE 300-Bus Netz... (v13.6 Janus + Lyapunov – sehr weicher Trigger)")
net = pn.case300()
...
ax1.set_title("IEEE 300-Bus – Voltage Collapse (Janus + Lyapunov v13.6)")
...
plt.tight_layout()
plt.savefig("ieee300_real_tunable_v13.6_janus_lyapunov.png", dpi=420, bbox_inches='tight')
print("\n📸 Plot gespeichert als: ieee300_real_tunable_v13.6_janus_lyapunov.png")
plt.show()
