...
def nexah_lorenz_ode(t, x):
    ...
    # Λ–φ–π Spiral Resonance (verzögert)
...
print("🚀 IEEE 300-Bus – v12.7 mit Λ–φ–π Spiral + Perlenketten-Farbverlauf")
net = pn.case300()
...
# ====================== 5-PANEL mit schöner Perlenkette ======================
...
# === Polar-Ring mit schönem Farbverlauf ===
ax5 = fig.add_subplot(2, 3, (5,6), projection='polar')
theta = np.arctan2(dc, c)
r = np.sqrt(c**2 + dc**2)

# Farbverlauf nach Zeit (Perlenkette fließend)
scatter = ax5.scatter(theta, r, c=t, cmap='plasma', s=28, alpha=0.9, edgecolors='none')
ax5.plot(theta, r, 'b-', lw=1.2, alpha=0.6)

ax5.scatter(0, 0, color='black', s=220, zorder=10)
ax5.set_title("Polar-Ring + Perlenkette (Λ–φ–π Spiral)", pad=20)

# Farbleiste
cbar = plt.colorbar(scatter, ax=ax5, pad=0.1, shrink=0.7)
cbar.set_label('Zeit t [s]')

ax5.grid(True)

plt.suptitle("IEEE 300-Bus – v12.7 mit Λ–φ–π Spiral + Perlenketten-Farbverlauf", fontsize=16)
plt.tight_layout()
plt.savefig("ieee300_v12.7_spiral_perlen_gradient_5panel.png", dpi=300)
print("📸 Volles 5-Panel mit Perlenketten-Farbverlauf gespeichert")
plt.show()
