...
# Tunable Parameters (etwas weicher für ersten Test)
ALPHA_FLOW = 0.95
BETA_SWIRL = 0.65
GAMMA_MEMORY = 0.40
...
JANU_STRENGTH = 0.95
KAPPA_L = 0.55
WINDING_THRESHOLD = 5.5          # etwas niedriger für Test
...
def nexah_lorenz_ode(t, x):
    ...
    d_phi = 0.0
    if t > 28.0 and abs(dc) > WINDING_THRESHOLD:   # früher und weicher
        d_phi = 18.0 + 14.0 * (phi == 2)          # starker Hold in Forward2
    
    return [dc * contraction, d_dc, d_phi]

# ====================== Simulation ======================
print("🚀 Lade IEEE 300-Bus Netz... (v13.1 Janus + Lyapunov)")
net = pn.case300()
...
plt.tight_layout()
plt.savefig("ieee300_real_tunable_v13.1_janus_lyapunov.png", dpi=420, bbox_inches='tight')
print("\n📸 Plot gespeichert als: ieee300_real_tunable_v13.1_janus_lyapunov.png")
plt.show()
