...
# Tunable Parameters (stärkerer Janus + Lyapunov für Test)
ALPHA_FLOW = 0.95
BETA_SWIRL = 0.65
GAMMA_MEMORY = 0.40
DELTA_RESONANCE = 0.25
Q = 1.62
JANU_STRENGTH = 1.25          # stärker
KAPPA_L = 0.85                # stärker
WINDING_THRESHOLD = 3.8       # deutlich weicher
...
def nexah_lorenz_ode(t, x):
    ...
    d_phi = 0.0
    if t > 22.0 and abs(dc) > WINDING_THRESHOLD:
        d_phi = 22.0 + 14.0 * (phi == 2)   # starker Hold in Forward2
    
    return [dc * contraction, d_dc, d_phi]

# ====================== Simulation ======================
print("🚀 Lade IEEE 300-Bus Netz... (v13.3 Janus + Lyapunov – stärkerer Takt)")
net = pn.case300()
...
ax1.set_title("IEEE 300-Bus – Voltage Collapse (Janus + Lyapunov v13.3)")
...
plt.tight_layout()
plt.savefig("ieee300_real_tunable_v13.3_janus_lyapunov.png", dpi=420, bbox_inches='tight')
print("\n📸 Plot gespeichert als: ieee300_real_tunable_v13.3_janus_lyapunov.png")
plt.show()
