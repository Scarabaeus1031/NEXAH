...
print("🚀 IEEE 300-Bus – v12.7 mit ZWEISEITIGER Dual-Dyad Perlenkette")
net = pn.case300()
...
# ====================== 5-PANEL mit ZWEISEITIGER FARBE ======================
fig = plt.figure(figsize=(20, 12))

ax1 = fig.add_subplot(2, 3, 1)
ax1.plot(t, voltage_classic, 'r', lw=3)
if switch_time: ax1.axvline(x=switch_time, color='purple', linestyle='--', lw=3.5, label='Phi-Split')
...
# === Polar-Ring ZWEISEITIG ===
ax5 = fig.add_subplot(2, 3, (5,6), projection='polar')
theta = np.arctan2(dc, c)
r = np.sqrt(c**2 + dc**2)

# Zweiseitige Farbzuordnung: blau/grün = Prime-Seite, rot/gelb/orange = Winkel-Seite
colors = np.zeros_like(phi_idx, dtype=float)
for i, p in enumerate(phi_idx):
    if p <= 2:          # 63:64 Seite → Prime / blau-grün
        colors[i] = 0.4 + p * 0.15
    else:               # 83:84 Seite → Winkel / rot-gelb-orange
        colors[i] = 0.7 + (p-2) * 0.08

scatter = ax5.scatter(theta, r, c=colors, cmap='RdYlBu_r', s=35, alpha=0.95)  # RdYlBu_r = rot-gelb-blau
ax5.plot(theta, r, 'white', lw=1.0, alpha=0.4)

ax5.scatter(0, 0, color='black', s=240, zorder=10)
ax5.set_title("Polar-Ring + Perlenkette (ZWEISEITIG: Prime vs Winkel)", pad=20)

cbar = plt.colorbar(scatter, ax=ax5, pad=0.1, shrink=0.7)
cbar.set_label('Resonanz-Seite (blau/grün = Prime | rot/gelb = Winkel)')

ax5.grid(True)

plt.suptitle("IEEE 300-Bus – v12.7 ZWEISEITIGE Dual-Dyad Perlenkette", fontsize=16)
plt.tight_layout()
plt.savefig("ieee300_v12.7_zweiseitig_perlen.png", dpi=300)
print("📸 ZWEISEITIGE Perlenkette gespeichert")
plt.show()
