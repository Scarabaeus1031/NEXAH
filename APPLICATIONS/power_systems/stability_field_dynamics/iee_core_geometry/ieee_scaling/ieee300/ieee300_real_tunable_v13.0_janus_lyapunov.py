import numpy as np
import matplotlib.pyplot as plt
from scipy.integrate import solve_ivp
import pandapower as pp
import pandapower.networks as pn

# ====================== NEXAH v10.2 (Janus + Lyapunov) ======================
PHI_NAMES = ["Neutral", "Forward1", "Forward2 (P-Regulator)", "Reverse1", "Reverse2"]

# Tunable Parameters
ALPHA_FLOW = 0.95
BETA_SWIRL = 0.65
GAMMA_MEMORY = 0.40
DELTA_RESONANCE = 0.25
Q = 1.62
JANU_STRENGTH = 1.0          # Janus-Reversal-Stärke
KAPPA_L = 0.6                # Lyapunov-Skalierung

SIGMA = 10.0
RHO = 28.0
BETA = 8.0 / 3.0
OMEGA = 2 * np.pi * 0.52

def nexah_lorenz_ode(t, x, vm_history):
    c, dc, phi_idx = x
    phi = np.clip(int(round(phi_idx)), 0, 4)
    
    # 5-Mode P-Drive
    p_drive = [0.0, 0.85, 1.48, -1.0, -1.7][phi]
    
    # Field Force (Lorenz-Core)
    f_field = SIGMA * (dc - c) + RHO * c * (1 - phi)
    
    # Van der Pol
    f_vdp = BETA * dc * (1 - c**2)
    
    # Kuramoto
    f_kuramoto = sum((1 + Q) * np.sin(2 * np.pi * (phi - i) / 5) for i in range(5))
    
    # Compass
    f_compass = 0.25 * np.sin(OMEGA * t + phi * 0.25) * np.cos(OMEGA * t + phi * 0.25 * 1.618)
    
    # Iota-Ring
    f_iota = 0.35 * np.sin(2 * np.pi * (t - 36) / 19)
    
    # Janus-Reversal (J-Flip)
    f_janus = JANU_STRENGTH * (np.cos(OMEGA * t + phi * 0.25) - np.sin(OMEGA * t + phi * 0.25)) * np.sign(dc)
    
    # Lyapunov-Rhythmus (4:3 + 3:2)
    L_t = KAPPA_L * abs(dc)                     # lokale Lyapunov-Approximation
    f_lyapunov = L_t * (np.sin(2 * np.pi * (4/3) * t) + np.sin(2 * np.pi * (3/2) * t))
    
    # Branch + Inversion
    I_phi = 1.0 if phi < 3 else 0.15 + 0.85 * np.tanh((phi - 1.85) * 5.8)
    slow_start = 1.0 if t > 5 else t / 5.0
    contraction = 0.92 if t < 36 else 0.65
    
    # Gesamt dc
    d_dc = (ALPHA_FLOW * f_field 
            + BETA_SWIRL * f_vdp 
            + GAMMA_MEMORY * f_kuramoto 
            + DELTA_RESONANCE * f_compass 
            + f_iota 
            + f_janus 
            + f_lyapunov) * I_phi * slow_start * contraction
    
    # Phi-Update
    d_phi = 0.0
    if t > 36.0 and abs(dc) > 6.5:
        d_phi = 22.0 + 12.0 * (phi == 2)   # starker Hold in Forward2
    
    return [dc * contraction, d_dc, d_phi]

# ====================== Simulation ======================
print("🚀 Lade IEEE 300-Bus Netz... (v13.0 mit Janus + Lyapunov)")
net = pn.case300()

t_span = (0, 80)
x0 = [0.05, 0.0, 0]                     # c, dc, phi_idx
t_eval = np.linspace(0, 80, 1600)

# Realer Load Ramp
def load_ramp(t):
    return 1.0 + 0.022 * t

sol = solve_ivp(
    fun=lambda t, x: nexah_lorenz_ode(t, x, None),
    t_span=t_span,
    y0=x0,
    t_eval=t_eval,
    method='RK45',
    rtol=1e-6,
    max_step=0.05
)

t = sol.t
c = sol.y[0]
dc = sol.y[1]
phi_idx = np.round(sol.y[2]).astype(int).clip(0, 4)

# Klassische Voltage Collapse
voltage_classic = 1.0 / (1.0 + 1.15 * (0.022 * t)**2)

# Phi-Split erkennen
switch_time = None
for i in range(len(phi_idx)):
    if phi_idx[i] > 0 and (i == 0 or phi_idx[i-1] == 0):
        switch_time = t[i]
        break

lead_time = 80 - switch_time if switch_time is not None else 0

print(f"✅ Phi-Split bei t = {switch_time:.2f} s")
print(f"   → Vorsprung gegenüber klassischem Collapse: {lead_time:.1f} s")

# ====================== 4-Panel Plot ======================
fig = plt.figure(figsize=(14, 10))

# Voltage + Split
ax1 = fig.add_subplot(2, 2, 1)
ax1.plot(t, voltage_classic, 'r', lw=3, label="Klassische Voltage")
if switch_time is not None:
    ax1.axvline(x=switch_time, color='purple', linestyle='--', lw=3.5, label=f'Phi-Split t={switch_time:.2f}')
ax1.set_title("IEEE 300-Bus – Voltage Collapse (Janus + Lyapunov)")
ax1.set_ylabel("Spannung [p.u.]")
ax1.grid(True, alpha=0.5)
ax1.legend()

# Phi-Regulator
ax2 = fig.add_subplot(2, 2, 2)
ax2.plot(t, phi_idx, 'gold', lw=2, drawstyle='steps-post')
ax2.set_title("Phi-Regulator Zustand")
ax2.set_ylabel("Phi-Index")
ax2.set_yticks(range(5))
ax2.set_yticklabels(PHI_NAMES)
ax2.grid(True, alpha=0.5)

# Real Drift
ax3 = fig.add_subplot(2, 2, 3)
drift = np.diff(voltage_classic) / np.diff(t)
ax3.plot(t[1:], drift, 'cyan', lw=2, label="Realer Spannungs-Drift")
ax3.set_title("Realer Drift (Membrane / Kipper)")
ax3.set_ylabel("dV/dt")
ax3.grid(True, alpha=0.5)
ax3.legend()

# Phase Portrait
ax4 = fig.add_subplot(2, 2, 4)
ax4.plot(c, dc, 'b-', lw=1.5, alpha=0.7)
ax4.plot(c[-1], dc[-1], 'bo', markersize=8)
ax4.set_title("Phase Portrait (c vs dc)")
ax4.set_xlabel("c")
ax4.set_ylabel("dc")
ax4.grid(True, alpha=0.5)

plt.tight_layout()
plt.savefig("ieee300_real_tunable_v13.0_janus_lyapunov.png", dpi=420, bbox_inches='tight')
print("\n📸 Plot gespeichert als: ieee300_real_tunable_v13.0_janus_lyapunov.png")
plt.show()
