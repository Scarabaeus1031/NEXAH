...
def nexah_lorenz_ode(t, x):
    ...
    d_phi = 0.0
    # OPTIMIERTE GEOMETRIE-TRIGGER-BEDINGUNG
    if t > 26.0 and abs(dc) > 2.0 and abs(c) > 1.3:
        d_phi = 26.0 + 16.0 * (phi == 2)
    
    return [dc * contraction, d_dc, d_phi]

print("🚀 Lade IEEE 300-Bus Netz... (v13.15 Optimized Geometry Trigger)")
net = pn.case300()
...
ax1.set_title("IEEE 300-Bus – Voltage Collapse (Optimized Trigger v13.15)")
...
plt.tight_layout()
plt.savefig("ieee300_real_tunable_v13.15_optimized_trigger.png", dpi=420, bbox_inches='tight')
print("\n📸 Plot gespeichert als: ieee300_real_tunable_v13.15_optimized_trigger.png")
plt.show()
