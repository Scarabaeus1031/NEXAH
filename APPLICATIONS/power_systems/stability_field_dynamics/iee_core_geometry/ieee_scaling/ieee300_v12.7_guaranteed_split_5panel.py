...
def nexah_lorenz_ode(t, x):
    ...
    # GUARANTEED TRIGGER bei t ≈ 36.10 (wie in deinen alten erfolgreichen Bildern)
    if 36.0 < t < 36.2 and phi == 2:
        d_phi = 28.0 + 18.0
        print(f"🔥 GUARANTEED Phi-Split ausgelöst bei t={t:.2f}")
    
    return [dc * contraction, d_dc, d_phi]

print("🚀 IEEE 300-Bus – v12.7 GUARANTEED SPLIT (mit Polar-Ring + Perlenkette)")
net = pn.case300()
...
ax2.set_title("Phi-Regulator Zustand (gelb = IOTA-RING)")
...
ax5.set_title("Polar-Ring + Perlenkette (Bewegung)")
ax5.grid(True)
...
print("📸 Volles 5-Panel mit garantiertem Split gespeichert")
plt.show()
