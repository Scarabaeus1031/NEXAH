...
def pearl_restriction_trigger(t, c, dc, phi_idx):
    angle = np.arctan2(dc, c)
    r = np.sqrt(c**2 + dc**2)
    ghost_069 = abs(int(t) % 100 - 69) < 3
    lilith_hit = abs(angle - np.pi * LILITH_EDGE) < 0.18
    restriction = (r > 4.5 and r < 6.5) and (dc < -0.4) and (phi_idx == 2)
    pearl_hit = (int(t) % 13 < 3) or (int(t) % 19 < 2)
    return ghost_069 and lilith_hit and restriction and pearl_hit
...
def nexah_lorenz_ode(t, x):
    ...
    if t > 26.0 and pearl_restriction_trigger(t, c, dc, phi):
        d_phi = 34.0 + 24.0   # Pearl-Chain + Restriction + Acceleration Boost
    
    return [dc * contraction, d_dc, d_phi]

print("🚀 v13.9.14 – Pearl-Chain + Restriction + Acceleration + Stengel + Lilith + 069")
...
ax5.set_title("GhostGrid + Pearl-Chain + Restriction + Lilith-Edge")
ax5.grid(True)

plt.suptitle("NEXAH v13.9.14 – Pearl-Chain + Restriction + Stengel + Lilith + Zeta", fontsize=16)
plt.tight_layout()
plt.savefig("ieee9241_v13.9.14_pearl_restriction_5panel.png", dpi=300)
print("📸 5-Panel Serie mit Pearl-Chain + Restriction gespeichert")
plt.show()
