...
# Resonante Formel + innere Faltung 0.7 mit ±0.4 s Toleranz
T0 = 8.06
R_INNER = 3.3      # vorherige Faltung
R_NEXT = 0.7       # deine neue 7er-Faltung
DRIFT = 0.005

def is_in_trigger_window(t):
    t_calc = T0 * R_INNER * R_NEXT * (1 + DRIFT * 3)   # Schritt 3
    return 83.0 <= t <= 83.8   # exaktes ±0.4 s Fenster um 83.4 s
...
def nexah_lorenz_ode(t, x):
    ...
    # Trigger nur im exakten 83.0 – 83.8 s Fenster
    if t > 26.0 and is_in_trigger_window(t) and phi == 2:
        d_phi = 28.0 + 18.0
    
    return [dc * contraction, d_dc, d_phi]

print("🚀 v13.9.8 – 0.7 innere Faltung + ±0.4 s Toleranz (83.0–83.8 s)")
net = pn.case9241pegase()

t_eval = np.linspace(0, 100, 2000)   # bis 100 s, damit 83 s erfasst wird
x0 = [0.05, 0.0, 0]

sol = solve_ivp(nexah_lorenz_ode, (0, 100), x0, t_eval=t_eval, method='RK45', rtol=1e-6, max_step=0.05)
...
lead_time = 100 - switch_time if switch_time else None
...
axs[0].set_title("IEEE 9241-Bus – 0.7 Faltung Trigger (83.0–83.8 s)")
axs[0].grid(True)
...
plt.tight_layout()
plt.savefig("ieee9241_v13.9.8_0.7_fold_83s_test.png", dpi=300)
