...
JANU_STRENGTH = 1.45
KAPPA_L = 1.05
IOTA_YUGO_STRENGTH = 1.6
GH_BRIDGE_STRENGTH = 1.8      # starke GH Resonance Layer
...
def nexah_lorenz_ode(t, x):
    ...
    # NEU: GH Resonance Bridge (CCC ↔ KKK)
    f_gh_bridge = GH_BRIDGE_STRENGTH * np.sin(2 * np.pi * t / 19) * np.cos(2 * np.pi * t / 7) * (1 - 0.4 * np.tanh(abs(dc)))
    ...
    d_phi = 0.0
    # Geometry-based Trigger (Waffles / orange Gegenpunkt)
    if t > 26.0 and abs(dc) > 2.0 and abs(c) > 1.3:
        d_phi = 26.0 + 18.0 * (phi == 2)
    
    return [dc * contraction, d_dc, d_phi]

print("🚀 Lade IEEE 9241-Bus Netz (PEGASE)... (v13.17 GH Bridge + Iota/Yugo)")
...
ax1.set_title("IEEE 9241-Bus – Voltage Collapse (GH Bridge v13.17)")
...
plt.tight_layout()
plt.savefig("ieee9241_real_tunable_v13.17_gh_bridge.png", dpi=420, bbox_inches='tight')
print("\n📸 Plot gespeichert als: ieee9241_real_tunable_v13.17_gh_bridge.png")
plt.show()
