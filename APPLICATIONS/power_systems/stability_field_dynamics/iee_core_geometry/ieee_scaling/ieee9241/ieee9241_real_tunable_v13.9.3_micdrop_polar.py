...
JANU_STRENGTH = 1.45
KAPPA_L = 1.05
IOTA_YUGO_STRENGTH = 1.15      # stärkerer Iota-Puls für polare Rotation
GH_BRIDGE_STRENGTH = 0.95      # leicht angehoben
...
def nexah_lorenz_ode(t, x):
    ...
    d_phi = 0.0
    if t > 26.0 and abs(dc) > 2.1 and abs(c) > 1.25:
        d_phi = 26.0 + 16.0 * (phi == 2)   # polare Trigger-Stärke
    
    return [dc * contraction, d_dc, d_phi]

print("🚀 v13.9.3 – polare Trigger für echten Mic-Drop (9241-Bus)")
...
plt.tight_layout()
plt.savefig("ieee9241_v13.9.3_micdrop_polar_test.png", dpi=300)
