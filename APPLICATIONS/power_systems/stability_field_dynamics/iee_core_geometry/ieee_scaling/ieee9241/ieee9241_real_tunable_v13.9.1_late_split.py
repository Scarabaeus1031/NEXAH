...
# === Basis exakt wie v13.9 (starke Dynamik) ===
ALPHA_FLOW = 0.95
BETA_SWIRL = 0.65
GAMMA_MEMORY = 0.40
...
def nexah_lorenz_ode(t, x):
    ...
    f_iota_yugo = 0.95 * np.sin(2 * np.pi * (t - 36) / 19) * np.cos(2 * np.pi * t / 7)
    ...
    if t > 26.0 and abs(dc) > 2.2 and abs(c) > 1.25:
        d_phi = 22.0 + 12.0 * (phi == 2)   # sanfter als vorher
    
    return [dc * contraction, d_dc, d_phi]

print("🚀 Lade IEEE 9241-Bus... (v13.9.1 – nur sanfter später Split)")
...
ax1.set_title("IEEE 9241-Bus – v13.9.1 (sanfter später Split)")
...
ax3.plot(t[1:], np.diff(voltage_classic)/np.diff(t), 'cyan', lw=2)
...
plt.tight_layout()
plt.savefig("ieee9241_v13.9.1_late_split.png", dpi=420)
