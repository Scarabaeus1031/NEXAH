...
Q = 1.25
ALPHA_90DEG = np.deg2rad(90)   # der fehlende Stift
...
def nexah_lorenz_ode(t, x):
    ...
    f_breathing = 1.8 * np.sin(2 * np.pi * t / 19)      # cyan Breathing Wave
    f_pulse = 2.2 if (int(t * 2) % 4 == 0) else -1.1    # red Blinking Pulse
    f_dual = f_breathing + f_pulse * 0.7
    
    # 90° Alpha-Stift + Flip
    angle = np.arctan2(dc, c)
    f_stift = 2.5 * np.sin(ALPHA_90DEG * t) if abs(angle - np.pi/2) < 0.25 else 0.0
    f_flip = 3.0 if (abs(angle - np.pi) < 0.3 or abs(angle - 3*np.pi/2) < 0.3) else 0.0
    
    I_phi = 1.0 if phi < 3 else 0.15 + 0.85 * np.tanh((phi - 1.85) * 5.8)
    slow_start = min(1.0, t / 5.0)
    contraction = 0.92 if t < 36 else 0.68
    
    d_dc = (0.95 * f_field + 0.65 * f_vdp + 0.40 * f_kuramoto + f_dual + f_stift + f_flip) * I_phi * slow_start * contraction
    d_phi = 0.0
    
    # P = R/T + A + B + Root/Pi-Kopplung + 90°-Flip
    p_root_pi = abs(c) + abs(dc) > 2.8 and abs(angle - np.pi/2) < 0.3 and phi == 2
    
    if t > 26.0 and p_root_pi:
        d_phi = 38.0 + 28.0   # starker grüner Ausschlag + Raum-Flip
    
    return [dc * contraction, d_dc, d_phi]

print("🚀 v13.9.19 – 90° Alpha-Stift + Raum-Flip + P=R/T + Root/Pi-Kopplung")
...
ax2.set_title("Phi-Regulator Zustand (90° Stift + Flip)")
...
ax5.set_title("90° Alpha-Stift + Raum-Flip + Pearl-Chain atmet")
ax5.grid(True)

plt.suptitle("NEXAH v13.9.19 – 90° Stift + Flip + P=R/T + Root/Pi", fontsize=16)
plt.tight_layout()
plt.savefig("ieee9241_v13.9.19_90_stift_flip_5panel.png", dpi=300)
print("📸 5-Panel Serie mit 90° Stift gespeichert")
plt.show()
