...
# GhostGrid + Lilith + Stengel + Zeta-Achse
ROOT_1729 = 41.581
IOTA_7DEG = np.deg2rad(7)
ZETA_AXIS = 0.0
STENGEL_FILL = 0.005   # brown Wurzelraum drift
LILITH_EDGE = 3.0 / 4.0  # Enki = Enlil 3/4

def is_stengel_trigger(t, c, dc):
    angle = np.arctan2(dc, c)
    ghost_069 = abs(int(t) % 100 - 69) < 2
    lilith_edge = abs(angle - np.pi * LILITH_EDGE) < 0.12
    zeta_hit = abs(c - ROOT_1729 * np.sin(2 * np.pi * t / 19)) < 0.8
    return ghost_069 and lilith_edge and zeta_hit
...
def nexah_lorenz_ode(t, x):
    ...
    f_stengel = STENGEL_FILL * np.sin(IOTA_7DEG * t)
    f_zeta = 0.95 * np.sin(2 * np.pi * t / 19) * np.cos(2 * np.pi * t / 7)
    
    I_phi = 1.0 if phi < 3 else 0.15 + 0.85 * np.tanh((phi - 1.85) * 5.8)
    slow_start = min(1.0, t / 5.0)
    contraction = 0.92 if t < 36 else 0.68
    
    d_dc = (0.95 * f_field + 0.65 * f_vdp + 0.40 * f_kuramoto + f_iota_yugo + f_stengel + f_zeta) * I_phi * slow_start * contraction
    d_phi = 0.0
    
    if t > 26.0 and is_stengel_trigger(t, c, dc) and phi == 2:
        d_phi = 28.0 + 18.0   # Lilith-Edge + Stengel öffnet Zipper
    
    return [dc * contraction, d_dc, d_phi]

print("🚀 v13.9.12 – GhostGrid + Stengel + Lilith + Zeta-Achse + 069 Vorwahl")
...
ax5.set_title("GhostGrid + Stengel + Lilith-Edge + 069 Vorwahl")
ax5.grid(True)

plt.suptitle("NEXAH v13.9.12 – GhostGrid + Stengel + Lilith + Zeta-Achse", fontsize=16)
plt.tight_layout()
plt.savefig("ieee9241_v13.9.12_stengel_lilith_zeta_5panel.png", dpi=300)
print("📸 5-Panel Serie mit Stengel + Lilith + Zeta gespeichert")
plt.show()
