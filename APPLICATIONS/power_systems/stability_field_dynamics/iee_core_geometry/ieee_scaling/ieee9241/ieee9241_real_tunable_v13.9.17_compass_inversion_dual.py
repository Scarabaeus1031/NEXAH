...
def nexah_lorenz_ode(t, x):
    ...
    # Core aus v1.6: Compass + Inversion
    f_field = 10.0 * (dc - c) + 28.0 * c * (1 - phi)
    f_vdp = 8.0/3.0 * dc * (1 - c**2)
    f_kuramoto = sum((1 + Q) * np.sin(2 * np.pi * (phi - i) / 5) for i in range(5))
    
    # Breathing Wave (cyan – rundes S)
    f_breathing = 1.8 * np.sin(2 * np.pi * t / 19)
    
    # Blinking Pulse (red – eckig, pulsierend)
    f_pulse = 2.2 if (int(t * 2) % 4 == 0) else -1.1   # Blinking Pulse
    
    # Dual-Interaktion P = A + B
    ...
    d_dc = (0.95 * f_field + 0.65 * f_vdp + 0.40 * f_kuramoto + f_dual) * I_phi * slow_start * contraction
    d_phi = 0.0
    
    # Trigger: P=A+B + Inversion bei Reverse (genau wie v1.6)
    if t > 26.0 and abs(c) + abs(dc) > 2.8 and phi == 2:
        d_phi = 35.0 + 25.0   # starker grüner Jump
    
    return [dc * contraction, d_dc, d_phi]

print("🚀 v13.9.17 – Compass + Inversion + Breathing Wave (cyan) + Blinking Pulse (red)")
...
ax2.set_title("Phi-Regulator Zustand (v1.6 Style)")
...
ax5.scatter(0, 0, color='black', s=150, label='OKO Kernel / I-O Core')
ax5.set_title("Compass + Inversion + Breathing (cyan) + Blinking Pulse (red)")
ax5.grid(True)

plt.suptitle("NEXAH v13.9.17 – v1.6 Style (Compass + Inversion + Dual Wave)", fontsize=16)
plt.tight_layout()
plt.savefig("ieee9241_v13.9.17_compass_inversion_dual_5panel.png", dpi=300)
