...
def nexah_lorenz_ode(t, x):
    ...
    d_dc = (0.95 * f_field + f_vdp + f_kuramoto + f_compass + 0.8 * p_drive + resonance + 0.9 * winding_number)
    d_dc *= inversion * contraction * slow_start
    d_dc += 1.18 * (0.022 * t) * slow_start   # etwas stärker, um Kipper zu erreichen
    
    d_phi = resonance + 1.1 * winding_number
    if abs(dc) > WINDING_THRESHOLD and phi < 4 and t > 32:   # früherer Trigger bei ~37s
        d_phi += 9.2
...
plt.title("IEEE 118-Bus – Echter Test mit n-Bands + Kipper-Sensitivität (v11.2)")
plt.xlabel("Zeit [s]")
...
plt.title("Phi-Regulator Zustand (n-Bands + ANU-Gate aktiv)")
plt.xlabel("Zeit [s]")
...
plt.tight_layout()
plt.savefig("ieee118_real_tunable_v11.2_nbands_kipper.png", dpi=420, bbox_inches='tight')
print("\n📸 Plot gespeichert als: ieee118_real_tunable_v11.2_nbands_kipper.png")
plt.show()
