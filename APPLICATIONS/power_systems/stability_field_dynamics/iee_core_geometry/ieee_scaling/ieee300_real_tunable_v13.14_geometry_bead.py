...
def nexah_lorenz_ode(t, x):
    ...
    d_phi = 0.0
    # Geometry-based Trigger (Ramanujan-Bead Style)
    if t > 24.0 and abs(dc) > 2.0 and abs(c) > 1.2:
        d_phi = 26.0 + 16.0 * (phi == 2)
    
    return [dc * contraction, d_dc, d_phi]

print("🚀 Lade IEEE 300-Bus Netz... (v13.14 Geometry-Bead Trigger)")
net = pn.case300()
...
ax1.set_title("IEEE 300-Bus – Voltage Collapse (Geometry-Bead v13.14)")
...
plt.tight_layout()
plt.savefig("ieee300_real_tunable_v13.14_geometry_bead.png", dpi=420, bbox_inches='tight')
print("\n📸 Plot gespeichert als: ieee300_real_tunable_v13.14_geometry_bead.png")
plt.show()
