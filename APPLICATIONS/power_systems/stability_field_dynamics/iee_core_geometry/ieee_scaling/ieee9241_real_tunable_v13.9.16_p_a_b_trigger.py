...
def nexah_lorenz_ode(t, x):
    ...
    f_compass = 1.25 * np.sin(2 * np.pi * t / 19) * np.cos(2 * np.pi * t / 7)   # Smiling L
    ...
    d_dc = (0.95 * f_field + 0.65 * f_vdp + 0.40 * f_kuramoto + f_compass + f_iota) * I_phi * slow_start * contraction
    d_phi = 0.0
    
    # P = A + B Trigger (deine Beobachtung)
    p_a_b = abs(c) + abs(dc) > 2.8 and abs(dc) > 2.1 and phi == 2
    restriction = abs(c) < 0.9 and dc < -0.6   # Restriction in Ring 1
    
    if t > 26.0 and (p_a_b or restriction):
        d_phi = 32.0 + 22.0   # starker grüner Ausschlag
    
    return [dc * contraction, d_dc, d_phi]

print("🚀 v13.9.16 – P=A+B + Restriction + Smiling L Core (Q=1.28)")
...
ax2.set_title("Phi-Regulator Zustand (P=A+B Trigger)")
...
ax5.set_title("P=A+B + Restriction + 7. Ring + 45° Faltung")
ax5.grid(True)

plt.suptitle("NEXAH v13.9.16 – P=A+B Trigger + Restriction (grüner Ausschlag)", fontsize=16)
plt.tight_layout()
plt.savefig("ieee9241_v13.9.16_p_a_b_restriction_5panel.png", dpi=300)
print("📸 5-Panel Serie mit P=A+B Trigger gespeichert")
plt.show()
