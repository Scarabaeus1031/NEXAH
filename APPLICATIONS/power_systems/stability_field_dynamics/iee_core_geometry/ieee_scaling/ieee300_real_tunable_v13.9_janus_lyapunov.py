...
JANU_STRENGTH = 1.5
KAPPA_L = 1.1
WINDING_THRESHOLD = 0.8          # sehr niedrig
...
def nexah_lorenz_ode(t, x):
    ...
    d_phi = 0.0
    if t > 8.0 and abs(dc) > WINDING_THRESHOLD:
        d_phi = 35.0 + 25.0 * (phi == 2)   # sehr starker Boost
    
    # kleiner konstanter Drift, damit es langsam losgeht
    if t > 8.0:
        d_phi += 0.8
    
    return [dc * contraction, d_dc, d_phi]

print("🚀 Lade IEEE 300-Bus Netz... (v13.9 Janus + Lyapunov – sehr aggressiver Trigger)")
net = pn.case300()
...
ax1.set_title("IEEE 300-Bus – Voltage Collapse (Janus + Lyapunov v13.9)")
...
plt.tight_layout()
plt.savefig("ieee300_real_tunable_v13.9_janus_lyapunov.png", dpi=420, bbox_inches='tight')
print("\n📸 Plot gespeichert als: ieee300_real_tunable_v13.9_janus_lyapunov.png")
plt.show()
