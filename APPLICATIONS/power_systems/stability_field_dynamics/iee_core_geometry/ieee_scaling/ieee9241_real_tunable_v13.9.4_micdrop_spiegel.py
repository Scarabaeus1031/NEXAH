...
def nexah_lorenz_ode(t, x):
    ...
    d_phi = 0.0
    # === NEUER SPIEGEL-TRIGGER (Pi/Phi-Dualität + Splinter-Geometrie) ===
    if t > 26.0 and abs(dc) > 2.05 and abs(c) > 1.22 and (phi == 2 or phi == 0):
        d_phi = 28.0 + 18.0 * (phi == 2)   # Spiegel-Punkt aktiviert Forward2
    
    return [dc * contraction, d_dc, d_phi]

print("🚀 v13.9.4 – Spiegel-Trigger (Pi/Phi-Dualität) für echten Mic-Drop")
...
ax1.set_title("IEEE 9241-Bus – v13.9.4 Spiegel-Trigger Mic-Drop")
ax1.grid(True)
...
plt.tight_layout()
plt.savefig("ieee9241_v13.9.4_micdrop_spiegel_test.png", dpi=300)
