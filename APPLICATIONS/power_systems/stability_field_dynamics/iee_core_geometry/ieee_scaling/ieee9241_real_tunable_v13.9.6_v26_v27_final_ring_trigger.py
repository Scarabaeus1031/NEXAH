...
def nexah_lorenz_ode(t, x):
    ...
    # V26 Ring + V27 Waltz + NEXIT-Gateway Trigger
    if t > 26.0 and abs(dc) > 2.05 and abs(c) > 1.25 and iota_pos > 13.4 and abs(waltz_phase) > 0.88:
        d_phi = 28.0 + 18.0 * (phi == 2)
    
    return [dc * contraction, d_dc, d_phi]

print("🚀 v13.9.6 – V26 Ring Flow + V27 Waltz + NEXIT Trigger (final)")
...
# Polar Ring Flow (V26 Style)
ax2 = fig.add_subplot(1, 2, 2, projection='polar')
theta = np.arctan2(dc, c)
r = np.sqrt(c**2 + dc**2)
ax2.plot(theta, r, 'b-', lw=1.5, alpha=0.8)
ax2.scatter(theta, r, c=phi_idx, cmap='viridis', s=30)
ax2.scatter(theta[-1], r[-1], color='red', s=120, label='Current State (NEXIT?)')
ax2.set_title("V26 Ring Flow + NEXIT Gateway (Polar)")
ax2.grid(True)

plt.tight_layout()
plt.savefig("ieee9241_v13.9.6_v26_v27_ring_nexit_test.png", dpi=300)
print("📸 Ring-Flow Polar Plot gespeichert")
plt.show()
