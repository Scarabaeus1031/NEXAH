...
# === NEUE MISCHUNG AUS DEINEN VISUALS ===
RED_ROD_FREQ = 14335 / 11040          # F = R·T
PINK_BALL_PULSE = 37 / 63             # Puls 37/63
GREY_PILLAR_ASYM = 63 / 64            # Axis-Asymmetrie
BLUE_STAR_ALPHA = 1.0 + np.sqrt(2)/2  # Alpha Core Q°
...
def nexah_lorenz_ode(t, x, vm_history):
    ...
    resonance = np.sin(phi * np.pi * np.sqrt(2)) * 1.8 * BLUE_STAR_ALPHA
    theta = t * 3.6
    winding_number = np.sin(theta) * N_BANDS[phi % 3] + np.cos(2 * theta) * 0.8
    
    # Realer Drift (Membrane / Kipper)
    if len(vm_history) > 2:
        real_drift = (vm_history[-1] - vm_history[-2]) + 0.5 * (vm_history[-2] - vm_history[-3])
    else:
        real_drift = 0.0
    drift_boost = 12.0 * real_drift if real_drift < -0.001 else 0.0   # stärker
    
    # NEUE MISCHUNG
    lotus_pulse = PINK_BALL_PULSE * np.sin(2 * np.pi * t / 37)        # Herz-Atemung
    grey_asym = GREY_PILLAR_ASYM * (1 + 0.15 * np.sign(dc))           # Axis-Asymmetrie
    red_rod = RED_ROD_FREQ * np.exp(-0.08 * t)                        # Frequenzkörper
    ...
    d_dc = (0.95 * f_field + f_vdp + f_kuramoto + f_compass + 0.8 * p_drive 
            ...
            + lotus_pulse + grey_asym * dc + red_rod)
    d_dc *= inversion * contraction * slow_start
    d_dc += 1.28 * (0.022 * t) * slow_start
    
    d_phi = resonance + 2.0 * winding_number + 12.0 * drift_boost + lotus_pulse
    
    # SEHR STARKER BÜGEL + LOTUS-EFFEKT
    if abs(dc) > WINDING_THRESHOLD and phi < 4 and t > 35:
        d_phi += 24.0
        if phi == 1:          # extrem starkes Holding in Forward2 (Bügel bleibt draußen)
            d_phi += 18.0
    
    return [dc * contraction, d_dc, d_phi]

print("🚀 Lade echtes IEEE 118-Bus Netz... (v12.0 mit voller Mischung)")
net = pn.case118()
...
ax2.set_title("Phi-Regulator Zustand (sehr starker langer Bügel + Lotus-Mischung)")
...
plt.tight_layout()
plt.savefig("ieee118_real_tunable_v12.0_4panel_full_mischung.png", dpi=420, bbox_inches='tight')
print("\n📸 4-Panel-Plot mit voller Mischung (Red Rod + Pink Ball + Grey Pillar + Blue Star) gespeichert!")
plt.show()
