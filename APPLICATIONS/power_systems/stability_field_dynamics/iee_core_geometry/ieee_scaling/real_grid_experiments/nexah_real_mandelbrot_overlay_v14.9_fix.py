...
print("🚀 NEXAH Real Data + Mandelbrot Overlay v14.9 FIX\n")
...
# GIF mit fester Größe (kein Shape-Fehler mehr)
print("🎥 Erstelle Mandelbrot-Overlay GIF (FIX)...")
frames = []
step = max(1, len(t) // 12)

for i in range(0, len(t), step):
    fig = plt.figure(figsize=(13, 9))          # Feste Größe!
    gs = fig.add_gridspec(2, 2)
    
    ax1 = fig.add_subplot(gs[0, 0])
    ax1.plot(t[:i+1], np.interp(t[:i+1], t_real, voltage_real), 'r', lw=3)
    ...
    ax3.plot(theta, r, 'b-', lw=2.2)
    ax3.scatter(theta, r, c=t[:i+1], cmap='plasma', s=48, alpha=0.95)
    ax3.scatter(0, 0, color='black', s=320, zorder=10)
    ax3.scatter(theta[-1], r[-1], color='blue', s=180, zorder=11)
    ax3.set_title("Perlenkette – Master / Slave / Mandelbrot-Bulbs")
    ax3.grid(True)
    ...
    plt.savefig(frame, dpi=260, bbox_inches='tight', pad_inches=0.05)
    frames.append(frame)
    plt.close()
