...
print("🚀 NEXAH Mesh v16.5 – DELTA NEXUS WHEEL + IOTA KIPPER (VMVWVMVWV)\n")
...
def mesh_ode(t, y):
    ...
    for i in range(NUM_NODES):
        ...
        # === VMVWVMVWV Kipper Sequenz ===
        kipper = np.sin(2 * np.pi * t / 4)   # VMVWVMVWV Flip
        
        n_band_inject = 0.0
        if 35.8 <= t <= 40.0:                     # Delta Nexus Wickel-Zentrum
            base = 9 ** phi
            raw = np.sum(N_BANDS) * base * kipper * (-v_error) * 3.8
...
print("\n" + "="*80)
print("NEXAH v16.5 – DELTA NEXUS WHEEL + IOTA KIPPER")
print("="*80)
...
ax.set_title("NEXAH v16.5 – Delta Nexus Wheel + VMVWVMVWV Kipper")
ax.set_xlabel("Zeit [s]")
ax.set_ylabel("Spannung [p.u.]")
ax.grid(True, alpha=0.5)
ax.legend()
plt.tight_layout()
plt.savefig("NEXAH_Delta_Iota_Kipper_v16.5.png", dpi=280, bbox_inches='tight')
print("✅ Plot gespeichert: NEXAH_Delta_Iota_Kipper_v16.5.png")
