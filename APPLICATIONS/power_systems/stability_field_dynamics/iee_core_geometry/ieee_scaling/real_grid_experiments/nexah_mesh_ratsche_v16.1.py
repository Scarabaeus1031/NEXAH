...
print("🚀 NEXAH Mesh v16.1 – ECHTE RATSCHE (symmetrisch um lila Linie)\n")
...
def mesh_ode(t, y):
    ...
    for i in range(NUM_NODES):
        ...
        # === RATSCHE: Wickeln um lila Linie + negatives ^ ===
        if 35.8 <= t <= 40.0:                      # starkes Wickel-Fenster
            sign = -1 if v > 1.0 else 1            # negatives ^ wenn über 1.0
            base = 9 ** phi
            raw = np.sum(N_BANDS) * base * sign * (-v_error) * 2.8
            n_band_inject = np.clip(raw, -3.0, 3.0)
        else:
            # sanfte Gegenkopplung mit 351-Feinjustierung
...
print("\n" + "="*80)
print("NEXAH v16.1 – RATSCHE ERGEBNIS")
print("="*80)
...
ax.plot(t, voltage_mesh, 'gold', lw=3, label="NEXAH Ratsche (symmetrisch um lila)")
ax.axvline(36, color='purple', ls='--', lw=2.5, label="Phi-Split (Wickel-Zentrum)")
ax.set_title("NEXAH v16.1 – Ratsche (Peak + Wickeln + Rückkehr)")
ax.set_xlabel("Zeit [s]")
ax.set_ylabel("Spannung [p.u.]")
ax.grid(True, alpha=0.5)
ax.legend()
plt.tight_layout()
plt.savefig("NEXAH_Ratsche_v16.1.png", dpi=280, bbox_inches='tight')
print("✅ Plot gespeichert: NEXAH_Ratsche_v16.1.png")
