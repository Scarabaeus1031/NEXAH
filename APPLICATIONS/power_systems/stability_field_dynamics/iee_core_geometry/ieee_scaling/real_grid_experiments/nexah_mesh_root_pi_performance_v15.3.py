...
print("🚀 NEXAH Mesh Network v15.3 – kontrollierte Root-Root + Pi^Pi\n")
...
def mesh_ode(t, y):
    ...
    for i in range(NUM_NODES):
        ...
        if 35.8 < t < 36.5:
            ...
            raw = np.sum(N_BANDS) * phi_phi * root_root * 0.035   # stark reduziert
            n_band_inject = np.clip(raw, -2.5, 2.5)               # HARTER LIMITER
        ...
        for neigh in [(i-1)%NUM_NODES, (i+1)%NUM_NODES]:
            d_dc += 0.85 * np.sin(2 * np.pi * (phi - states[neigh,2]) / 5)
...
voltage_mesh = voltage_classic + 0.12 * np.mean(sol.y[0::3], axis=0)
...
print("\n" + "="*65)
print("NEXAH MESH v15.3 – KONTROLLIERTE LEISTUNG")
print("="*65)
...
ax.plot(t, voltage_mesh, 'gold', lw=3, label="NEXAH Mesh (kontrolliert)")
ax.axvline(36, color='purple', ls='--', lw=2.5, label="Phi-Split")
ax.set_title("NEXAH Mesh – jetzt kontrolliert und messbar")
ax.set_xlabel("Zeit [s]")
ax.set_ylabel("Spannung [p.u.]")
ax.grid(True, alpha=0.5)
ax.legend()
plt.tight_layout()
plt.savefig("NEXAH_Mesh_Leistung_kontrolliert_v15.3.png", dpi=280, bbox_inches='tight')
print("✅ Plot gespeichert: NEXAH_Mesh_Leistung_kontrolliert_v15.3.png")
