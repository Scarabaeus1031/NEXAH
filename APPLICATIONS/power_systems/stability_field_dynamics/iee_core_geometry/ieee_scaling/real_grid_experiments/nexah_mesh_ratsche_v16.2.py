...
print("🚀 NEXAH Mesh v16.2 – STÄRKERE RATSCHE + KREUZUNG\n")
...
def mesh_ode(t, y):
    ...
    for i in range(NUM_NODES):
        ...
        # === STÄRKERE RATSCHE ===
        if 35.8 <= t <= 40.0:                      # Wickel-Fenster
            sign = -1 if v > 1.0 else 1
            base = 9 ** phi
            raw = np.sum(N_BANDS) * base * sign * (-v_error) * 3.5   # stärker
            n_band_inject = np.clip(raw, -3.5, 3.5)
        else:
            raw = 351 * 0.0015 * (-v_error)                        # stärkerer Decay
        ...
        if t > 40.0:
            d_dc -= 0.65 * c   # stärkerer Scarabäus-Decay
...
print("\n" + "="*80)
print("NEXAH v16.2 – RATSCHE MIT KREUZUNG")
print("="*80)
...
ax.plot(t, voltage_mesh, 'gold', lw=3, label="NEXAH Ratsche v16.2")
ax.axvline(36, color='purple', ls='--', lw=2.5, label="Phi-Split")
ax.set_title("NEXAH v16.2 – hoffentlich mit Kreuzung + symmetrischem Wickeln")
ax.set_xlabel("Zeit [s]")
ax.set_ylabel("Spannung [p.u.]")
ax.grid(True, alpha=0.5)
ax.legend()
plt.tight_layout()
plt.savefig("NEXAH_Ratsche_v16.2.png", dpi=280, bbox_inches='tight')
print("✅ Plot gespeichert: NEXAH_Ratsche_v16.2.png")
