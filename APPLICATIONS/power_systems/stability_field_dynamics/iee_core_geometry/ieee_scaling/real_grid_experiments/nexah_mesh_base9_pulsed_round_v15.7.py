...
print("🚀 NEXAH Mesh v15.7 – Pulsed Base-9 + Root-12/13 + 66→69 Phasenwechsel\n")
...
def mesh_ode(t, y):
    ...
    for i in range(NUM_NODES):
        ...
        # === 1. Puls: Base-9 (kurz hoch) ===
        if 36.0 <= t <= 40.0 and phi >= 2:
            base9 = 9 ** phi
            raw = np.sum(N_BANDS) * base9 * BASE9_LOCK_RATIO * (-v_error) * 1.8
            n_band_inject = np.clip(raw, -2.2, 2.2)
        
        # === 2. Gegenkurve: Root-12/13 + 66→69 Phasenwechsel (runter) ===
        elif t > 40.0 and phi >= 2:
            counter = (ROOT_12 + ROOT_13) * PHASE_66_69 * 0.75
            raw = np.sum(N_BANDS) * counter * v_error * 2.2   # aktiv gegen v_error
            n_band_inject = np.clip(raw, -2.5, 2.5)
        ...
        for neigh in [(i-1)%NUM_NODES, (i+1)%NUM_NODES]:
            d_dc += BASE9_LOCK_RATIO * 0.6 * np.sin(2 * np.pi * (phi - states[neigh,2]) / 5)
...
print("\n" + "="*75)
print("NEXAH MESH v15.7 – ROOT-12/13 + 66→69 GEGENKURVE")
print("="*75)
...
ax.plot(t, voltage_mesh, 'gold', lw=3, label="NEXAH Mesh + Root-12/13 + 66→69 Gegenkurve")
ax.axvline(36, color='purple', ls='--', lw=2.5, label="Phi-Split")
ax.set_title("NEXAH Mesh v15.7 – jetzt mit runder Gegenkurve (sollte peak + runter)")
ax.set_xlabel("Zeit [s]")
ax.set_ylabel("Spannung [p.u.]")
ax.grid(True, alpha=0.5)
ax.legend()
plt.tight_layout()
plt.savefig("NEXAH_Mesh_Root12_66_69_v15.7.png", dpi=280, bbox_inches='tight')
print("✅ Plot gespeichert: NEXAH_Mesh_Root12_66_69_v15.7.png")
