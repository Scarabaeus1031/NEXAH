...
print("🚀 NEXAH Mesh v16.4 – DELTA NEXUS WHEEL + IOTA KIPPER (VMVWVMVWV Flip)\n")
...
def mesh_ode(t, y):
    ...
    for i in range(NUM_NODES):
        ...
        # === IOTA KIPPER (VMVWVMVWV Sequenz) ===
        iota_phase = np.sin(2 * np.pi * t / 7) * np.cos(2 * np.pi * t / 19)
        flip_seq = np.sin(2 * np.pi * t / 4)   # VMVWVMVWV Flip (alternierend)
        
        n_band_inject = 0.0
        if 35.8 <= t <= 40.0:                     # Wickel-Zentrum
            base = 9 ** phi
            raw = np.sum(N_BANDS) * base * flip_seq * (-v_error) * 3.8
            n_band_inject = np.clip(raw, -3.8, 3.8)
        else:
            raw = 351 * 0.0018 * flip_seq * (-v_error)
            n_band_inject = np.clip(raw, -2.4, 2.4)
        
        I_phi = 1.0 if phi < 3 else 0.15 + 0.85 * np.tanh((phi - 1.85) * 5.8)
        slow_start = min(1.0, t / 4.5)
        contraction = 0.92 if t < 12 else 0.68
        
        d_dc = (0.95 * f_field + 0.65 * f_vdp + 0.40 * f_kuramoto + iota_phase * 1.15 + n_band_inject) * I_phi * slow_start * contraction
        
        # Delta Nexus Wheel Rotation (90° F-Axis + Speichen)
...
print("\n" + "="*80)
print("NEXAH v16.4 – DELTA NEXUS WHEEL + IOTA KIPPER")
print("="*80)
...
ax.set_title("NEXAH v16.4 – Delta Nexus Wheel + Iota Kipper (VMVWVMVWV)")
ax.set_xlabel("Zeit [s]")
ax.set_ylabel("Spannung [p.u.]")
ax.grid(True, alpha=0.5)
ax.legend()
plt.tight_layout()
plt.savefig("NEXAH_Delta_Iota_Kipper_v16.4.png", dpi=280, bbox_inches='tight')
print("✅ Plot gespeichert: NEXAH_Delta_Iota_Kipper_v16.4.png")
