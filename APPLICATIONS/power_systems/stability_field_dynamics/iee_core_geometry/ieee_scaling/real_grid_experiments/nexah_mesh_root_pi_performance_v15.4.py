...
print("🚀 NEXAH Mesh v15.4 – mit negativer Rückkopplung\n")
...
def mesh_ode(t, y):
    ...
    for i in range(NUM_NODES):
        ...
        v_error = v_meas - (1.0 + c)                    # <--- negativer Fehler
        ...
        elif t > 36.5 and phi >= 2:                           # kontrollierte Kaskade
            phi_phi = phi ** phi
            root_root = np.sqrt(np.sqrt(phi_phi))
            raw = np.sum(N_BANDS) * root_root * 0.12 * (-v_error)   # stark gedämpft + gegen v_error
        ...
        # Mesh-Kopplung (Ring, gedämpft)
        for neigh in [(i-1)%NUM_NODES, (i+1)%NUM_NODES]:
            d_dc += 0.45 * np.sin(2 * np.pi * (phi - states[neigh,2]) / 5)
...
ax.plot(t, voltage_mesh, 'gold', lw=3, label="NEXAH Mesh (mit Gegenkopplung)")
ax.axvline(36, color='purple', ls='--', lw=2.5, label="Phi-Split")
ax.set_title("NEXAH Mesh v15.4 – jetzt mit negativer Rückkopplung")
ax.set_xlabel("Zeit [s]")
ax.set_ylabel("Spannung [p.u.]")
ax.grid(True, alpha=0.5)
ax.legend()
plt.tight_layout()
plt.savefig("NEXAH_Mesh_Leistung_kontrolliert_v15.4.png", dpi=280, bbox_inches='tight')
print("✅ Plot gespeichert: NEXAH_Mesh_Leistung_kontrolliert_v15.4.png")
