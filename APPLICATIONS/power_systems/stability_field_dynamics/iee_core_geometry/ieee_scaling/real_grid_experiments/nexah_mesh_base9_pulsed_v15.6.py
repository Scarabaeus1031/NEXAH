...
print("🚀 NEXAH Mesh v15.6 – Pulsed Base-9 (kurzer Impuls am Phi-Split)\n")
...
def mesh_ode(t, y):
    ...
    for i in range(NUM_NODES):
        ...
        n_band_inject = 0.0
        # === PULSED INJECTION nur 36.0–40.0 s ===
        if 36.0 <= t <= 40.0 and phi >= 2:
            base9_step = 9 ** phi
            raw = np.sum(N_BANDS) * base9_step * BASE9_LOCK_RATIO * (-v_error) * 1.8
...
print("\n" + "="*70)
print("NEXAH MESH v15.6 – PULSED BASE-9 (kurzer Impuls)")
print("="*70)
...
ax.plot(t, voltage_mesh, 'gold', lw=3, label="NEXAH Mesh + Base-9 Puls (36–40 s)")
ax.axvline(36, color='purple', ls='--', lw=2.5, label="Phi-Split")
ax.set_title("NEXAH Mesh v15.6 – Pulsed Base-9 (sollte jetzt peak + abfallen)")
ax.set_xlabel("Zeit [s]")
ax.set_ylabel("Spannung [p.u.]")
ax.grid(True, alpha=0.5)
ax.legend()
plt.tight_layout()
plt.savefig("NEXAH_Mesh_Base9_Pulsed_v15.6.png", dpi=280, bbox_inches='tight')
print("✅ Plot gespeichert: NEXAH_Mesh_Base9_Pulsed_v15.6.png")
