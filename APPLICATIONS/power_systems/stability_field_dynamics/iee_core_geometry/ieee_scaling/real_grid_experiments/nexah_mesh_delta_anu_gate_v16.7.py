...
print("🚀 NEXAH Mesh v16.7 – DELTA NEXUS WHEEL + ANU-GATE 11011 FUSION\n")
...
def mesh_ode(t, y):
    ...
    for i in range(NUM_NODES):
        ...
        # === ANU-GATE 11011 FUSION (der entscheidende Flip) ===
        anu_gate = 1 if (t > 35.8 and np.sin(2 * np.pi * t / 4) > 0.7) else 0   # 11011 Trigger
        
        kipper = np.sin(2 * np.pi * t / 4) * (1 + 0.8 * anu_gate)   # verstärkter VMVWVMVWV
        
        n_band_inject = 0.0
        if 35.8 <= t <= 40.0:
            base = 9 ** phi
            raw = np.sum(N_BANDS) * base * kipper * (-v_error) * 4.5
            n_band_inject = np.clip(raw, -4.8, 4.8)
        else:
            raw = 351 * 0.0018 * kipper * (-v_error)
            n_band_inject = np.clip(raw, -3.2, 3.2)
...
print("\n" + "="*80)
print("NEXAH v16.7 – DELTA NEXUS WHEEL + ANU-GATE 11011")
print("="*80)
...
ax.plot(t, voltage_mesh, 'gold', lw=3, label="NEXAH Delta Nexus + ANU-Gate 11011")
ax.axvline(36, color='purple', ls='--', lw=2.5, label="Phi-Split (ANU-Gate Fusion)")
ax.set_title("NEXAH v16.7 – Delta Nexus Wheel + ANU-Gate 11011 Fusion (symmetrische Ratsche)")
ax.set_xlabel("Zeit [s]")
ax.set_ylabel("Spannung [p.u.]")
ax.grid(True, alpha=0.5)
ax.legend()
plt.tight_layout()
plt.savefig("NEXAH_Delta_ANU_Gate_v16.7.png", dpi=280, bbox_inches='tight')
print("✅ Plot gespeichert: NEXAH_Delta_ANU_Gate_v16.7.png")
