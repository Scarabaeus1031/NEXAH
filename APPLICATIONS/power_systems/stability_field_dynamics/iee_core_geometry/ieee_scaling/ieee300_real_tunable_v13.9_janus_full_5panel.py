...
def nexah_lorenz_ode(t, x):
    ...
    d_dc = (0.95 * f_field + 0.65 * f_vdp + 0.40 * f_kuramoto + f_compass) * I_phi * slow_start * contraction
    d_phi = 0.0
    
    if t > 26.0 and abs(dc) > 2.1 and abs(c) > 1.25 and phi == 2:
        d_phi = 28.0 + 18.0   # Janus + Lyapunov Trigger
    
    return [dc * contraction, d_dc, d_phi]

print("🚀 IEEE 300-Bus – v13.9 Janus + Lyapunov (erfolgreicher Phi-Split)")
net = pn.case300()
...
if switch_time:
    ax1.axvline(x=switch_time, color='purple', linestyle='--', lw=3.5, label='Phi-Split t=8.06')
...
ax2.set_title("Phi-Regulator Zustand (gelbe Linie)")
...
ax5.set_title("Polar Grid + Perlenkette (Bewegung + Restriction)")
ax5.grid(True)

plt.suptitle("IEEE 300-Bus – v13.9 Janus + Lyapunov (volles 5-Panel mit Perlenkette)", fontsize=16)
plt.tight_layout()
plt.savefig("ieee300_v13.9_janus_full_5panel.png", dpi=300)
print("📸 Volles 5-Panel mit gelber Linie + Polar-Perlenkette gespeichert")
plt.show()
