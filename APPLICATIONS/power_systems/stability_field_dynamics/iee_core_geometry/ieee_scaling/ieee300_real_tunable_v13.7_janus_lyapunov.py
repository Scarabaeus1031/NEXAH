...
# === Sehr aggressiver Trigger für Test ===
ALPHA_FLOW = 0.95
BETA_SWIRL = 0.65
GAMMA_MEMORY = 0.40
...
JANU_STRENGTH = 1.5
KAPPA_L = 1.1
WINDING_THRESHOLD = 1.2       # sehr niedrig
...
def nexah_lorenz_ode(t, x):
    ...
    if t > 8.0 and abs(dc) > WINDING_THRESHOLD:
        d_phi = 32.0 + 22.0 * (phi == 2)   # sehr starker Boost
    
    return [dc * contraction, d_dc, d_phi]

# ====================== Simulation ======================
print("🚀 Lade IEEE 300-Bus Netz... (v13.7 Janus + Lyapunov – sehr aggressiver Trigger)")
net = pn.case300()
...
ax1.set_title("IEEE 300-Bus – Voltage Collapse (Janus + Lyapunov v13.7)")
...
plt.tight_layout()
plt.savefig("ieee300_real_tunable_v13.7_janus_lyapunov.png", dpi=420, bbox_inches='tight')
print("\n📸 Plot gespeichert als: ieee300_real_tunable_v13.7_janus_lyapunov.png")
plt.show()
