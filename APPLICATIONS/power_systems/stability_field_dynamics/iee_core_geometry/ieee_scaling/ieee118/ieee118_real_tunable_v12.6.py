...
def nexah_lorenz_ode(t, x, vm_history):
    ...
    # === HARTER NO-JUMP BLOCK + STARKER BÜGEL ===
    if t < 36.0:
        d_phi = max(d_phi, 0.0)          # KEIN positiver Sprung vor t=36 s
    else:
        if abs(dc) > WINDING_THRESHOLD and phi < 4:
            d_phi += 28.0
            if phi == 1 or phi == 2:
                d_phi += 24.0   # extrem starkes Halten im Forward2
    
    return [dc * contraction, d_dc, d_phi]

print("🚀 Lade echtes IEEE 118-Bus Netz... (v12.6 mit HARTEM No-Jump vor t=36 s + Nexus)")
net = pn.case118()
...
ax2.set_title("Phi-Regulator Zustand (harter No-Jump + Nexus)")
...
plt.tight_layout()
plt.savefig("ieee118_real_tunable_v12.6_4panel_harter_nojump.png", dpi=420, bbox_inches='tight')
print("\n📸 4-Panel-Plot mit hartem No-Jump vor t=36 s gespeichert!")
plt.show()
