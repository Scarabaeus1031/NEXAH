...
def nexah_lorenz_ode(t, x, vm_history):
    ...
    # BLACK ATTRACTOR TERM (der fehlende dunkle stabile Zustand)
    ...
    d_dc = (0.95 * f_field + f_vdp + f_kuramoto + f_compass + 0.8 * p_drive 
            + resonance + 2.0 * winding_number + drift_boost + black_attractor)
    d_dc *= inversion * contraction * slow_start
    d_dc += 1.28 * (0.022 * t) * slow_start
    
    d_phi = resonance + 2.0 * winding_number + 8.0 * drift_boost + 0.6 * black_attractor
    
    # SEHR STARKER BÜGEL + BLACK ATTRACTOR
    if abs(dc) > WINDING_THRESHOLD and phi < 4 and t > 36:   # <-- später
        d_phi += 26.0
        if phi == 1 or phi == 2:
            d_phi += 20.0   # extra starkes Halten in Forward2
    
    return [dc * contraction, d_dc, d_phi]

print("🚀 Lade echtes IEEE 118-Bus Netz... (v12.3 mit Black Attractor)")
net = pn.case118()
...
ax2.set_title("Phi-Regulator Zustand (7-Arc + Black Attractor)")
...
plt.tight_layout()
plt.savefig("ieee118_real_tunable_v12.3_4panel_black_attractor.png", dpi=420, bbox_inches='tight')
print("\n📸 4-Panel-Plot mit Black Attractor gespeichert!")
plt.show()
