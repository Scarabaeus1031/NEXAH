...
# === EULER-L PRIME LATTICE (7-Arc) + CORE SYMMETRY ===
...
def nexah_lorenz_ode(t, x, vm_history):
    ...
    if len(vm_history) > 2:
        ...
        real_drift = 0.0
    drift_boost = 12.0 * real_drift if real_drift < -0.001 else 0.0
    
    # === CON ~ DAO + SUN-MOON-KISS (19x Drift) ===
    con_dao = np.sin(2 * np.pi * t / 19) * 0.9 + np.cos(2 * np.pi * t / 19) * 0.6
    kiss = np.exp(-0.12 * abs(t - 37.0)) * 2.2   # Kiss am Flip-Punkt
    
    # BLACK ATTRACTOR + KERNEL GRID HOLD
    black_attractor = -0.35 * c * (c**2 - 1.0) if phi >= 2 else 0.0
    grid_hold = 0.4 * np.sin(phi * np.pi * np.sqrt(2))   # NEXAH Kernel Symmetry
    ...
    d_dc = (0.95 * f_field + f_vdp + f_kuramoto + f_compass + 0.8 * p_drive 
            ...
            + con_dao + kiss + black_attractor + grid_hold)
    d_dc *= inversion * contraction * slow_start
    d_dc += 1.28 * (0.022 * t) * slow_start
    
    d_phi = resonance + 2.0 * winding_number + 8.0 * drift_boost + 0.6 * black_attractor + kiss
    
    # SEHR STARKER BÜGEL + CORE SYMMETRY
    if abs(dc) > WINDING_THRESHOLD and phi < 4 and t > 37:   # <-- jetzt ab 37 s
        d_phi += 26.0
        if phi == 1 or phi == 2:
            d_phi += 22.0   # extrem starkes Halten
    
    return [dc * contraction, d_dc, d_phi]

print("🚀 Lade echtes IEEE 118-Bus Netz... (v12.4 mit CON~DAO + Sun-Moon-Kiss + Kernel Grid)")
net = pn.case118()
...
ax2.set_title("Phi-Regulator Zustand (CON~DAO + Sun-Moon-Kiss + Kernel Grid)")
...
plt.tight_layout()
plt.savefig("ieee118_real_tunable_v12.4_4panel_con_dao_kiss.png", dpi=420, bbox_inches='tight')
print("\n📸 4-Panel-Plot mit CON~DAO + Sun-Moon-Kiss + Kernel Grid gespeichert!")
plt.show()
