...
# === EULER-L PRIME LATTICE (7-Arc) + NEXUS ===
...
def nexah_lorenz_ode(t, x, vm_history):
    ...
    if len(vm_history) > 2:
        ...
        real_drift = 0.0
    drift_boost = 10.0 * real_drift if real_drift < -0.001 else 0.0   # etwas gedämpft früh
    
    # NEXUS HOLD (7 Sphären + zentrales Grid)
    ...
    kiss = np.exp(-0.12 * abs(t - 38.0)) * 2.8   # Kiss genau am Kipper
    ...
    d_dc = (0.95 * f_field + f_vdp + f_kuramoto + f_compass + 0.8 * p_drive 
            ...
            + con_dao + kiss + black_attractor + nexus_hold)
    ...
    # HARTER SPÄTER SPLIT + SEHR STARKER BÜGEL
    if abs(dc) > WINDING_THRESHOLD and phi < 4 and t > 38:   # <-- jetzt ab 38 s
        d_phi += 28.0
        if phi == 1 or phi == 2:
            d_phi += 24.0   # extrem starkes Halten
    
    return [dc * contraction, d_dc, d_phi]

print("🚀 Lade echtes IEEE 118-Bus Netz... (v12.5 mit Nexus-Hold + hartem Kipper-Split)")
net = pn.case118()
...
ax2.set_title("Phi-Regulator Zustand (Nexus + harter Kipper-Split)")
...
plt.tight_layout()
plt.savefig("ieee118_real_tunable_v12.5_4panel_nexus_kipper.png", dpi=420, bbox_inches='tight')
print("\n📸 4-Panel-Plot mit Nexus + hartem Kipper-Split gespeichert!")
plt.show()
