"""
NEXAH IEEE 57-Bus — LORENZ TUNABLE v7.6
Stärkere Slow-Start-Rampe (Mitte 28 s) + Fold + Waffelschicht + Vortex Winding
"""
...
drift_threshold = 15.5       # du kannst das später noch anpassen
...
def nexah_lorenz_ode(t, x, params):
    ...
    # === STÄRKERE SLOW-START-RAMPE (Mitte bei 28 s) ===
    slow_start = 1.0 / (1.0 + np.exp(-0.42 * (t - 28.0)))   # steiler + später

    d_dc = (field_force + p_drive + kuramoto + vdp_force + compass_op + ring_offset + branch_pulse) * inversion
    d_dc += 0.195 * t * 1.35 * slow_start                    # Load-Ramp nur langsam
...
if __name__ == "__main__":
    print(f"🚀 NEXAH IEEE 57-Bus — LORENZ TUNABLE v7.6 (drift_threshold = {drift_threshold})")
    ...
    ax3d.set_title("Lorenz Core – Waffelschicht + Fold + Vortex Winding")
    ax3d.set_xlabel("c")
    ...
    ax2d.set_title("Collapse Prediction – IEEE 57-Bus (v7.6)")
    ...
    ax2d.legend()
    plt.tight_layout()
    plt.savefig(f"ieee57_lorenz_tunable_v7.6_threshold_{drift_threshold}.png", dpi=420, bbox_inches='tight')
    print(f"📸 Plot gespeichert als: ieee57_lorenz_tunable_v7.6_threshold_{drift_threshold}.png")
    plt.show()
