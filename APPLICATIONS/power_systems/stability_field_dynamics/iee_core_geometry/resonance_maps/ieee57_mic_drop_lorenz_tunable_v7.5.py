"""
NEXAH IEEE 57-Bus — LORENZ TUNABLE v7.5
Slow-Start-Ramp + fester Stengel + Backflow-Pillar + Golfschläger-Geometrie
"""
...
drift_threshold = 16.0       # <--- du kannst das später noch feiner stellen
...
def nexah_lorenz_ode(t, x, params):
    ...
    # Stengel-Tanz + Backflow-Pillar verstärkt
    ...
    # SLOW-START RAMP – hier passiert der Zauber
    slow_start = 1.0 / (1.0 + np.exp(-0.35 * (t - 22.0)))   # ramp ab ca. 15–30 s

    d_dc = (field_force + p_drive + kuramoto + vdp_force + compass_op + ring_offset + branch_pulse) * inversion
    d_dc += 0.195 * t * 1.45 * slow_start   # Load-Ramp nur mit Slow-Start
...
if __name__ == "__main__":
    print(f"🚀 NEXAH IEEE 57-Bus — LORENZ TUNABLE v7.5 (drift_threshold = {drift_threshold})")

    params = {'Q': 1.62}
    x0 = [0.05, 0.0, 0]

    sol = solve_ivp(
        fun=lambda t, x: nexah_lorenz_ode(t, x, params),   # direkt die ODE, kein extra wrapper
        t_span=(0, 80),
        y0=x0,
        method='RK45',
        rtol=1e-6,
    ...
    ax3d.set_title("Lorenz Core – Retrograde Mirror + Stengel-Tanz + Slow-Start")
    ax3d.set_xlabel("c")
    ...
    ax2d.set_title("Collapse Prediction – IEEE 57-Bus (v7.5)")
    ...
    ax2d.legend()
    plt.tight_layout()
    plt.savefig(f"ieee57_lorenz_tunable_v7.5_threshold_{drift_threshold}.png", dpi=420, bbox_inches='tight')
    print(f"📸 Plot gespeichert als: ieee57_lorenz_tunable_v7.5_threshold_{drift_threshold}.png")
    plt.show()
