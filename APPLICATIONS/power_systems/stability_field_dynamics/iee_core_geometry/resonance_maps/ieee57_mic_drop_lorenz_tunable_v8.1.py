"""
NEXAH IEEE 57-Bus — LORENZ TUNABLE v8.1
Unity + smiling L + Hirtenstock + offener Durchfluss + 17-29-5
"""
...
def nexah_lorenz_ode(t, x, params):
    ...
    contraction = 1.0 - 0.28 * np.tanh((t - 34.0) * 0.35)   # smiling L + Schlaufe zieht sich zusammen
    ...
    # harter Guard für Unity / Plateau
    if drift > drift_threshold and phi < 4 and t > 30.0:
        d_phi += 9.2

    d_c  = dc * contraction

    slow_start = 1.0 / (1.0 + np.exp(-0.55 * (t - 38.0)))   # extrem später Slow-Start
...
if __name__ == "__main__":
    print(f"🚀 NEXAH IEEE 57-Bus — LORENZ TUNABLE v8.1 (drift_threshold = {drift_threshold})")
    ...
    ax3d.set_title("Lorenz Core – Unity + smiling L + Hirtenstock + offener Durchfluss")
    ax3d.set_xlabel("c")
    ...
    ax2d.set_title("Collapse Prediction – IEEE 57-Bus (v8.1)")
    ...
    ax2d.legend()
    plt.tight_layout()
    plt.savefig(f"ieee57_lorenz_tunable_v8.1_threshold_{drift_threshold}.png", dpi=420, bbox_inches='tight')
    print(f"📸 Plot gespeichert als: ieee57_lorenz_tunable_v8.1_threshold_{drift_threshold}.png")
    plt.show()
