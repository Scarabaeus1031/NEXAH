"""
NEXAH IEEE 57-Bus — LORENZ TUNABLE v8.6
WINDING-NUMBER TRIGGER – schneller Spirale + realistischer Threshold
"""
...
winding_threshold = 1.8      # <-- jetzt realistisch für deine Z26-Z29 Dynamik
...
if __name__ == "__main__":
    print(f"🚀 NEXAH IEEE 57-Bus — LORENZ TUNABLE v8.6 (Winding-Number Trigger = {winding_threshold})")
    ...
    # === WINDING NUMBER BERECHNUNG (schneller Spirale) ===
    theta = t * 1.8          # <--- hier schneller (vorher 0.52)
    x_traj = c * np.cos(theta)
    y_traj = c * np.sin(theta)
    dtheta = np.diff(np.arctan2(y_traj, x_traj))
    winding_number = np.cumsum(dtheta) / (2 * np.pi)
    ...
    ax2d.set_title("Collapse Prediction – IEEE 57-Bus (v8.6 Winding)")
    ...
    ax2d.legend()
    plt.tight_layout()
    plt.savefig(f"ieee57_lorenz_tunable_v8.6_winding_{winding_threshold}.png", dpi=420, bbox_inches='tight')
    print(f"📸 Plot gespeichert als: ieee57_lorenz_tunable_v8.6_winding_{winding_threshold}.png")
    plt.show()
