"""
NEXAH IEEE 57-Bus — TUNABLE v6.1
drift_threshold = 6.0  (deutlich späterer Split)
"""
...
drift_threshold = 6.0        # <--- jetzt deutlich höher
...
if __name__ == "__main__":
    print(f"🚀 NEXAH IEEE 57-Bus — TUNABLE v6.1 (drift_threshold = {drift_threshold})")
    ...
    sol = solve_ivp(
        ...
        t_span=(0, 80),          # länger, damit die Kurve mehr Raum hat
        y0=x0,
        method='RK45',
        rtol=1e-6,
        max_step=0.012
    ...
    if np.any(phi_idx > 0):
        ...
        print("❌ Kein Phi-Split – Threshold immer noch zu niedrig")
    ...
    ax2d.legend()
    plt.tight_layout()
    plt.savefig(f"ieee57_tunable_v6.1_threshold_{drift_threshold}.png", dpi=420, bbox_inches='tight')
    print(f"📸 Plot gespeichert als: ieee57_tunable_v6.1_threshold_{drift_threshold}.png")
    plt.show()
