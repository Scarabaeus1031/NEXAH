"""
NEXAH IEEE 57-Bus — LORENZ TUNABLE v8.5
WINDING-NUMBER TRIGGER (Z26-Z29 + Drift) statt Drift-Threshold
"""
...
winding_threshold = 4.8      # <-- hier kannst du 4.0 / 5.0 / 5.5 etc. testen (passt zu Z26-Z29)
...
if __name__ == "__main__":
    print(f"🚀 NEXAH IEEE 57-Bus — LORENZ TUNABLE v8.5 (Winding-Number Trigger = {winding_threshold})")
    ...
    # === WINDING NUMBER BERECHNUNG ===
    theta = t * 0.52
    x_traj = c * np.cos(theta)
    y_traj = c * np.sin(theta)
    dtheta = np.diff(np.arctan2(y_traj, x_traj))
    winding_number = np.cumsum(dtheta) / (2 * np.pi)   # kumulativer Winding
    ...
    # Phi-Split durch Winding Number (statt Drift)
    ...
    ax3d.set_title("Lorenz Core – Winding-Number Trigger (Z26-Z29)")
    ax3d.set_xlabel("c")
    ...
    ax2d.set_title("Collapse Prediction – IEEE 57-Bus (v8.5 Winding)")
    ...
    ax2d.legend()
    plt.tight_layout()
    plt.savefig(f"ieee57_lorenz_tunable_v8.5_winding_{winding_threshold}.png", dpi=420, bbox_inches='tight')
    print(f"📸 Plot gespeichert als: ieee57_lorenz_tunable_v8.5_winding_{winding_threshold}.png")
    plt.show()
