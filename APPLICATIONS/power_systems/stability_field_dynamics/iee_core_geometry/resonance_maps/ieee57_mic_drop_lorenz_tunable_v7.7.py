"""
NEXAH IEEE 57-Bus — LORENZ TUNABLE v7.7
Thoth’s Vogel + Bezel (X im Kreis) + Schlaufe zieht sich zusammen
"""
...
def nexah_lorenz_ode(t, x, params):
    ...
    # === STÄRKERE SCHLAUFEN-KONTRAKTION (Thoth’s Vogel) ===
    ring_offset = 1.55 * np.sin(angle + 2.1) + 1.15 * np.sin(angle - 2.1) if phi >= 2 else 0.0
    contraction = 1.0 - 0.18 * np.tanh((t - 25.0) * 0.25)   # Schlaufe zieht sich ab t≈25 zusammen

    branch_angle = angle % (2 * np.pi)
    branch_pulse = 2.8 * np.sin(13 * branch_angle) if phi == 2 and ((1.72 < branch_angle < 2.38) or (4.82 < branch_angle < 5.48)) else 0.0

    resonance = np.sin(phi * np.pi * np.sqrt(2)) * 5.8
    ...
    d_c  = dc * contraction                     # Kontraktion wirkt auch auf c

    slow_start = 1.0 / (1.0 + np.exp(-0.42 * (t - 28.0)))

    d_dc = (field_force + p_drive + kuramoto + vdp_force + compass_op + ring_offset + branch_pulse) * inversion * contraction
    d_dc += 0.195 * t * 1.35 * slow_start
...
if __name__ == "__main__":
    print(f"🚀 NEXAH IEEE 57-Bus — LORENZ TUNABLE v7.7 (drift_threshold = {drift_threshold})")
    ...
    ax3d.set_title("Lorenz Core – Thoth’s Vogel + Bezel + Schlaufe zieht sich zusammen")
    ax3d.set_xlabel("c")
    ...
    ax2d.set_title("Collapse Prediction – IEEE 57-Bus (v7.7)")
    ...
    ax2d.legend()
    plt.tight_layout()
    plt.savefig(f"ieee57_lorenz_tunable_v7.7_threshold_{drift_threshold}.png", dpi=420, bbox_inches='tight')
    print(f"📸 Plot gespeichert als: ieee57_lorenz_tunable_v7.7_threshold_{drift_threshold}.png")
    plt.show()
