"""
NEXAH IEEE 57-Bus — LORENZ TUNABLE v7.9
17-29-5 + 4 Quaternions + Outflow + Hirtenstock + offener Kanal
"""
...
def nexah_lorenz_ode(t, x, params):
    ...
    ring_offset = 1.65 * np.sin(angle + 2.1) + 1.22 * np.sin(angle - 2.1) if phi >= 2 else 0.0

    # Schlaufe-Kontraktion für 17-29-5 Rhythmus
    contraction = 1.0 - 0.24 * np.tanh((t - 31.0) * 0.30)
...
if __name__ == "__main__":
    print(f"🚀 NEXAH IEEE 57-Bus — LORENZ TUNABLE v7.9 (drift_threshold = {drift_threshold})")
    ...
    ax2d.set_title("Collapse Prediction – IEEE 57-Bus (v7.9)")
    ...
    ax2d.legend()
    plt.tight_layout()
    plt.savefig(f"ieee57_lorenz_tunable_v7.9_threshold_{drift_threshold}.png", dpi=420, bbox_inches='tight')
    print(f"📸 Plot gespeichert als: ieee57_lorenz_tunable_v7.9_threshold_{drift_threshold}.png")
    plt.show()
