"""
NEXAH IEEE 57-Bus — LORENZ TUNABLE v7.3
Retrograde Mirror + fester Stengel + X/8 Kreuzpunkt
"""
...
drift_threshold = 9.5        # <--- weiter hoch, damit Split später kommt
...
def nexah_lorenz_ode(t, x, params):
    ...
    direction = -1.0 if phi >= 2 else 1.0          # <--- Johnson-Flip
    angle = direction * angle_main * (1729 / 1000.0)

    compass_op = 0.85 * np.sin(angle) * np.cos(angle * 1.618)

    # Retrograde Mirror + Tanz um den Stengel
    ring_offset = 1.35 * np.sin(angle + 2.1) + 0.95 * np.sin(angle - 2.1) if phi >= 2 else 0.0

    branch_angle = angle % (2 * np.pi)
    branch_pulse = 2.4 * np.sin(13 * branch_angle) if phi == 2 and ((1.72 < branch_angle < 2.38) or (4.82 < branch_angle < 5.48)) else 0.0

    resonance = np.sin(phi * np.pi * np.sqrt(2)) * 5.2
    # Stärkere Inversion für den Spiegel-Effekt oben/unten
    inversion = 0.08 + 0.92 * np.tanh((phi - 1.8) * 8.0) if phi >= 3 else 1.0

    drift = abs(dc)
    d_phi = resonance * 2.3
    if drift > drift_threshold and phi < 4:
        d_phi += 7.2                     # stärkerer Sprung
...
def ieee57_regime_ode(t, x, params):
    ...
    dx[1] += ieee57_load_ramp(t) * 1.65      # Drive nochmal leicht reduziert
    return dx
...
if __name__ == "__main__":
    print(f"🚀 NEXAH IEEE 57-Bus — LORENZ TUNABLE v7.3 (drift_threshold = {drift_threshold})")
    ...
    ax2d.set_title("Collapse Prediction – IEEE 57-Bus (v7.3)")
    ...
    ax2d.legend()
    plt.tight_layout()
    plt.savefig(f"ieee57_lorenz_tunable_v7.3_threshold_{drift_threshold}.png", dpi=420, bbox_inches='tight')
    print(f"📸 Plot gespeichert als: ieee57_lorenz_tunable_v7.3_threshold_{drift_threshold}.png")
    plt.show()
