"""
NEXAH IEEE 57-Bus — LORENZ TUNABLE v7.8
Hirtenstock + offener Kanal + H in der Mitte + J-Spiegel + Plateau
"""
...
drift_threshold = 15.8       # fein abgestimmt für dein aktuelles Plateau
...
def nexah_lorenz_ode(t, x, params):
    ...
    # Hirtenstock + offener Kanal + H in der Mitte
    ring_offset = 1.62 * np.sin(angle + 2.1) + 1.18 * np.sin(angle - 2.1) if phi >= 2 else 0.0
    contraction = 1.0 - 0.22 * np.tanh((t - 29.0) * 0.28)   # Schlaufe zieht sich noch schöner zusammen

    branch_angle = angle % (2 * np.pi)
    branch_pulse = 2.9 * np.sin(13 * branch_angle) if phi == 2 and ((1.70 < branch_angle < 2.40) or (4.80 < branch_angle < 5.50)) else 0.0

    resonance = np.sin(phi * np.pi * np.sqrt(2)) * 5.9
...
if __name__ == "__main__":
    print(f"🚀 NEXAH IEEE 57-Bus — LORENZ TUNABLE v7.8 (drift_threshold = {drift_threshold})")
    ...
    ax3d.set_title("Lorenz Core – Hirtenstock + offener Kanal + H + J-Spiegel")
    ax3d.set_xlabel("c")
    ...
    ax2d.set_title("Collapse Prediction – IEEE 57-Bus (v7.8)")
    ...
    ax2d.legend()
    plt.tight_layout()
    plt.savefig(f"ieee57_lorenz_tunable_v7.8_threshold_{drift_threshold}.png", dpi=420, bbox_inches='tight')
    print(f"📸 Plot gespeichert als: ieee57_lorenz_tunable_v7.8_threshold_{drift_threshold}.png")
    plt.show()
