"""
NEXAH IEEE 57-Bus — LORENZ TUNABLE v7.2
Reduzierter Drive + höherer Threshold → Split später auf der Kurve
"""
...
drift_threshold = 12.0       # <--- höher, damit Split später kommt
...
def ieee57_regime_ode(t, x, params):
    ...
    dx[1] += ieee57_load_ramp(t) * 1.8      # <--- Drive etwas reduziert
    return dx
...
if __name__ == "__main__":
    print(f"🚀 NEXAH IEEE 57-Bus — LORENZ TUNABLE v7.2 (drift_threshold = {drift_threshold})")
    ...
    ax2d.legend()
    plt.tight_layout()
    plt.savefig(f"ieee57_lorenz_tunable_v7.2_threshold_{drift_threshold}.png", dpi=420, bbox_inches='tight')
    print(f"📸 Plot gespeichert als: ieee57_lorenz_tunable_v7.2_threshold_{drift_threshold}.png")
    plt.show()
