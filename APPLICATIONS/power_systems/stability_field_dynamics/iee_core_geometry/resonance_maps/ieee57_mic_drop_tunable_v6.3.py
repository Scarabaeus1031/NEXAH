"""
NEXAH IEEE 57-Bus — TUNABLE v6.3
Stärkerer Drive + tunable Threshold
"""
...
drift_threshold = 3.2        # <--- Starte hier (kannst du später auf 3.5, 3.8, 4.0 etc. erhöhen)
...
def ieee57_regime_ode(t, x, params):
    ...
    dx[1] += ieee57_load_ramp(t) * 3.0      # <--- stärkerer Drive
    return dx
...
if __name__ == "__main__":
    print(f"🚀 NEXAH IEEE 57-Bus — TUNABLE v6.3 (drift_threshold = {drift_threshold})")
    ...
    print(f"Maximaler Drift (abs(dc)) = {abs(dc).max():.3f}")
    ...
    ax2d.legend()
    plt.tight_layout()
    plt.savefig(f"ieee57_tunable_v6.3_threshold_{drift_threshold}.png", dpi=420, bbox_inches='tight')
    print(f"📸 Plot gespeichert als: ieee57_tunable_v6.3_threshold_{drift_threshold}.png")
    plt.show()
