"""
NEXAH IEEE 118-Bus — STATISTIK v5
...
if __name__ == "__main__":
    print("🚀 NEXAH IEEE 118-Bus — STATISTIK v5 (20 Runs)")
    ...
    for run in range(N_RUNS):
        # kleines Rauschen auf Startbedingungen
        ...
        if np.any(phi_idx > 0):
            ...
            lead_times.append(np.nan)  # kein Split
    ...
    if np.any(valid):
        ...
        plt.title("Lead-Time Histogram – NEXAH Phi-Split (20 Runs)")
        plt.xlabel("Phi-Split Zeitpunkt t [s]")
        plt.ylabel("Anzahl Runs")
        plt.grid(True, alpha=0.4)
        plt.legend()
        plt.savefig("ieee118_lead_time_histogram_v5.png", dpi=420, bbox_inches='tight')
        print("📸 Histogramm gespeichert als: ieee118_lead_time_histogram_v5.png")
        plt.show()
    else:
