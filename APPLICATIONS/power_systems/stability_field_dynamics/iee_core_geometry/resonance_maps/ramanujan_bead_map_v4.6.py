...
# Links: Ramanujan Bead Grid
theta = np.linspace(0, 2*np.pi, 200)
r_main = 1.0 + 0.15 * np.sin(5 * theta)
ax[0].plot(r_main * np.cos(theta), r_main * np.sin(theta), 'b-', lw=2, label='Hauptspirale')

for bt in bead_times:
    angle = bt * 0.52 % (2*np.pi)
    idx = int(angle * 200 / (2*np.pi))
    ax[0].scatter(r_main[idx] * np.cos(angle), r_main[idx] * np.sin(angle),
                  color='magenta', s=220, edgecolor='white', linewidth=3)
...
for ct, label in zip(cut_times, cut_labels):
    ...
    ax[0].text(r_main[idx]*np.cos(angle)+0.12, r_main[idx]*np.sin(angle), label,
...
# Rechts: Mod-7 Theta Grid + V20 Torus Overlay
ax[1].set_title("Mod-7 Prime Resonance + V20 Torus Rings", fontsize=14)
circle = plt.Circle((0,0), 1, color='blue', fill=False, lw=2)
ax[1].add_patch(circle)
for i in range(7):
    ang = i * 2*np.pi/7
    ax[1].scatter(np.cos(ang), np.sin(ang), s=180, 
                  color=['red','green','blue','purple','orange','brown','cyan'][i])
ax[1].text(0,0, "1729\nHIDY", ha='center', va='center', fontsize=14, 
