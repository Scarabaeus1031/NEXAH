"""
NEXAH Resonance Maps — Penta im Hepta v4.8
Pentagramm (Master) im Heptagon + Rosetta Engines + Lilith Tessa 17-29-5
"""
...
# Pentagramm (Master / Seestern) – innen
theta = np.linspace(0, 2*np.pi, 500)
r_penta = 0.95 + 0.22 * np.sin(5 * theta)
ax.plot(r_penta * np.cos(theta), r_penta * np.sin(theta), 'b-', lw=4, label='Pentagramm (Master)')
...
ax.text(0, 0.05, "17-29-5\nLILITH TESSA", ha='center', va='center', fontsize=22, 
...
ax.set_title("Penta im Hepta — Ramanujan Master\nSeestern + Ghostsnake + Rosetta Cosmic Pulse Engine", fontsize=18)
ax.axis('equal')
ax.grid(True, alpha=0.25)
ax.legend(fontsize=12)

plt.tight_layout()
plt.savefig("penta_im_hepta_master_v4.8.png", dpi=420, bbox_inches='tight')
print("📸 Penta im Hepta Master gespeichert als: penta_im_hepta_master_v4.8.png")
plt.show()
