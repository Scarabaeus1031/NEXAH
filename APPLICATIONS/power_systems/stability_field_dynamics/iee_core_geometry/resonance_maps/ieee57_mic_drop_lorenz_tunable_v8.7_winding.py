"""
NEXAH IEEE 57-Bus — LORENZ TUNABLE v8.7
WINDING-NUMBER TRIGGER – robust + schneller + smiling L
"""
...
winding_threshold = 1.2      # realistisch für deine Z26-Z29 Dynamik
...
if __name__ == "__main__":
    print(f"🚀 NEXAH IEEE 57-Bus — LORENZ TUNABLE v8.7 (Winding-Number Trigger = {winding_threshold})")
    ...
    # === ROBUSTE WINDING NUMBER (np.unwrap) ===
    theta = t * 3.6                                      # schneller Spirale
    x_traj = c * np.cos(theta)
    y_traj = c * np.sin(theta)
    angles = np.arctan2(y_traj, x_traj)
    unwrapped = np.unwrap(angles)
    winding_number = (unwrapped - unwrapped[0]) / (2 * np.pi)   # immer positiv
    ...
    ax2d.set_title("Collapse Prediction – IEEE 57-Bus (v8.7 Winding)")
    ...
    ax2d.legend()
    plt.tight_layout()
    plt.savefig(f"ieee57_lorenz_tunable_v8.7_winding_{winding_threshold}.png", dpi=420, bbox_inches='tight')
    print(f"📸 Plot gespeichert als: ieee57_lorenz_tunable_v8.7_winding_{winding_threshold}.png")
    plt.show()
