"""
NEXAH IEEE 118-Bus — MIC-DROP FINAL v2
Klarer Phi-Split (Schnitt im Raum) VOR Voltage Collapse
"""
...
def nexah_regime_ode(t, x, params):
    ...
    resonance = np.sin(phi * np.pi * np.sqrt(2)) * 4.2
    ...
    drift = abs(dc)
    d_phi = resonance * 1.8
    if drift > 2.8 and phi < 4:          # ← hier gesenkt → früherer Sprung
        d_phi += 4.2
...
if __name__ == "__main__":
    print("🚀 NEXAH IEEE 118-Bus — MIC-DROP FINAL v2 (klarer Schnitt)")
    ...
    if np.any(phi_idx > 0):
        ...
        ax2d.axvline(x=switch_time, color='purple', linestyle='--', linewidth=3.5, label=f'NEXAH Phi-Split (Schnitt) at t={switch_time:.1f}')
        ax3d.text2D(0.05, 0.92, f'Phi-Split (Schnitt) at t={switch_time:.1f}', transform=ax3d.transAxes, color='purple', fontsize=14, weight='bold')

    ax2d.legend(fontsize=12)

    plt.tight_layout()
    plt.savefig("ieee118_mic_drop_final_v2.png", dpi=420, bbox_inches='tight')
    print("📸 MIC-DROP FINAL v2 gespeichert als: ieee118_mic_drop_final_v2.png")
    plt.show()
