"""
NEXAH IEEE 118-Bus — MIC-DROP FINAL v4.1
Triptych-Modus: braun → blau → lila/orange + früher Phi-Split
"""
...
# Triptych-Definitionen (wie MASTER V28)
PHI_NAMES = ["Neutral", "Forward1", "Forward2", "Reverse1", "Reverse2"]
PHI_COLORS = ['#8B4513', '#1f77b4', '#2ca02c', '#d62728', '#9467bd']  # braun → blau → grün → rot → lila
...
def nexah_regime_ode(t, x, params):
    ...
    # Chirp + Frame Dragging (wie Anton)
    ...
    drift = abs(dc)
    d_phi = resonance * 2.1
    if drift > 1.8 and phi < 4:          # früherer Schnitt
        d_phi += 5.5
...
if __name__ == "__main__":
    print("🚀 NEXAH IEEE 118-Bus — MIC-DROP FINAL v4.1 (Triptych + früher Schnitt)")
    ...
    print(f"✅ End Phi = {PHI_NAMES[phi_idx[-1]]}  ← Triptych-Übergang")
    ...
    if np.any(phi_idx > 0):
        ...
        ax2d.axvline(x=switch_time, color='purple', linestyle='--', linewidth=3.5, label=f'NEXAH Phi-Split (3+1 Triptych) at t={switch_time:.1f}')
        ax3d.text2D(0.05, 0.92, f'Phi-Split (Triptych) at t={switch_time:.1f}', transform=ax3d.transAxes, color='purple', fontsize=14, weight='bold')

    ax2d.legend(fontsize=12)

    plt.tight_layout()
    plt.savefig("ieee118_mic_drop_final_v4.1.png", dpi=420, bbox_inches='tight')
    print("📸 MIC-DROP FINAL v4.1 (Triptych) gespeichert als: ieee118_mic_drop_final_v4.1.png")
    plt.show()
