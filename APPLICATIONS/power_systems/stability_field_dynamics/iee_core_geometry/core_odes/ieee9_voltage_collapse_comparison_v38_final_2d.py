"""
NEXAH 2D FINAL — v3.8
Maximaler Peitschen-Impuls + 9 Grapes + atmendes Grid + Selbstähnlichkeit
"""
...
def nexah_regime_ode(t, x, params):
    ...
    vdp_force = 0.92 * dc * (1.0 - c**2)

    angle = 2 * np.pi * t / 3.2 + phi * 1.35
    compass_op = 0.82 * np.sin(angle) * np.cos(angle * 1.618)

    resonance = np.sin(phi * np.pi * np.sqrt(2)) * 1.6
    pulse = 1.35 * np.sin(2 * np.pi * t * 6.5) * np.exp(-0.005 * t)   # stärkerer diskreter Impuls

    inversion = 1.0
    if phi >= 3:
        inversion = 0.15 + 0.85 * np.tanh((phi - 1.85) * 5.8)

    drift = abs(dc)
    d_phi = resonance * 1.5 + pulse * 1.1
...
if __name__ == "__main__":
    print("🚀 NEXAH 2D FINAL — v3.8 (9 Grapes + atmendes Grid)")
    ...
    print(f"✅ 2D FINAL fertig — End Phi = {PHI_NAMES[phi_idx[-1]]}")

    fig, axs = plt.subplots(2, 2, figsize=(14, 10))
    fig.suptitle("NEXAH 2D FINAL — v3.8 (9 Grapes + atmendes Grid)", fontsize=16)
    ...
    plt.tight_layout()
    plt.savefig("ieee9_nexah_vs_voltage_collapse_v38_final_2d.png", dpi=300, bbox_inches='tight')
    print("📸 2D FINAL v3.8 gespeichert")
    plt.show()
