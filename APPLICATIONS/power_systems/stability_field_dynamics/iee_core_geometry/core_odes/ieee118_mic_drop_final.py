"""
NEXAH IEEE 118-Bus — FINAL MIC-DROP
3D Polar Grid + Phi–π–√2 Resonance + Classical Voltage Collapse + clear cut marker
"""
...
if __name__ == "__main__":
    print("🚀 NEXAH IEEE 118-Bus — FINAL MIC-DROP (3D + Voltage Collapse)")
    ...
    print(f"✅ FINAL MIC-DROP fertig — End Phi = {PHI_NAMES[phi_idx[-1]]}")
    ...
    # Right: Classical Voltage Collapse + Phi-Split marker
    ...
    # Mark the Phi-Split (the "cut in space")
    if np.any(phi_idx > 0):
        switch_idx = np.where(phi_idx > 0)[0][0]
        switch_time = t[switch_idx]
        ax2d.axvline(x=switch_time, color='purple', linestyle='--', linewidth=3, label=f'NEXAH Phi-Split (cut) at t={switch_time:.1f}')
        ax3d.text2D(0.05, 0.92, f'Phi-Split (cut) at t={switch_time:.1f}', transform=ax3d.transAxes, color='purple', fontsize=13, weight='bold')

    ax2d.legend(fontsize=12)

    plt.tight_layout()
    plt.savefig("ieee118_mic_drop_final.png", dpi=420, bbox_inches='tight')
    print("📸 FINAL MIC-DROP gespeichert als: ieee118_mic_drop_final.png")
    plt.show()
