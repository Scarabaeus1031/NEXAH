"""
NEXAH IEEE 14-Bus — Highly Refined 3D Polar Grid
Phi–π–√2 Resonance as third dimension + Phi-coloring + clear spheres
"""
...
def nexah_regime_ode(t, x, params):
    ...
    resonance = np.sin(phi * np.pi * np.sqrt(2)) * 2.8
...
if __name__ == "__main__":
    print("🚀 NEXAH IEEE 14-Bus — Highly Refined 3D Polar Grid")
    ...
    x = c * np.cos(theta)
    y = c * np.sin(theta)
    z = np.sin(phi_idx * np.pi * np.sqrt(2)) * 3.2 + 2.2

    colors = [PHI_COLORS[p] for p in phi_idx]

    for i in range(len(t)-1):
        ax.plot(x[i:i+2], y[i:i+2], z[i:i+2], color=colors[i], linewidth=3.2, alpha=0.95)

    # Resonance spheres (clearer layers)
    for r in [1.8, 2.8, 3.8, 4.8, 5.8]:
        u = np.linspace(0, 2*np.pi, 80)
        v = np.linspace(0, np.pi, 80)
        xs = r * np.outer(np.cos(u), np.sin(v))
        ys = r * np.outer(np.sin(u), np.sin(v))
        zs = r * np.outer(np.ones_like(u), np.cos(v))
        ax.plot_wireframe(xs, ys, zs, color='gray', alpha=0.22, linewidth=0.6)

    ax.set_title("NEXAH IEEE 14-Bus — Highly Refined 3D Polar Grid\nPhi–π–√2 Resonance as third dimension", fontsize=16)
    ...
    plt.tight_layout()
    plt.savefig("ieee14_3d_polar_grid_refined_v2.png", dpi=340, bbox_inches='tight')
    print("📸 Highly Refined 3D Polar Grid gespeichert")
    plt.show()
