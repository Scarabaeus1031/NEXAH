...
def nexah_regime_ode(t, x, params):
    ...
    vdp_force = 0.75 * dc * (1.0 - c**2)

    angle = 2 * np.pi * t / 3.8 + phi * 1.1
    compass_op = 0.62 * np.sin(angle) * np.cos(angle * 1.618)

    inversion = 1.0
    if phi >= 3:
        inversion = 0.32 + 0.68 * np.tanh((phi - 2.1) * 4.0)
...
def classical_voltage(load_factor):
    return 1.0 / (1.0 + 0.82 * load_factor**2)

if __name__ == "__main__":
    print("🚀 NEXAH vs. Classical Voltage Collapse — v3.3 (max Drive)")

    params = {'Q': 1.48, 'use_vdp': True}
    x0 = [0.05, 0.0, 0]
    ...
    fig.suptitle("NEXAH vs. Classical Voltage Collapse — v3.3 (max Drive)", fontsize=16)
    ...
    plt.tight_layout()
    plt.savefig("ieee9_nexah_vs_voltage_collapse_v33.png", dpi=240, bbox_inches='tight')
    print("📸 Vergleich v3.3 gespeichert")
    plt.show()
