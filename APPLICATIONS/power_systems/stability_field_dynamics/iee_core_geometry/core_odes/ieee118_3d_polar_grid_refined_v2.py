"""
NEXAH IEEE 118-Bus — Highly Refined 3D Polar Grid v2
Phi–π–√2 Resonance as third dimension + clearer shells + Phi-coloring
"""
...
def nexah_regime_ode(t, x, params):
    ...
    resonance = np.sin(phi * np.pi * np.sqrt(2)) * 3.5
...
if __name__ == "__main__":
    print("🚀 NEXAH IEEE 118-Bus — Highly Refined 3D Polar Grid v2")
    ...
    x = c * np.cos(theta)
    y = c * np.sin(theta)
    z = np.sin(phi_idx * np.pi * np.sqrt(2)) * 4.2 + 2.8

    colors = [PHI_COLORS[p] for p in phi_idx]

    for i in range(len(t)-1):
        ax.plot(x[i:i+2], y[i:i+2], z[i:i+2], color=colors[i], linewidth=3.8, alpha=0.95)

    # Clearer resonance spheres
    for r in [2.2, 3.5, 4.8, 6.1, 7.4, 8.7]:
        ...
        ax.plot_wireframe(xs, ys, zs, color='gray', alpha=0.20, linewidth=0.6)

    ax.set_title("NEXAH IEEE 118-Bus — Highly Refined 3D Polar Grid v2\nPhi–π–√2 Resonance as third dimension", fontsize=16)
    ...
    plt.tight_layout()
    plt.savefig("ieee118_3d_polar_grid_refined_v2.png", dpi=380, bbox_inches='tight')
    print("📸 Highly Refined 3D Polar Grid v2 gespeichert")
    plt.show()
