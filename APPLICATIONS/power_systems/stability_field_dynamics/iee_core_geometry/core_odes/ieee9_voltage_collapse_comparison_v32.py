"""
NEXAH vs. Classical Voltage Collapse — Version 3.2
Stärkster Drive + aggressiver Phase-Error-Lock
"""
...
def nexah_regime_ode(t, x, params):
    ...
    vdp_force = 0.72 * dc * (1.0 - c**2)

    angle = 2 * np.pi * t / 4.0 + phi * 1.05
    compass_op = 0.58 * np.sin(angle) * np.cos(angle * 1.618)

    inversion = 1.0
    if phi >= 3:
        inversion = 0.35 + 0.65 * np.tanh((phi - 2.2) * 3.8)
...
def classical_voltage(load_factor):
    return 1.0 / (1.0 + 0.75 * load_factor**2)

if __name__ == "__main__":
    print("🚀 NEXAH vs. Classical Voltage Collapse — v3.2 (stärkster Drive)")

    params = {'Q': 1.45, 'use_vdp': True}
    x0 = [0.05, 0.0, 0]
    ...
    fig.suptitle("NEXAH vs. Classical Voltage Collapse — v3.2 (stärkster Drive)", fontsize=16)
    ...
    if np.any(phi_idx > 0):
        ...
        for ax in axs.flat:
            ax.axvline(x=switch_time, color='purple', linestyle='--', alpha=0.9, linewidth=2, label=f'Phi-Split bei t={switch_time:.1f}')

    plt.tight_layout()
    plt.savefig("ieee9_nexah_vs_voltage_collapse_v32.png", dpi=240, bbox_inches='tight')
    print("📸 Vergleich v3.2 gespeichert")
    plt.show()
