"""
NEXAH IEEE 118-Bus — Final Refined 3D Polar Grid
Phi–π–√2 Resonance as third dimension + clear multi-shell structure
"""
...
if __name__ == "__main__":
    print("🚀 NEXAH IEEE 118-Bus — Final Refined 3D Polar Grid")
    ...
    # Clear multi-shell resonance spheres
    for r in [2.0, 3.2, 4.4, 5.6, 6.8, 8.0]:
        u = np.linspace(0, 2*np.pi, 100)
        v = np.linspace(0, np.pi, 100)
        xs = r * np.outer(np.cos(u), np.sin(v))
        ys = r * np.outer(np.sin(u), np.sin(v))
        zs = r * np.outer(np.ones_like(u), np.cos(v))
        ax.plot_wireframe(xs, ys, zs, color='gray', alpha=0.18, linewidth=0.55)

    ax.set_title("NEXAH IEEE 118-Bus — Final Refined 3D Polar Grid\nPhi–π–√2 Resonance as third dimension", fontsize=16)
    ...
    plt.tight_layout()
    plt.savefig("ieee118_3d_polar_grid_final.png", dpi=380, bbox_inches='tight')
    print("📸 Final Refined 3D Polar Grid gespeichert")
    plt.show()
