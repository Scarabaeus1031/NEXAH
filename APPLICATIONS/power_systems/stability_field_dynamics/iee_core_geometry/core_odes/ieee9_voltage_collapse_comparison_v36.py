"""
NEXAH vs. Classical Voltage Collapse — Version 3.6
Maximaler Dolphin-Flip + Phi–π–√2 Resonance (Ziel: Phi-Sprung)
"""
...
def nexah_regime_ode(t, x, params):
    ...
    # Dolphin-Flip Compass (stark)
    angle = 2 * np.pi * t / 3.5 + phi * 1.2
    compass_op = 0.72 * np.sin(angle) * np.cos(angle * 1.618)

    # Phi–π–√2 Resonance (Core Geometry – verstärkt)
    resonance = np.sin(phi * np.pi * np.sqrt(2)) * 1.1
...
def classical_voltage(load_factor):
    return 1.0 / (1.0 + 0.95 * load_factor**2)

if __name__ == "__main__":
    print("🚀 NEXAH vs. Classical Voltage Collapse — v3.6 (max Dolphin-Flip)")
    ...
    fig.suptitle("NEXAH vs. Classical Voltage Collapse — v3.6 (max Dolphin-Flip)", fontsize=16)
    ...
    plt.tight_layout()
    plt.savefig("ieee9_nexah_vs_voltage_collapse_v36.png", dpi=240, bbox_inches='tight')
    print("📸 v3.6 gespeichert")
    plt.show()
