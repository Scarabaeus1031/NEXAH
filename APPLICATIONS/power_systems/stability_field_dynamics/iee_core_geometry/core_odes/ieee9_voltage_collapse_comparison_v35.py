"""
NEXAH vs. Classical Voltage Collapse — v3.5
Dolphin-Flip + Phi–π–√2 Resonance (Core Geometry)
"""
...
def nexah_regime_ode(t, x, params):
    ...
    # NEU: Phi–π–√2 Resonance (Core Geometry)
    resonance = np.sin(phi * np.pi * np.sqrt(2)) * 0.85

    inversion = 1.0
    if phi >= 3:
        inversion = 0.28 + 0.72 * np.tanh((phi - 2.0) * 4.5)

    # Flip-Trigger + Resonance
    drift = abs(dc)
    d_phi = resonance * 0.6
    if drift > 2.9 and phi < 4:
        d_phi += 1.4
...
def classical_voltage(load_factor):
    return 1.0 / (1.0 + 0.9 * load_factor**2)

if __name__ == "__main__":
    print("🚀 NEXAH vs. Classical Voltage Collapse — v3.5 Dolphin-Flip + Phi–π–√2")
    ...
    fig.suptitle("NEXAH vs. Classical Voltage Collapse — v3.5 (Phi–π–√2 Resonance)", fontsize=16)
    ...
    plt.tight_layout()
    plt.savefig("ieee9_nexah_vs_voltage_collapse_v35.png", dpi=240, bbox_inches='tight')
    print("📸 v3.5 mit Phi–π–√2 Resonance gespeichert")
    plt.show()
