"""
NEXAH IEEE 118-Bus — MIC-DROP FINAL v4.2
Stengel + Apfel + Birne + Gelb anschalten
"""
...
if __name__ == "__main__":
    print("🚀 NEXAH IEEE 118-Bus — MIC-DROP FINAL v4.2 (Stengel + Birne + Gelb)")
    ...
    print(f"✅ End Phi = {PHI_NAMES[phi_idx[-1]]}  ← Triptych komplett")
    ...
    # Birnen-Deformation (weiche Ausbuchtung)
    ...
    if np.any(phi_idx > 0):
        ...
        ax2d.axvline(x=switch_time, color='purple', linestyle='--', linewidth=3.5, label=f'NEXAH Phi-Split (Stengel) at t={switch_time:.1f}')
        ax3d.text2D(0.05, 0.92, f'Phi-Split (Stengel + Birne) at t={switch_time:.1f}', transform=ax3d.transAxes, color='purple', fontsize=14, weight='bold')

    ax2d.legend(fontsize=12)

    plt.tight_layout()
    plt.savefig("ieee118_mic_drop_final_v4.2.png", dpi=420, bbox_inches='tight')
    print("📸 MIC-DROP FINAL v4.2 (Stengel + Birne + Gelb) gespeichert als: ieee118_mic_drop_final_v4.2.png")
    plt.show()
