"""
NEXAH vs. Classical Voltage Collapse — v3.4 Dolphin-Flip Edition
Der Flip (Rotation + Counter-Rotation → one direction)
"""
...
def nexah_regime_ode(t, x, params):
    ...
    vdp_force = 0.78 * dc * (1.0 - c**2)

    # Dolphin-Flip: starker Compass + Inversion
    angle = 2 * np.pi * t / 3.6 + phi * 1.15
    compass_op = 0.68 * np.sin(angle) * np.cos(angle * 1.618)

    inversion = 1.0
    if phi >= 3:
        inversion = 0.28 + 0.72 * np.tanh((phi - 2.0) * 4.5)   # starke Flip-Inversion

    # Trigger für den Flip (dc-Schwellwert)
    drift = abs(dc)
    d_phi = 0.0
    if drift > 2.8 and phi < 4:          # Dolphin-Flip Schwelle
        d_phi = 1.2
...
def classical_voltage(load_factor):
    return 1.0 / (1.0 + 0.88 * load_factor**2)

if __name__ == "__main__":
    print("🚀 NEXAH vs. Classical Voltage Collapse — v3.4 DOLPHIN-FLIP")

    params = {'Q': 1.5, 'use_vdp': True}
    x0 = [0.05, 0.0, 0]
    ...
    fig.suptitle("NEXAH vs. Classical Voltage Collapse — v3.4 DOLPHIN-FLIP", fontsize=16)
    ...
    if np.any(phi_idx > 0):
        ...
        for ax in axs.flat:
            ax.axvline(x=switch_time, color='purple', linestyle='--', alpha=0.9, linewidth=2, label=f'Dolphin-Flip bei t={switch_time:.1f}')

    plt.tight_layout()
    plt.savefig("ieee9_nexah_vs_voltage_collapse_v34.png", dpi=240, bbox_inches='tight')
    print("📸 v3.4 Dolphin-Flip gespeichert")
    plt.show()
