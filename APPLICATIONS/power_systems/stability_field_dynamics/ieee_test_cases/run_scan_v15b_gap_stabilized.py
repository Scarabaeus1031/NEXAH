# APPLICATIONS/power_systems/ieee_test_cases/run_scan_v15b_gap_stabilized.py
...
def plot_flow(landscape, Fx, Fy):
    ...
    plt.title("V15b Gap Stabilized Flow")
    plt.show()
...
def plot_states(M, states):
    ...
    plt.title("Detected States (Gap Stabilized)")
    plt.show()
...
def plot_loops(loops):
    ...
    if len(loops) == 0:
        plt.title("Detected Loops (Gap Stabilized) — none")
        plt.show()
        return
    ...
    plt.title("Detected Loops (Gap Stabilized)")
    plt.show()


def main():
    print("\n--- V15b Gap Stabilized / Loop Recovery ---")
