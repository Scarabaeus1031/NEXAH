...
# V13.1 PHASE ERROR LOCK + TARGET BIAS
...
k_speed = 1.5
k_turn = 0.9
k_lock = 0.55     # reference lock
k_target = 0.25   # NEW: target bias
k_relax = 0.08
...
for t in range(len(controlled)):
    ...
    if t > 1:
        ...
        # ------------------------------------------
        # TARGET PHASE (simple regime-based mapping)
        # ------------------------------------------
        y = controlled[t - 1, 1]

        if y > base_layer + 0.3 * spread:
            target_phi[t] = 0.35 * np.pi      # upper regime
        elif y < base_layer - 0.3 * spread:
            target_phi[t] = 1.35 * np.pi      # lower regime
        else:
            target_phi[t] = np.pi             # base regime
...
def plot_result(original, controlled, rift, phi, ref_phi, target_phi,
drive, phase_error_ref, phase_error_target,
...
plt.savefig(os.path.join(RIFT_DIR, "phase_error_lock_v13_1.png"), dpi=150)
plt.close()
...
plt.plot(drive, label="drive", color="black", alpha=0.7)
plt.legend()
plt.grid(True)
plt.title("Phase + Target (v13.1)")

plt.savefig(os.path.join(RIFT_DIR, "phase_error_lock_v13_1_phase.png"), dpi=150)
plt.close()
...
plt.savefig(os.path.join(RIFT_DIR, "phase_error_lock_v13_1_error.png"), dpi=150)
plt.close()
```
