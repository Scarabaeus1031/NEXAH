# rift_phase_target_controller_v14_1.py
...
def compute_target_phase_and_regime(y, base_layer, upper_layer, lower_layer):
    mid_upper = 0.5 * (base_layer + upper_layer)
    mid_lower = 0.5 * (base_layer + lower_layer)

    if y >= mid_upper:
        return 0.35 * np.pi, 1   # upper
    elif y <= mid_lower:
        return 1.35 * np.pi, -1  # lower
    else:
        return np.pi, 0          # base
...
# --------------------------------------------------
# V14.1 CONTROLLER + ANALYSIS SIGNALS
# --------------------------------------------------

def phase_target_controller_v14_1(trajectory, rift):
    ...
    for t in range(n):
        ...
        if t > 1:
            ...
            phi_target, ridx = compute_target_phase_and_regime(
                current_prev[1], base_layer, upper_layer, lower_layer
            )
...
def plot_regime_colored_trajectory(original, controlled, regime_idx,
    ...
    plt.plot(original[:, 0], original[:, 1], color="lightgray", linewidth=1.2, label="original")
    ...
    plt.title("V14.1 Regime-Colored Trajectory")
    plt.grid(True)
    plt.legend(loc="best")

    path = os.path.join(RIFT_DIR, "v14_1_regime_trajectory.png")
...
def plot_phase_analysis(phi, ref_phi, target_phi, drive, dphi_series, switch_flag):
    ...
    for t in switch_times:
        axes[0].axvline(t, color="red", linestyle="--", alpha=0.4)

    axes[0].set_title("V14.1 Phase / Reference / Target / Drive")
    axes[0].grid(True)
    axes[0].legend(loc="best")

    axes[1].plot(dphi_series, color="darkred", label="dphi / dt")
    for t in switch_times:
        axes[1].axvline(t, color="red", linestyle="--", alpha=0.4)

    axes[1].set_title("V14.1 Phase Velocity")
    axes[1].grid(True)
    axes[1].legend(loc="best")
    axes[1].set_xlabel("time step")

    plt.tight_layout()
    path = os.path.join(RIFT_DIR, "v14_1_phase_analysis.png")
    plt.savefig(path, dpi=150)
    print(f"💾 Saved → {path}")
    plt.close()
...
def plot_error_analysis(phase_error_ref, phase_error_target, switch_flag):
    ...
    plt.title("V14.1 Errors + Regime Switches")
    plt.grid(True)
    plt.legend(loc="best")

    path = os.path.join(RIFT_DIR, "v14_1_error_analysis.png")
    plt.savefig(path, dpi=150)
    print(f"💾 Saved → {path}")
    plt.close()
...
def save_regime_summary(regime_idx, switch_flag):
    ...
    lines = []
    lines.append("# V14.1 Regime Summary\n")
    ...
    path = os.path.join(RIFT_DIR, "v14_1_regime_summary.md")
...
def main():
    ...
    result = phase_target_controller_v14_1(trajectory, rift)

    np.save(os.path.join(RIFT_DIR, "phase_target_v14_1.npy"), result["controlled"])
    print("💾 Saved controlled trajectory → phase_target_v14_1.npy")
