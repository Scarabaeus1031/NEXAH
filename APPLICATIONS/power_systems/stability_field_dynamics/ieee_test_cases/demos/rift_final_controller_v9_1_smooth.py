# rift_final_controller_v9_1_smooth.py
...
# --------------------------------------------------
# V9.1 SMOOTH DUAL LAYER
# --------------------------------------------------

def smooth_dual_layer_controller(trajectory, rift):
    ...
    upper_layer = base_layer + spread * 0.95

    print(f"🎯 Upper Layer: {upper_layer:.4f}")
    print(f"⚡ Events: {events}")
    print(f"🔊 Frequencies: {f1:.4f}, {f2:.4f}")

    # gains (leicht reduziert für Stabilität)
    gain_rift = 0.16
    gain_freq = 0.10
    gain_event = 0.30
    gain_layer = 0.20
    ...
    for t in range(len(controlled)):
        ...
        blend = min(inst / 0.05, 1.0)

        target_layer = (1 - blend) * base_layer + blend * upper_layer
        ...
        correction = np.tanh(momentum[t]) * 0.04  # weniger aggressiv
...
def plot_result(original, controlled, rift, base_layer, upper_layer):
    ...
    plt.plot(controlled[:, 0], controlled[:, 1], label="V9.1 smooth", color="gold")
    ...
    save_path = os.path.join(RIFT_DIR, "final_controller_v9_1_smooth.png")
...
def main():
    ...
    controlled, base_layer, upper_layer = smooth_dual_layer_controller(trajectory, rift)

    np.save(os.path.join(RIFT_DIR, "final_controller_v9_1.npy"), controlled)
    print("💾 Saved controlled trajectory → final_controller_v9_1.npy")

    plot_result(trajectory, controlled, rift, base_layer, upper_layer)

    print("🚀 FINAL CONTROLLER V9.1 (SMOOTH) complete")
