...
# --------------------------------------------------
# EXPERIMENT CONFIG (FAST VERSION)
# --------------------------------------------------

loads = np.linspace(1.0, 6.0, 12)   # 🔥 reduziert (war 80)
USE_NOISE = True
NOISE_STD = 0.01

results = []

print("\n--- IEEE vs NEXAH Relevance Test (FAST) ---\n")
...
df.to_csv("ieee_relevance_results_fast.csv", index=False)

print("\nSaved results to ieee_relevance_results_fast.csv")
...
plt.tight_layout()
plt.savefig("ieee_relevance_plots_fast.png", dpi=200)
plt.show()

print("\nSaved plots to ieee_relevance_plots_fast.png")
