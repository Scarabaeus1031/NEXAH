# APPLICATIONS/power_systems/stability_field_dynamics/ieee_test_cases/validate_ieee9.py
...
def plot_histogram(radius, peaks, gap):
    ...
    plt.title(f"IEEE 9 Peaks: {np.round(peaks, 3)} | Gap: {gap:.3f}")
    plt.show()
...
def plot_states(M, states):
    ...
    plt.title("IEEE 9 — Detected States")
    plt.show()
...
def plot_loops(loops):
    ...
    if len(loops) == 0:
        plt.title("IEEE 9 — Detected Loops: none")
        plt.show()
        return
    ...
    plt.title("IEEE 9 — Detected Loops")
    plt.show()
...
def main():
    ...
    # same setup idea as IEEE 14:
    # choose a representative load bus index safely
    load_values = net.load["bus"].values
    load_bus = int(load_values[min(2, len(load_values) - 1)])
