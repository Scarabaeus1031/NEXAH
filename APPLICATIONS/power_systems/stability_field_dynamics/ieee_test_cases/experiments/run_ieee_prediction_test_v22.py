# run_ieee_prediction_test_v22.py
...
print("RUNNING IEEE PREDICTION TEST V22 (FRAGMENTATION-AWARE PREDICTOR)")
...
def compute_metrics(theta, c, loops):
    ...
    # NEW: fragmentation / decoherence proxy
...
def run_case(case_name):
    ...
    # NEW SCORE: fragmentation-aware
    w1, w2, w3, w4 = 0.25, 0.20, 0.30, 0.25
    df["score"] = (
        w1 * df["c_struct_norm"] +
        w2 * df["dc_dload_norm"] +
        w3 * df["d2c_dload2_norm"] +
        w4 * df["fragmentation_norm"]
    )
    ...
    summary = {
        ...
        "max_fragmentation": valid["fragmentation"].max(),
    }
...
# Save
csv_main = os.path.join(OUTPUT_DIR, "ieee_prediction_test_v22.csv")
csv_summary = os.path.join(OUTPUT_DIR, "ieee_prediction_test_v22_summary.csv")
...
for i, case_name in enumerate(CASES):
    ...
    ax.set_title(f"{case_name} — Fragmentation-Aware Score")
    ax.set_xlabel("Load")
    ax.set_ylabel("Score")
    ax.grid()
    ax.legend()
