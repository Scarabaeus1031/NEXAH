# APPLICATIONS/power_systems/ieee_test_cases/run_scan_v13c_lanif_band.py
...
from .lanif_band_v13c import apply_lanif_band
...
def plot_flow(landscape, Fx, Fy):
    ...
    plt.title("V13c LANIF Flow Field")
    plt.show()


def plot_band(mask, radius):
...
def plot_states(M, states):
    ...
    plt.title("Detected States (LANIF Activated)")
    plt.show()
...
def plot_loops(loops):
    ...
    if len(loops) == 0:
        plt.title("Detected Loops (LANIF) — none")
        plt.show()
        return
    ...
    plt.title("Detected Loops (LANIF)")
    plt.show()
...
# =========================
def main():
    print("\n--- V13c LANIF Band / Resonance Quantization ---")
    ...
    # ===== LANIF BAND =====
    Fx, Fy, mask, radius = apply_lanif_band(
        Fx, Fy,
        target_radius=0.26,
        band_width=0.05,
        in_band_boost=1.8,
