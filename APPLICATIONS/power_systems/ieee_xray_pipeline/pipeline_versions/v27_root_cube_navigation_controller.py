"""
v27_root_cube_navigation_controller.py
=======================================

NEXAH v27 – Prime Membrane Break + Enclosed White Trigger
Reagiert direkt auf die weißen enclosed Parts aus deinem Screenshot
"""
...
TS_PATH    = OUTDIR / "ieee57_v27_root_cube_timeseries.png"
POLAR_PATH = OUTDIR / "ieee57_v27_root_cube_polar.png"
CUBE_PATH  = OUTDIR / "ieee57_v27_root_cube_3d_projection.png"
REPORT_PATH= OUTDIR / "ieee57_v27_root_cube_report.txt"

print(f"📁 Saving to → {OUTDIR.resolve()}\n")

# ============================================================
# Settings (v27 – Enclosed White Trigger)
# ============================================================
TIME_STEPS = 300
SEED = 42
...
def is_enclosed_white_pattern(u_history):
    """Erkennt die weißen enclosed Parts (starke periodische Blöcke)"""
    if len(u_history) < 20:
        return False
    recent_u = np.array(u_history[-20:])
    # Starke, regelmäßige On/Off-Blöcke
    transitions = np.sum(np.abs(np.diff(np.sign(recent_u))) > 0)
    return transitions >= 8 and np.std(recent_u) > 0.08

def choose_mode(r, theta, prev_mode, ncs_prox, escape_count, u_history):
    if escape_count == PRIME_BREAK or is_enclosed_white_pattern(u_history):
        return "gate_lock"
    if prev_mode == "gate_lock":
        if ncs_prox > 0.76: return "gate_lock"
...
def simulate_v27(time_steps=TIME_STEPS, seed=SEED):
    np.random.seed(seed)
    net = pp.networks.case57()
    base_p = net.load["p_mw"].copy()
    base_q = net.load.get("q_mvar", None)
    ...
    for t in range(time_steps):
        ...
        mode = choose_mode(r_est, th_est, last_mode, gate_score, escape_count, u_hist)
        ...
        if escape_count == PRIME_BREAK or is_enclosed_white_pattern(u_hist):
            nudge *= 4.0   # starker Prime-Break Nudge
        ...
        if mode == "capture":
            ...
            snap = K_SNAP * gate_score if gate_score > 0.76 else 0.0
...
controlled = simulate_v27()

print("✅ v27 (Enclosed White Trigger + Prime Break) erfolgreich ausgeführt!")
...
axs[0].plot(t, controlled["voltage_mean"], label="v27 Controlled")
...
fig.suptitle("NEXAH v27 – Golden Scarabaeus Möbius Breathing Pulse + Enclosed White Trigger")
fig.tight_layout()
...
ax.set_title("Polar View – Root Cube Projection (v27)")
ax.legend()
fig.tight_layout()
...
ax.set_title("3D Root Cube Projection (v27)")
fig.tight_layout()
fig.savefig(CUBE_PATH, dpi=160)
plt.close(fig)

report = f"""NEXAH v27 Root Cube Navigation Report (Enclosed White Trigger + Prime Break)
========================================
