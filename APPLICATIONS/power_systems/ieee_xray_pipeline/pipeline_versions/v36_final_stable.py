...
NEXAH v36 FINAL STABLE – genau die gute Version mit -0.0770
Minimal, mit Agg-Backend + Save-Check → Bilder werden jetzt garantiert geschrieben
"""
...
TS_PATH    = OUTDIR / "v36_final_timeseries.png"
POLAR_PATH = OUTDIR / "v36_final_polar.png"
CUBE_PATH  = OUTDIR / "v36_final_3d.png"
REPORT_PATH= OUTDIR / "v36_final_report.txt"
...
for t in range(300):
    ...
    # Sanfte Control-Logik (v36)
    u = 0.0
    u += 0.68 * max(0.0, 0.032 - r)                    # lift
    u += 0.95 * (0.0325 + 0.014*np.sin(0.118*t) - r)   # hold + breath
    u += -0.215 * np.exp(-2.0*dist_elastic) * np.sin(theta - np.pi/4 + 0.3)   # flow
    u += 0.19 * (np.pi/4 - theta)                       # axis pull
    ...
    controlled["voltage"].append(np.mean(net.res_bus.vm_pu))
...
# 2. REPORT
# ============================================================
report = f"""NEXAH v36 FINAL STABLE
...
# (Hier könntest du die 5 Subplots einfügen – falls du sie brauchst, sag Bescheid)
save_plot(fig, TS_PATH, "v36_final Timeseries")

# Polar
fig = plt.figure(figsize=(10, 10))
ax = fig.add_subplot(111, projection='polar')
ax.plot(controlled["radius"], controlled["gate_score"], 'b-')
save_plot(fig, POLAR_PATH, "v36_final Polar")

# 3D
fig = plt.figure(figsize=(12, 10))
ax = fig.add_subplot(111, projection='3d')
ax.scatter(controlled["radius"], [np.arctan2(0,0)]*len(controlled["radius"]), controlled["dist_elastic"], c=controlled["gate_score"], cmap='plasma')
save_plot(fig, CUBE_PATH, "v36_final 3D Cube")

print("\n✅ v36_final fertig – bitte schick mir den Konsolen-Output!")
