"""
v19b_root_cube_navigation_controller.py
=======================================

NEXAH v19b – Counterclockwise Breathing Crystal
(exactly as in your Triple-Six Resonance image)
Yellow-black pointer rotates counterclockwise
"""
...
TS_PATH    = OUTDIR / "ieee57_v19b_root_cube_timeseries.png"
POLAR_PATH = OUTDIR / "ieee57_v19b_root_cube_polar.png"
CUBE_PATH  = OUTDIR / "ieee57_v19b_root_cube_3d_projection.png"
REPORT_PATH= OUTDIR / "ieee57_v19b_root_cube_report.txt"
...
# v19b: Counterclockwise + stronger breathing
K_FLOW          = 0.165
...
def simulate_v19b(time_steps=TIME_STEPS, seed=SEED):
    np.random.seed(seed)
    net = pp.networks.case57()
    base_p = net.load["p_mw"].copy()
    base_q = net.load.get("q_mvar", None)
    ...
    for t in range(time_steps):
        ...
        if mode == "capture":
            ...
            # Counterclockwise Flow (wie in deinem Bild)
            axis_factor = np.exp(-2.0 * cube["dist_to_elastic"])
            flow = -K_FLOW * axis_factor * np.sin(th_est - ELASTIC_AXIS_ANGLE + FLOW_PHASE)   # <--- Minuszeichen für Gegen-Uhrzeigersinn
...
controlled = simulate_v19b()

print("✅ v19b (counterclockwise) erfolgreich ausgeführt!")

# Plots + Report (wie vorher)
t = np.arange(TIME_STEPS)

fig, axs = plt.subplots(5, 1, figsize=(14, 18), sharex=True)
axs[0].plot(t, baseline["voltage_mean"], alpha=0.4, label="Baseline")
axs[0].plot(t, controlled["voltage_mean"], label="v19b Controlled")
...
fig.suptitle("NEXAH v19b – Triple-Six Resonance Breathing Crystal (Counterclockwise)")
fig.tight_layout()
...
ax.set_title("Polar View – Root Cube Projection (v19b Counterclockwise)")
ax.legend()
fig.tight_layout()
...
ax.set_title("3D Root Cube Projection (v19b)")
fig.tight_layout()
fig.savefig(CUBE_PATH, dpi=160)
plt.close(fig)

report = f"""NEXAH v19b Root Cube Navigation Report (Counterclockwise)
========================================
