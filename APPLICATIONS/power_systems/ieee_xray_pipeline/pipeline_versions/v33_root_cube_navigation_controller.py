"""
v33_root_cube_navigation_controller.py
======================================

NEXAH v33 – Golden Scarabaeus Möbius Breathing Pulse + 7-Arc + 5×17 Full Break
Korrigiert: weniger Aggression, stabilere Spirale
"""
...
TS_PATH    = OUTDIR / "ieee57_v33_root_cube_timeseries.png"
POLAR_PATH = OUTDIR / "ieee57_v33_root_cube_polar.png"
CUBE_PATH  = OUTDIR / "ieee57_v33_root_cube_3d_projection.png"
REPORT_PATH= OUTDIR / "ieee57_v33_root_cube_report.txt"

print(f"📁 Speichere in: {OUTDIR}")

# ============================================================
# 1. SETTINGS (ausbalanciert für v33)
# ============================================================
TIME_STEPS = 300
SEED = 42
...
K_LIFT          = 0.85
K_R_HOLD        = 1.12
K_FLOW          = 0.275          # Counterclockwise
K_AXIS_PULL     = 0.32           # reduziert
K_SNAP          = 0.22
BREATH_FREQ     = 0.118
BREATH_AMP      = 0.032          # reduziert
BREATH_TWIST    = 0.013          # reduziert
NUDGE_AMP       = 0.028
PRIME_BREAK     = 64
...
def is_enclosed_white_pattern(u_history):
    ...
    return transitions >= 16 and strong_blocks >= 22
...
def choose_mode(r, theta, dist_elastic, ncs_prox, escape_count, u_history):
    if escape_count >= PRIME_BREAK or is_enclosed_white_pattern(u_history) or is_five_seventeen_trigger(np.mean(u_history[-10:]) if u_history else 0):
        return "gate_lock"
    if r > R_ENVELOPE_MAX:
        return "outer_return"
...
def simulate_v33():
    ...
    baseline = {"voltage": [], "coherence": [], "radius": [], "dist_elastic": [], "u": []}
    ...
    for t in range(TIME_STEPS):
        ...
        baseline["voltage"].append(v_mean)
        baseline["coherence"].append(1.0 - np.std(net.res_bus.vm_pu))
        baseline["u"].append(0.0)
        ...
        if mode == "capture":
            ...
            snap = K_SNAP * ncs_prox if ncs_prox > 0.65 else 0.0
            if escape_count == PRIME_BREAK or is_enclosed_white_pattern(u_hist) or is_five_seventeen_trigger(np.mean(u_hist[-10:]) if u_hist else 0):
                nudge *= 5.0
...
# 4. RUN & SAVE
# ============================================================
baseline, controlled, escape_count = simulate_v33()

report = f"""NEXAH v32 Root Cube Navigation Report (7-Arc + 5×17 Full Break)
========================================
...
# Plots (Timeseries, Polar, 3D) – gleiche wie vorher, nur Titel v33
# ... (die Plot-Blöcke sind identisch mit v31/v32, nur Titel angepasst)

print(f"✅ v33 erfolgreich ausgeführt!")
