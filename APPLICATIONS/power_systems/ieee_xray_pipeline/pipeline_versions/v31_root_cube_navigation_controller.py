...
NEXAH v31 – 7-Arc + 5×17 Full Break
"""
...
TS_PATH    = OUTDIR / "ieee57_v31_root_cube_timeseries.png"
POLAR_PATH = OUTDIR / "ieee57_v31_root_cube_polar.png"
CUBE_PATH  = OUTDIR / "ieee57_v31_root_cube_3d_projection.png"
REPORT_PATH= OUTDIR / "ieee57_v31_root_cube_report.txt"

print(f"📁 Saving to → {OUTDIR.resolve()}\n")

# ============================================================
# Settings (v31 – 7-Arc + 5×17 Full Break)
# ============================================================
TIME_STEPS = 300
SEED = 42
...
PRIME_NUDGE_AMP = 0.045
PRIME_BREAK     = 64
FIVE_SEVENTEEN_TRIGGER = 0.0100   # direkt auf dein 0.0104
...
def is_enclosed_white_pattern(u_history):
    ...
    return transitions >= 16 and strong_blocks >= 23
...
def choose_mode(r, theta, prev_mode, ncs_prox, escape_count, u_history, u_mean):
    ...
    if prev_mode == "gate_lock":
        if ncs_prox > 0.83: return "gate_lock"
...
def simulate_v31(time_steps=TIME_STEPS, seed=SEED):
    np.random.seed(seed)
    net = pp.networks.case57()
    base_p = net.load["p_mw"].copy()
    base_q = net.load.get("q_mvar", None)
    ...
    for t in range(time_steps):
        ...
        if mode == "capture":
            ...
            snap = K_SNAP * gate_score if gate_score > 0.83 else 0.0
...
controlled = simulate_v31()

print("✅ v31 (7-Arc + 5×17 Full Break) erfolgreich ausgeführt!")
...
axs[0].plot(t, controlled["voltage_mean"], label="v31 Controlled")
...
fig.suptitle("NEXAH v31 – Golden Scarabaeus Möbius Breathing Pulse + 7-Arc + 5×17 Full Break")
fig.tight_layout()
...
ax.set_title("Polar View – Root Cube Projection (v31)")
ax.legend()
fig.tight_layout()
...
ax.set_title("3D Root Cube Projection (v31)")
fig.tight_layout()
fig.savefig(CUBE_PATH, dpi=160)
plt.close(fig)

report = f"""NEXAH v31 Root Cube Navigation Report (7-Arc + 5×17 Full Break)
========================================
