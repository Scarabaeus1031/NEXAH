"""
v21_root_cube_navigation_controller.py
=======================================

NEXAH v21 – Triple-Six Breathing Crystal (tighter + stronger breathing)
Counterclockwise + deutliche Atmung + bessere Axis-Hug
"""
...
TS_PATH    = OUTDIR / "ieee57_v21_root_cube_timeseries.png"
POLAR_PATH = OUTDIR / "ieee57_v21_root_cube_polar.png"
CUBE_PATH  = OUTDIR / "ieee57_v21_root_cube_3d_projection.png"
REPORT_PATH= OUTDIR / "ieee57_v21_root_cube_report.txt"

print(f"📁 Saving to → {OUTDIR.resolve()}\n")

# ============================================================
# Settings (v21 – stärkeres Breathing + Axis-Pull)
# ============================================================
TIME_STEPS = 300
SEED = 42
...
K_LIFT          = 0.28
K_AXIS_PULL     = 0.18      # stärkerer Pull
K_R_HOLD        = 0.09
K_SNAP          = 0.12
K_PULSE         = 0.10

BREATH_FREQ     = 0.09
BREATH_AMP      = 0.018     # stärkeres Atmen

NCS_LOCKS_DEG = [97.0, 277.0, 292.0]
NCS_LOCKS     = np.deg2rad(NCS_LOCKS_DEG)
SNAP_TOL      = np.deg2rad(6.5)   # etwas sensibler
...
def choose_mode(r, theta, prev_mode, ncs_prox):
    if prev_mode == "gate_lock":
        if ncs_prox > 0.68: return "gate_lock"
...
def simulate_v21(time_steps=TIME_STEPS, seed=SEED):
    np.random.seed(seed)
    net = pp.networks.case57()
    base_p = net.load["p_mw"].copy()
    base_q = net.load.get("q_mvar", None)
...
controlled = simulate_v21()

print("✅ v21 (stärkere Breathing Crystal) erfolgreich ausgeführt!")
...
axs[0].plot(t, controlled["voltage_mean"], label="v21 Controlled")
...
fig.suptitle("NEXAH v21 – Triple-Six Resonance Breathing Crystal (Counterclockwise)")
fig.tight_layout()
...
ax.set_title("Polar View – Root Cube Projection (v21)")
ax.legend()
fig.tight_layout()
...
ax.set_title("3D Root Cube Projection (v21)")
fig.tight_layout()
fig.savefig(CUBE_PATH, dpi=160)
plt.close(fig)

report = f"""NEXAH v21 Root Cube Navigation Report
========================================
