"""
v16_root_cube_navigation_controller.py
======================================

NEXAH v16 – Root Cube Navigation Controller
Goal: Stronger expansion + orbital flow around Elastic Axis + better 292 NCS gate triggering
"""
...
TS_PATH    = OUTDIR / "ieee57_v16_root_cube_timeseries.png"
POLAR_PATH = OUTDIR / "ieee57_v16_root_cube_polar.png"
CUBE_PATH  = OUTDIR / "ieee57_v16_root_cube_3d_projection.png"
REPORT_PATH= OUTDIR / "ieee57_v16_root_cube_report.txt"
...
# === v16 improvements ===
K_FLOW      = 0.12          # stärkerer Flow
FLOW_PHASE  = np.pi / 2 + 0.3
K_LIFT      = 0.18          # deutlich stärkere Expansion
K_R_HOLD    = 0.065
K_THETA_HOLD= 0.035
K_SNAP      = 0.085
K_PULSE     = 0.065
...
# v16 Simulation
# ============================================================
def simulate_v16(time_steps=TIME_STEPS, seed=SEED):
    np.random.seed(seed)
    net = pp.networks.case57()
    base_p = net.load["p_mw"].copy()
    base_q = net.load.get("q_mvar", None)
    ...
    for t in range(time_steps):
        ...
        # === v16 improvements ===
        if mode == "capture":
            u += K_LIFT * max(0.0, (R_CAPTURE_TARGET - r_est))           # starke Expansion
        elif mode in ["band_hold", "gate_lock"]:
            u += K_R_HOLD * (R_TARGET - r_est)
            
            # stärkerer Flow, je näher an Elastic Axis
            axis_factor = np.exp(-3.0 * cube["dist_to_elastic"])
...
controlled = simulate_v16()

print("✅ v16 Root Cube Navigation Controller erfolgreich ausgeführt!")
...
axs[0].plot(t, controlled["voltage_mean"], label="v16 Controlled")
...
fig.suptitle("NEXAH v16 – Root Cube Navigation Controller")
fig.tight_layout()
...
ax.set_title("3D Root Cube Projection (v16)")
fig.tight_layout()
fig.savefig(CUBE_PATH, dpi=160)
plt.close(fig)

# Report
report = f"""NEXAH v16 Root Cube Navigation Report
========================================
