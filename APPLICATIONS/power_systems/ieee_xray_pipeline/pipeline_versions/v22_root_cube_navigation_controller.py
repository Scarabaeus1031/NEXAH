"""
v22_root_cube_navigation_controller.py
=======================================

NEXAH v22 – Triple-Six Breathing Crystal (pressed + stronger breathing)
Counterclockwise + sichtbares Atmen + kräftiger Axis-Pull + Snap
"""
...
TS_PATH    = OUTDIR / "ieee57_v22_root_cube_timeseries.png"
POLAR_PATH = OUTDIR / "ieee57_v22_root_cube_polar.png"
CUBE_PATH  = OUTDIR / "ieee57_v22_root_cube_3d_projection.png"
REPORT_PATH= OUTDIR / "ieee57_v22_root_cube_report.txt"

print(f"📁 Saving to → {OUTDIR.resolve()}\n")

# ============================================================
# Settings (v22 – stärker gepresst)
# ============================================================
TIME_STEPS = 300
SEED = 42
...
K_LIFT          = 0.29
K_AXIS_PULL     = 0.20      # stärkerer Pull
K_R_HOLD        = 0.095
K_SNAP          = 0.13      # kräftigerer Snap
K_PULSE         = 0.11

BREATH_FREQ     = 0.095
BREATH_AMP      = 0.021     # deutlich stärkeres Atmen

NCS_LOCKS_DEG = [97.0, 277.0, 292.0]
NCS_LOCKS     = np.deg2rad(NCS_LOCKS_DEG)
SNAP_TOL      = np.deg2rad(6.0)
...
def choose_mode(r, theta, prev_mode, ncs_prox):
    if prev_mode == "gate_lock":
        if ncs_prox > 0.70: return "gate_lock"
...
def simulate_v22(time_steps=TIME_STEPS, seed=SEED):
    np.random.seed(seed)
    net = pp.networks.case57()
    base_p = net.load["p_mw"].copy()
    base_q = net.load.get("q_mvar", None)
    ...
    for t in range(time_steps):
        ...
        breath = BREATH_AMP * np.sin(BREATH_FREQ * t)
        ...
        if mode == "capture":
            ...
            flow = -K_FLOW * axis_factor * np.sin(th_est - ELASTIC_AXIS_ANGLE + FLOW_PHASE)   # counterclockwise
            axis_pull = K_AXIS_PULL * (ELASTIC_AXIS_ANGLE - th_est)

            # Snap bei hoher NCS-Nähe (pressen)
            snap = K_SNAP * gate_score if gate_score > 0.65 else 0.0

            u += flow + axis_pull + snap
...
controlled = simulate_v22()

print("✅ v22 (stärker gepresst + Breathing) erfolgreich ausgeführt!")
...
axs[0].plot(t, controlled["voltage_mean"], label="v22 Controlled")
...
fig.suptitle("NEXAH v22 – Triple-Six Resonance Breathing Crystal (pressed)")
fig.tight_layout()
...
ax.set_title("Polar View – Root Cube Projection (v22)")
ax.legend()
fig.tight_layout()
...
ax.set_title("3D Root Cube Projection (v22)")
fig.tight_layout()
fig.savefig(CUBE_PATH, dpi=160)
plt.close(fig)

report = f"""NEXAH v22 Root Cube Navigation Report
========================================
