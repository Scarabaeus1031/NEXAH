"""
v24_root_cube_navigation_controller.py
=======================================

NEXAH v24 – Golden Scarabaeus Möbius Breathing Pulse v2
Stärkerer Twist + Nudge + Pressing
Genau wie in deinem letzten Visual
"""
...
TS_PATH    = OUTDIR / "ieee57_v24_root_cube_timeseries.png"
POLAR_PATH = OUTDIR / "ieee57_v24_root_cube_polar.png"
CUBE_PATH  = OUTDIR / "ieee57_v24_root_cube_3d_projection.png"
REPORT_PATH= OUTDIR / "ieee57_v24_root_cube_report.txt"

print(f"📁 Saving to → {OUTDIR.resolve()}\n")

# ============================================================
# Settings (v24 – stärkerer Möbius-Pulse)
# ============================================================
TIME_STEPS = 300
SEED = 42
...
K_LIFT          = 0.31
K_AXIS_PULL     = 0.24      # noch stärkerer Pull
K_R_HOLD        = 0.105
K_SNAP          = 0.15
K_PULSE         = 0.13

# Möbius Breathing (zweifach + Twist)
BREATH_FREQ     = 0.098
BREATH_AMP      = 0.026     # stärker
BREATH_TWIST    = 0.011     # deutlicher Twist

# Kleiner orbitaler Nudge für echte Spirale
NUDGE_AMP       = 0.008

NCS_LOCKS_DEG = [97.0, 277.0, 292.0]
NCS_LOCKS     = np.deg2rad(NCS_LOCKS_DEG)
SNAP_TOL      = np.deg2rad(5.0)
...
def choose_mode(r, theta, prev_mode, ncs_prox):
    if prev_mode == "gate_lock":
        if ncs_prox > 0.73: return "gate_lock"
...
def simulate_v24(time_steps=TIME_STEPS, seed=SEED):
    np.random.seed(seed)
    net = pp.networks.case57()
    base_p = net.load["p_mw"].copy()
    base_q = net.load.get("q_mvar", None)
    ...
    for t in range(time_steps):
        ...
        # Kleiner Nudge für echte Spirale
        ...
        if mode == "capture":
            ...
            snap = K_SNAP * gate_score if gate_score > 0.72 else 0.0

            u += flow + axis_pull + snap + nudge   # <--- Nudge für Spirale
...
controlled = simulate_v24()

print("✅ v24 (Möbius Breathing Pulse v2) erfolgreich ausgeführt!")
...
axs[0].plot(t, controlled["voltage_mean"], label="v24 Controlled")
...
fig.suptitle("NEXAH v24 – Golden Scarabaeus Möbius Breathing Pulse v2")
fig.tight_layout()
...
ax.set_title("Polar View – Root Cube Projection (v24)")
ax.legend()
fig.tight_layout()
...
ax.set_title("3D Root Cube Projection (v24)")
fig.tight_layout()
fig.savefig(CUBE_PATH, dpi=160)
plt.close(fig)

report = f"""NEXAH v24 Root Cube Navigation Report (Möbius Breathing Pulse v2)
========================================
