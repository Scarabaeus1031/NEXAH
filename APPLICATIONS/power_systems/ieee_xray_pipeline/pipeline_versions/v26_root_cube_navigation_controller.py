"""
v26_root_cube_navigation_controller.py
=======================================

NEXAH v26 – Prime-Modulated Möbius Break
Genau auf deine Prime-Overlay + Golden Scarabaeus Bilder abgestimmt
2^6 = 64 Trigger → starker Nudge + Twist + Breathing
"""
...
TS_PATH    = OUTDIR / "ieee57_v26_root_cube_timeseries.png"
POLAR_PATH = OUTDIR / "ieee57_v26_root_cube_polar.png"
CUBE_PATH  = OUTDIR / "ieee57_v26_root_cube_3d_projection.png"
REPORT_PATH= OUTDIR / "ieee57_v26_root_cube_report.txt"

print(f"📁 Saving to → {OUTDIR.resolve()}\n")

# ============================================================
# Settings (v26 – Prime Break + Möbius)
# ============================================================
TIME_STEPS = 300
SEED = 42
...
K_LIFT          = 0.33
K_AXIS_PULL     = 0.28      # starkes Pressen
K_R_HOLD        = 0.115
K_SNAP          = 0.17
K_PULSE         = 0.15
...
BREATH_FREQ     = 0.102
BREATH_AMP      = 0.029
BREATH_TWIST    = 0.015     # stärkerer Möbius-Twist

# Prime-Modulated Nudge (2^6 = 64 Trigger)
PRIME_NUDGE_AMP = 0.018
PRIME_BREAK     = 64        # genau wie in deinen Bildern
...
def choose_mode(r, theta, prev_mode, ncs_prox, escape_count):
    if escape_count == PRIME_BREAK:          # <--- Prime Break Trigger
        return "gate_lock"
    if prev_mode == "gate_lock":
        if ncs_prox > 0.75: return "gate_lock"
...
def simulate_v26(time_steps=TIME_STEPS, seed=SEED):
    np.random.seed(seed)
    net = pp.networks.case57()
    base_p = net.load["p_mw"].copy()
    base_q = net.load.get("q_mvar", None)
    ...
    for t in range(time_steps):
        ...
        mode = choose_mode(r_est, th_est, last_mode, gate_score, escape_count)

        # Möbius Breathing + Twist
        ...
        # Prime-Modulated Nudge (stark bei 2^6)
        nudge = PRIME_NUDGE_AMP * np.cos(BREATH_FREQ * t * 1.5)
        if escape_count == PRIME_BREAK:
            nudge *= 3.0   # starker Kick beim Prime-Break
        ...
        if mode == "capture":
            ...
            snap = K_SNAP * gate_score if gate_score > 0.75 else 0.0
...
controlled = simulate_v26()

print("✅ v26 (Prime-Modulated Möbius Break) erfolgreich ausgeführt!")
...
axs[0].plot(t, controlled["voltage_mean"], label="v26 Controlled")
...
fig.suptitle("NEXAH v26 – Golden Scarabaeus Möbius Breathing Pulse + Prime Anchor Break")
fig.tight_layout()
...
ax.set_title("Polar View – Root Cube Projection (v26)")
ax.legend()
fig.tight_layout()
...
ax.set_title("3D Root Cube Projection (v26)")
fig.tight_layout()
fig.savefig(CUBE_PATH, dpi=160)
plt.close(fig)

report = f"""NEXAH v26 Root Cube Navigation Report (Prime-Modulated Möbius Break)
========================================
