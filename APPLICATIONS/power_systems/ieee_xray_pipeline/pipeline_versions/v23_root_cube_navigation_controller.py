"""
v23_root_cube_navigation_controller.py
=======================================

NEXAH v23 – Golden Scarabaeus Möbius Breathing Pulse
Stärkere zweifache Breathing + Twist + Axis-Press
Genau wie in deinem neuen Visual
"""
...
TS_PATH    = OUTDIR / "ieee57_v23_root_cube_timeseries.png"
POLAR_PATH = OUTDIR / "ieee57_v23_root_cube_polar.png"
CUBE_PATH  = OUTDIR / "ieee57_v23_root_cube_3d_projection.png"
REPORT_PATH= OUTDIR / "ieee57_v23_root_cube_report.txt"

print(f"📁 Saving to → {OUTDIR.resolve()}\n")

# ============================================================
# Settings (v23 – Möbius Breathing)
# ============================================================
TIME_STEPS = 300
SEED = 42
...
K_LIFT          = 0.30
K_AXIS_PULL     = 0.22      # noch stärkerer Pull
K_R_HOLD        = 0.10
K_SNAP          = 0.14
K_PULSE         = 0.12

# Möbius Breathing (zweifach)
BREATH_FREQ     = 0.095
BREATH_AMP      = 0.024     # stärker
BREATH_TWIST    = 0.008     # leichter Twist für Möbius-Effekt

NCS_LOCKS_DEG = [97.0, 277.0, 292.0]
NCS_LOCKS     = np.deg2rad(NCS_LOCKS_DEG)
SNAP_TOL      = np.deg2rad(5.5)
...
def choose_mode(r, theta, prev_mode, ncs_prox):
    if prev_mode == "gate_lock":
        if ncs_prox > 0.72: return "gate_lock"
...
def simulate_v23(time_steps=TIME_STEPS, seed=SEED):
    np.random.seed(seed)
    net = pp.networks.case57()
    base_p = net.load["p_mw"].copy()
    base_q = net.load.get("q_mvar", None)
    ...
    for t in range(time_steps):
        ...
        breath_twist = BREATH_TWIST * np.sin(2 * BREATH_FREQ * t)   # leichter Twist
        ...
        if mode == "capture":
            ...
            snap = K_SNAP * gate_score if gate_score > 0.70 else 0.0
...
controlled = simulate_v23()

print("✅ v23 (Möbius Breathing Pulse) erfolgreich ausgeführt!")
...
axs[0].plot(t, controlled["voltage_mean"], label="v23 Controlled")
...
fig.suptitle("NEXAH v23 – Golden Scarabaeus Möbius Breathing Pulse")
fig.tight_layout()
...
ax.set_title("Polar View – Root Cube Projection (v23 Möbius Breathing)")
ax.legend()
fig.tight_layout()
...
ax.set_title("3D Root Cube Projection (v23)")
fig.tight_layout()
fig.savefig(CUBE_PATH, dpi=160)
plt.close(fig)

report = f"""NEXAH v23 Root Cube Navigation Report (Möbius Breathing Pulse)
========================================
