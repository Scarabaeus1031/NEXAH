"""
v28_root_cube_navigation_controller.py
=======================================

NEXAH v28 – Enclosed White Break + Full Möbius Rotation
...
TS_PATH    = OUTDIR / "ieee57_v28_root_cube_timeseries.png"
POLAR_PATH = OUTDIR / "ieee57_v28_root_cube_polar.png"
CUBE_PATH  = OUTDIR / "ieee57_v28_root_cube_3d_projection.png"
REPORT_PATH= OUTDIR / "ieee57_v28_root_cube_report.txt"

print(f"📁 Saving to → {OUTDIR.resolve()}\n")

# ============================================================
# Settings (v28 – Enclosed White Break)
# ============================================================
TIME_STEPS = 300
SEED = 42
...
K_LIFT          = 0.35
K_AXIS_PULL     = 0.32      # sehr starker Pull
K_R_HOLD        = 0.125
K_SNAP          = 0.19
K_PULSE         = 0.17
...
def choose_mode(r, theta, prev_mode, ncs_prox, escape_count, u_history):
    ...
    if prev_mode == "gate_lock":
        if ncs_prox > 0.78: return "gate_lock"
...
def simulate_v28(time_steps=TIME_STEPS, seed=SEED):
    np.random.seed(seed)
    net = pp.networks.case57()
    base_p = net.load["p_mw"].copy()
    base_q = net.load.get("q_mvar", None)
    ...
    for t in range(time_steps):
        ...
        if escape_count == PRIME_BREAK or is_enclosed_white_pattern(u_hist):
            nudge *= 5.0   # sehr starker Break-Nudge
        ...
        if mode == "capture":
            ...
            snap = K_SNAP * gate_score if gate_score > 0.78 else 0.0
...
controlled = simulate_v28()

print("✅ v28 (Enclosed White Break + Full Möbius Rotation) erfolgreich ausgeführt!")
...
axs[0].plot(t, controlled["voltage_mean"], label="v28 Controlled")
...
fig.suptitle("NEXAH v28 – Golden Scarabaeus Möbius Breathing Pulse + Enclosed White Break")
fig.tight_layout()
...
ax.set_title("Polar View – Root Cube Projection (v28)")
ax.legend()
fig.tight_layout()
...
ax.set_title("3D Root Cube Projection (v28)")
fig.tight_layout()
fig.savefig(CUBE_PATH, dpi=160)
plt.close(fig)

report = f"""NEXAH v28 Root Cube Navigation Report (Enclosed White Break + Full Möbius Rotation)
========================================
