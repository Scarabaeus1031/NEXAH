"""
v18_root_cube_navigation_controller.py
======================================

NEXAH v18 – Root Cube Navigation Controller
Inspired by Pink Panther Q-Warp + VINUS Breath Spiral
Goal: Strong pull to Elastic Axis + true orbital spiral motion
"""
...
# ============================================================
# 0. Simple robust path
# ============================================================
OUTDIR = Path("APPLICATIONS/power_systems/ieee_xray_pipeline/results")
OUTDIR.mkdir(parents=True, exist_ok=True)

TS_PATH    = OUTDIR / "ieee57_v18_root_cube_timeseries.png"
POLAR_PATH = OUTDIR / "ieee57_v18_root_cube_polar.png"
CUBE_PATH  = OUTDIR / "ieee57_v18_root_cube_3d_projection.png"
REPORT_PATH= OUTDIR / "ieee57_v18_root_cube_report.txt"
...
# v18 improvements (Pink Panther Q-Warp style)
K_FLOW          = 0.16      # stärkerer orbitaler Flow
FLOW_PHASE      = np.pi / 2 + 0.3
K_LIFT          = 0.25      # starke Expansion aus dem Core
K_AXIS_PULL     = 0.12      # direkter Pull zur Elastic Axis
K_R_HOLD        = 0.075
K_THETA_HOLD    = 0.04
K_SNAP          = 0.095
K_PULSE         = 0.075
...
def choose_mode(r, theta, prev_mode, ncs_prox):
    if prev_mode == "gate_lock":
        if ncs_prox > 0.60: return "gate_lock"
...
def simulate_v18(time_steps=TIME_STEPS, seed=SEED):
    np.random.seed(seed)
    net = pp.networks.case57()
    base_p = net.load["p_mw"].copy()
    base_q = net.load.get("q_mvar", None)
    ...
    for t in range(time_steps):
        ...
        if mode == "capture":
            ...
            # v18: Pink Panther Q-Warp style
            axis_factor = np.exp(-2.0 * cube["dist_to_elastic"])          # stärkerer Pull
            flow = K_FLOW * axis_factor * np.sin(th_est - ELASTIC_AXIS_ANGLE + FLOW_PHASE)
            axis_pull = K_AXIS_PULL * (ELASTIC_AXIS_ANGLE - th_est)      # direkter Zug zur Axis
...
controlled = simulate_v18()

print("✅ v18 erfolgreich ausgeführt!")
...
axs[0].plot(t, controlled["voltage_mean"], label="v18 Controlled")
...
fig.suptitle("NEXAH v18 – Root Cube Navigation Controller (Pink Panther Q-Warp Style)")
fig.tight_layout()
...
ax.set_title("3D Root Cube Projection (v18)")
fig.tight_layout()
fig.savefig(CUBE_PATH, dpi=160)
plt.close(fig)

# Report
report = f"""NEXAH v18 Root Cube Navigation Report
========================================
