"""
v19_root_cube_navigation_controller.py
======================================

NEXAH v19 – Triple-Six Resonance Breathing Crystal
Inspired by your "Breathing Crystal Preview" + Pink Panther Q-Warp
Clockwise rotation + explicit breathing of the orange pearl chain
"""
...
TS_PATH    = OUTDIR / "ieee57_v19_root_cube_timeseries.png"
POLAR_PATH = OUTDIR / "ieee57_v19_root_cube_polar.png"
CUBE_PATH  = OUTDIR / "ieee57_v19_root_cube_3d_projection.png"
REPORT_PATH= OUTDIR / "ieee57_v19_root_cube_report.txt"

print(f"📁 Saving to → {OUTDIR.resolve()}\n")

# ============================================================
# 1. Settings (v19 tuned to your Triple-Six image)
# ============================================================
TIME_STEPS = 300
SEED = 42
...
# Breathing (orange pearl chain)
BREATH_FREQ     = 0.08      # how fast it breathes
BREATH_AMP      = 0.012     # amplitude of breathing
...
# v19 Simulation
def simulate_v19(time_steps=TIME_STEPS, seed=SEED):
    np.random.seed(seed)
    net = pp.networks.case57()
    base_p = net.load["p_mw"].copy()
    base_q = net.load.get("q_mvar", None)
    ...
    for t in range(time_steps):
        ...
        if mode == "capture":
            ...
            u += K_R_HOLD * (R_TARGET + breath - r_est)   # breathing radius

            # Clockwise Flow + Axis Pull
...
controlled = simulate_v19()

print("✅ v19 erfolgreich ausgeführt!")
...
axs[0].plot(t, controlled["voltage_mean"], label="v19 Controlled")
...
fig.suptitle("NEXAH v19 – Triple-Six Resonance Breathing Crystal")
fig.tight_layout()
fig.savefig(TS_PATH, dpi=160)
plt.close(fig)

# Polar + 3D + Report (wie in v18)
...
ax.set_title("Polar View – Root Cube Projection (v19)")
ax.legend()
fig.tight_layout()
...
ax.set_title("3D Root Cube Projection (v19)")
fig.tight_layout()
fig.savefig(CUBE_PATH, dpi=160)
plt.close(fig)

report = f"""NEXAH v19 Root Cube Navigation Report
========================================
