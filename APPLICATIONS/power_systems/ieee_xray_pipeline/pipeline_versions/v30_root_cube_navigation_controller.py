"""
v30_root_cube_navigation_controller.py
=======================================

NEXAH v30 – 5×17 Prime Ultra-Break + Maximum Möbius Rotation
"""
...
TS_PATH    = OUTDIR / "ieee57_v30_root_cube_timeseries.png"
POLAR_PATH = OUTDIR / "ieee57_v30_root_cube_polar.png"
CUBE_PATH  = OUTDIR / "ieee57_v30_root_cube_3d_projection.png"
REPORT_PATH= OUTDIR / "ieee57_v30_root_cube_report.txt"

print(f"📁 Saving to → {OUTDIR.resolve()}\n")

# ============================================================
# Settings (v30 – Ultra-Break)
# ============================================================
TIME_STEPS = 300
SEED = 42
...
K_LIFT          = 0.37
K_AXIS_PULL     = 0.36      # MAXIMUM Pull
K_R_HOLD        = 0.135
K_SNAP          = 0.22
K_PULSE         = 0.19

BREATH_FREQ     = 0.115
BREATH_AMP      = 0.042     # sehr starkes Atmen
BREATH_TWIST    = 0.024     # starker Twist

PRIME_NUDGE_AMP = 0.038
PRIME_BREAK     = 64
FIVE_SEVENTEEN_TRIGGER = 0.0080   # noch sensibler um dein 0.0066–0.0085
...
def is_enclosed_white_pattern(u_history):
    ...
    return transitions >= 15 and strong_blocks >= 22

def is_five_seventeen_trigger(u_mean):
    return abs(u_mean - FIVE_SEVENTEEN_TRIGGER) < 0.0015   # breiteres Fenster um 0.0066–0.0085
...
def choose_mode(r, theta, prev_mode, ncs_prox, escape_count, u_history, u_mean):
    ...
    if prev_mode == "gate_lock":
        if ncs_prox > 0.82: return "gate_lock"
...
def simulate_v30(time_steps=TIME_STEPS, seed=SEED):
    np.random.seed(seed)
    net = pp.networks.case57()
    base_p = net.load["p_mw"].copy()
    base_q = net.load.get("q_mvar", None)
    ...
    for t in range(time_steps):
        ...
        if mode == "capture":
            ...
            snap = K_SNAP * gate_score if gate_score > 0.82 else 0.0
...
controlled = simulate_v30()

print("✅ v30 (5×17 Prime Ultra-Break) erfolgreich ausgeführt!")
...
axs[0].plot(t, controlled["voltage_mean"], label="v30 Controlled")
...
fig.suptitle("NEXAH v30 – Golden Scarabaeus Möbius Breathing Pulse + 5×17 Prime Ultra-Break")
fig.tight_layout()
...
ax.set_title("Polar View – Root Cube Projection (v30)")
ax.legend()
fig.tight_layout()
...
ax.set_title("3D Root Cube Projection (v30)")
fig.tight_layout()
fig.savefig(CUBE_PATH, dpi=160)
plt.close(fig)

report = f"""NEXAH v30 Root Cube Navigation Report (5×17 Prime Ultra-Break + Full Möbius Rotation)
========================================
