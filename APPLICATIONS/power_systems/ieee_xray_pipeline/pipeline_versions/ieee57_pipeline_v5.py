"""
NEXAH IEEE 57 Pipeline v5 — Multi-Band Pre-Collapse Detector
============================================================

Goal:
- Classical collapse detection via voltage threshold
- NEXAH early detection via:
    * multi-band coherence structure
    * coherence slope
    * coherence acceleration
    * switch clustering
    * early instability score

This version is designed to decouple NEXAH detection from direct voltage threshold crossing.
"""
...
for t in range(time_steps):
    ...
    # Switch signal = derivative of voltage
    ...
    if nexah_event is None and t > 10:
        early_band_break = (
            band_96[-2] == 1 and band_96[-1] == 0
        )

        structural_drift = (
            coherence_slope_history[-1] < -0.002
            and switch_cluster_history[-1] > 0.008
        )

        instability_trigger = instability_score_history[-1] >= 3

        if early_band_break or structural_drift or instability_trigger:
            nexah_event = t
...
ax[3].axhline(-0.002, linestyle="--", label="Slope warn")
...
ax[5].axhline(3, linestyle="--", label="Trigger")
if classical_event is not None:
    ax[5].axvline(classical_event, linestyle="--", label="Classical")
if nexah_event is not None:
    ax[5].axvline(nexah_event, linestyle=":", label="NEXAH")
ax[5].set_ylabel("Score")
ax[5].set_xlabel("Time")
ax[5].grid(True)
ax[5].legend(loc="best")

title = "NEXAH IEEE 57 — Multi-Band Pre-Collapse Detection (v5)"
if lead_time is not None:
...
out_file = save_dir / "ieee57_pipeline_v5_multiband.png"
