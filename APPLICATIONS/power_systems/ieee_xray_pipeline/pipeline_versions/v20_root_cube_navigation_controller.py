"""
v20_root_cube_navigation_controller.py
=======================================

NEXAH v20 – Triple-Six Breathing Crystal (final tuning)
Counterclockwise + stronger breathing + tighter Elastic Axis hug
"""
...
TS_PATH    = OUTDIR / "ieee57_v20_root_cube_timeseries.png"
POLAR_PATH = OUTDIR / "ieee57_v20_root_cube_polar.png"
CUBE_PATH  = OUTDIR / "ieee57_v20_root_cube_3d_projection.png"
REPORT_PATH= OUTDIR / "ieee57_v20_root_cube_report.txt"

print(f"📁 Saving to → {OUTDIR.resolve()}\n")

# ============================================================
# Settings (v20 – tuned to your Triple-Six image)
# ============================================================
TIME_STEPS = 300
SEED = 42
...
K_FLOW          = 0.175      # stärkerer Flow
FLOW_PHASE      = np.pi / 2 + 0.3
K_LIFT          = 0.26
K_AXIS_PULL     = 0.16       # stärkerer Pull zur Axis
K_R_HOLD        = 0.085
K_SNAP          = 0.11
K_PULSE         = 0.09

# Breathing (orangene Perlenkette)
BREATH_FREQ     = 0.085
BREATH_AMP      = 0.015      # stärkeres Atmen

NCS_LOCKS_DEG = [97.0, 277.0, 292.0]
NCS_LOCKS     = np.deg2rad(NCS_LOCKS_DEG)
SNAP_TOL      = np.deg2rad(7.0)
...
def simulate_v20(time_steps=TIME_STEPS, seed=SEED):
    np.random.seed(seed)
    net = pp.networks.case57()
    base_p = net.load["p_mw"].copy()
    base_q = net.load.get("q_mvar", None)
    ...
    for t in range(time_steps):
        ...
        if mode == "capture":
            ...
            # Counterclockwise (wie in deinem Bild)
...
controlled = simulate_v20()

print("✅ v20 (stärkere Breathing Crystal) erfolgreich ausgeführt!")
...
axs[0].plot(t, controlled["voltage_mean"], label="v20 Controlled")
...
fig.suptitle("NEXAH v20 – Triple-Six Resonance Breathing Crystal (Counterclockwise)")
fig.tight_layout()
...
ax.set_title("Polar View – Root Cube Projection (v20)")
ax.legend()
fig.tight_layout()
...
ax.set_title("3D Root Cube Projection (v20)")
fig.tight_layout()
fig.savefig(CUBE_PATH, dpi=160)
plt.close(fig)

report = f"""NEXAH v20 Root Cube Navigation Report
========================================
