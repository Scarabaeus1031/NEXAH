"""
v25_root_cube_navigation_controller.py
=======================================

NEXAH v25 – Golden Scarabaeus Möbius Breathing Pulse + Prime Anchor
Integriert deine neuen 3D-Prime-Overlay + Regime Coloring Bilder
"""
...
TS_PATH    = OUTDIR / "ieee57_v25_root_cube_timeseries.png"
POLAR_PATH = OUTDIR / "ieee57_v25_root_cube_polar.png"
CUBE_PATH  = OUTDIR / "ieee57_v25_root_cube_3d_projection.png"
REPORT_PATH= OUTDIR / "ieee57_v25_root_cube_report.txt"

print(f"📁 Saving to → {OUTDIR.resolve()}\n")

# ============================================================
# Settings (v25 – Prime Anchor + Membrane Nudge)
# ============================================================
TIME_STEPS = 300
SEED = 42
...
# Prime-Anchor Nudge (aus deinen Distance-Comparison-Bildern)
PRIME_NUDGE_AMP = 0.012
PRIME_BREAK     = 64   # 2^6 → Trigger für starken Snap

NCS_LOCKS_DEG = [97.0, 277.0, 292.0]
NCS_LOCKS     = np.deg2rad(NCS_LOCKS_DEG)
SNAP_TOL      = np.deg2rad(4.5)
...
def simulate_v25(time_steps=TIME_STEPS, seed=SEED):
    np.random.seed(seed)
    net = pp.networks.case57()
    base_p = net.load["p_mw"].copy()
    base_q = net.load.get("q_mvar", None)
    ...
    for t in range(time_steps):
        ...
        nudge = PRIME_NUDGE_AMP * np.cos(BREATH_FREQ * t * 1.5)   # Prime-Anchor Nudge
...
controlled = simulate_v25()

print("✅ v25 (Prime Anchor + Möbius Breathing) erfolgreich ausgeführt!")
...
axs[0].plot(t, controlled["voltage_mean"], label="v25 Controlled")
...
fig.suptitle("NEXAH v25 – Golden Scarabaeus Möbius Breathing Pulse + Prime Anchor")
fig.tight_layout()
...
ax.set_title("Polar View – Root Cube Projection (v25)")
ax.legend()
fig.tight_layout()
...
ax.set_title("3D Root Cube Projection (v25)")
fig.tight_layout()
fig.savefig(CUBE_PATH, dpi=160)
plt.close(fig)

report = f"""NEXAH v25 Root Cube Navigation Report (Prime Anchor + Möbius Breathing)
========================================
