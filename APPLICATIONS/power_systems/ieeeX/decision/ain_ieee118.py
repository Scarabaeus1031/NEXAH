...
# =========================================
# REAL GRID SIMULATION (IEEE118)
# =========================================

np.random.seed(42)

solver = RealPowerFlowSolverGeneric("ieee118")

lambdas = np.linspace(0.8, 1.8, 80)  # 🔥 kleiner Bereich für Stabilität
...
for lam in lambdas:
    ...
    # simple fallback control during rollout
...
if np.all(centers == 0):
    states = ["SAFE" if r < 0.4 else "CRITICAL" for r in risk]
else:
...
results_dir = f"APPLICATIONS/power_systems/nexah_ieeeX/results/run_ieee118_{timestamp}"
