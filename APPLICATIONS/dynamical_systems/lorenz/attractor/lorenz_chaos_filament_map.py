"""
Lorenz Chaos Filament Map

Build a filament map from the strongest 3D FTLE points by connecting
nearby points in phase space.

This reveals the transport skeleton as a filament network.
"""
...
def extract_ridge_points(x_vals, y_vals, z_vals, ftle, percentile=96):
...
def plot_filaments(points, values, k=3):
    ...
    ax.set_title("Lorenz Chaos Filament Map")
    ax.set_xlabel("X")
    ax.set_ylabel("Y")
    ax.set_zlabel("Z")

    file_output = f"{OUTPUT_DIR}/lorenz_chaos_filament_map_{timestamp}.png"
...
def main():
    ...
    points, values = extract_ridge_points(x_vals, y_vals, z_vals, ftle, percentile=96)

    print("\nRendering Lorenz Chaos Filament Map...\n")
    plot_filaments(points, values, k=3)

    print("\nLorenz Chaos Filament Map complete.\n")
